...
usage() {
  cat <<'USAGE'
Usage: ./scripts/viewer-gameplay-attraction-playthrough.sh [options] [run-launcher-stack options...]

Run the TASK-GAME-076 first-30-minute attraction playthrough as a real browser
scenario. Unlike the smoke path, this script executes and asserts every design
beat in order.

Options:
  --url <url>               Use an existing viewer URL; skip stack bootstrap
  --out-dir <path>          Artifact root (default: output/playwright/gameplay-attraction-playthrough)
...
Artifacts:
  <out-dir>/<run-id>/states/*.json
  <out-dir>/<run-id>/gameplay-attraction-playthrough-summary.json
  <out-dir>/<run-id>/gameplay-attraction-playthrough-summary.md
USAGE
}
...
GAME_URL=""
OUT_ROOT="output/playwright/gameplay-attraction-playthrough"
...
summary_json_path="$out_dir/gameplay-attraction-playthrough-summary.json"
summary_md_path="$out_dir/gameplay-attraction-playthrough-summary.md"
screenshot_path="$out_dir/gameplay-attraction-playthrough.png"
session="gameplay-attraction-$run_id"
...
record_beat() {
  ...
  local reason=${6:-}
  local state_file=${7:-}
  python3 - "$summary_items_jsonl" "$beat_id" "$time_label" "$action_label" "$assertion_label" "$status" "$reason" "$state_file" <<'PY'
import json, sys
path, beat_id, time_label, action, assertion, status, reason, state_file = sys.argv[1:]
...
run_beat() {
  ...
  ab_eval "$session" "$action_js" >>"$ab_log" 2>&1 || true
  local ok="false"
  ...
  if [[ "$ok" == "true" ]]; then
    record_beat "$beat_id" "$time_label" "$action_label" "$assertion_label" "pass" "" "$state_file"
    return 0
  fi
  record_beat "$beat_id" "$time_label" "$action_label" "$assertion_label" "fail" "assertion timed out" "$state_file"
  return 1
}
...
overall_status="pass"
run_beat "0-1m" "identity_goal_landing" \
  "select target agent and request initial snapshot" \
  "(() => { window.__AW_TEST__.select('agent:${AGENT_ID}'); window.__AW_TEST__.sendGameplayAction('request_snapshot'); return true; })()" \
...
run_beat "1-3m" "first_control_proof" \
...
run_beat "3-5m" "first_consequence_read" \
  "refresh snapshot after first control" \
...
run_beat "5-7m" "first_choice_hook" \
  "inspect recommended action catalog" \
  "window.__AW_TEST__.sendGameplayAction('request_snapshot')" \
...
run_beat "7-10m" "first_blocker_recovery" \
  "advance or surface recovery after blocker" \
  "window.__AW_TEST__.sendControl('step', {count: 1})" \
...
run_beat "8-12m" "first_visible_output" \
  "request output/economy snapshot" \
  "window.__AW_TEST__.sendGameplayAction('request_snapshot')" \
...
run_beat "12-18m" "persistent_capability" \
  "advance and re-read capability state" \
  "window.__AW_TEST__.sendControl('step', {count: 1})" \
...
run_beat "18-23m" "first_expansion_tradeoff" \
  "inspect branch/choice strip through action catalog" \
  "window.__AW_TEST__.sendGameplayAction('request_snapshot')" \
...
run_beat "20-25m" "agency_correction" \
  "inspect agency moves after action history" \
  "window.__AW_TEST__.sendGameplayAction('request_snapshot')" \
...
run_beat "25-30m" "return_hook_small_player_value" \
  "final return-hook and share-replay inspection" \
  "window.__AW_TEST__.sendControl('step', {count: 1})" \
...
python3 - "$summary_items_jsonl" "$summary_json_path" "$summary_md_path" "$run_id" "$GAME_URL" "$overall_status" "$final_state_path" <<'PY'
import json
import pathlib
import sys
...
lines = [
    "# TASK-GAME-076 Playwright 30m Playthrough",
    "",
    f"- ok: `{payload['ok']}`",
    f"- runId: `{run_id}`",
...
printf 'TASK-GAME-076 playthrough summary: %s\n' "$summary_json_path"
printf 'TASK-GAME-076 playthrough report: %s\n' "$summary_md_path"
