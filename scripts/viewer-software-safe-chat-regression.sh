...
usage() {
  cat <<'USAGE'
Usage: ./scripts/viewer-software-safe-chat-regression.sh [options] [run-launcher-stack options...]

Run a Web-first QA regression for the canonical Viewer prompt/chat flow.
...
[[ "$auth_ready" == "true" ]] || { echo "error: viewer auth bootstrap is unavailable in canonical viewer mode" >&2; exit 1; }
...
lines = [
    '# Viewer prompt/chat regression summary',
    '',
    f"- ok: `{data['ok']}`",
