...
Purpose:
  Orchestrate one public-testnet-rehearsal rehearsal window around the same candidate truth:
  ...
  - network-rehearsal-track-gate invocation

Key flags:
  --window-id <id>                        Public testnet rehearsal window id (required)
  ...
  --out-dir <path>                        Output root (default: output/network-rehearsal)
...
Access / rollback:
  ...
  --fallback-candidate-bundle <path>      Previous public-testnet-rehearsal pass bundle to use as rollback target
...
Examples:
  ./scripts/public-testnet-rehearsal.sh \
    --window-id public-testnet-rehearsal-20260324-02 \
    --candidate-bundle output/release-candidates/public-testnet-rehearsal-dry-run-20260324-01.json \
...
out_root="output/network-rehearsal"
bundle_dir=""
...
mixed_topology_baseline_evidence_ref="doc/testing/evidence/p2p-mixed-topology-validation-matrix-2026-04-07.md"
...
if [[ -n "$candidate_bundle" ]]; then
  ...
  if (( ${#candidate_evidence_refs[@]} > 0 )); then
...
# Public Testnet Rehearsal Multi-Entry Closure Summary
...
{
  echo "# Public Testnet Rehearsal Access Check"
  echo
  echo "审计轮次: 2"
  echo
  echo "## Current Window"
  ...
  echo "- \`track\`: \`public_testnet_rehearsal\`"
  echo
...
governance_note="governance truth was not rerun inside this public-testnet-rehearsal window"
...
# Public Testnet Rehearsal Governance Drill Summary
...
# Public Testnet Rehearsal Short-Window Longrun Summary
...
rollback_note="there is no previous public-testnet-rehearsal pass candidate pinned as formal fallback"
...
# Public Testnet Rehearsal Rollback Target Note
...
mixed_topology_note="P2PARCH-6 matrix baseline is pinned, but network-rehearsal mixed-topology evidence remains incomplete and proxy drills are not equivalent to a dedicated sentry/NAT lab"
if [[ -z "$mixed_topology_baseline_evidence_ref" ]]; then
  mixed_topology_note="no mixed-topology baseline evidence is pinned; keep the network-rehearsal lane partial until P2PARCH-6 baseline or same-window evidence is attached"
fi
if [[ -n "$mixed_topology_shared_evidence_ref" ]]; then
...
if [[ "$mixed_topology_pass" -eq 1 ]]; then
  ...
  mixed_topology_note="same-window mixed-topology evidence is pinned and an audited producer/QA pass-uplift decision is attached for this public-testnet-rehearsal gate"
fi
cat >"$mixed_topology_summary_path" <<EOF
# Public Testnet Rehearsal Mixed-Topology Gate Note
...
lanes_tsv="$window_dir/lanes.public_testnet_rehearsal.tsv"
cat >"$lanes_tsv" <<EOF
...
if [[ "$(cat "$logs_dir/shared_network_gate.rc")" != "0" ]]; then
  echo "error: network-rehearsal gate generation failed" >&2
  exit 1
fi
...
payload = {
    "schema_version": "oasis7.public_testnet_rehearsal.v1",
...
summary_path="$window_dir/public-testnet-rehearsal-summary.md"
cat >"$summary_path" <<EOF
# Public Testnet Rehearsal Summary
...
echo "public-testnet-rehearsal rehearsal summary: $summary_path"
echo "public-testnet-rehearsal gate summary: ${gate_summary_path:-missing}"
echo "public-testnet-rehearsal lanes: $lanes_tsv"
