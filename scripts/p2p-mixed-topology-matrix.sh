...
relay_only_lane_budget|required|exact|substrate_exact|relay_only|Validate relay role stays confined to control-lane service surface.|Ensures relay/public ingress does not silently regain sync/blob request or serve rights.|source ./scripts/cargo-dev-lib.sh; oasis7_cargo_dev test -p oasis7_node network_policy_limits_relay_to_control_lane -- --nocapture
cgnat_relay_path_ranking|required|exact|substrate_exact|cgnat|Validate CGNAT peers rank direct before hole-punch before relay fallback.|Uses signed peer-record transport ordering as the exact proxy for no-public-IP path selection.|source ./scripts/cargo-dev-lib.sh; oasis7_cargo_dev test -p oasis7_net --features libp2p peer_record_transport_paths_rank_direct_before_hole_punch_before_relay -- --nocapture
