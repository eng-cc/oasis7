...
if OASIS7_PRODUCT_DOC_BASE="$base_oid" ./scripts/doc-governance-check.sh >"$tmp_dir/partial.out" 2>&1; then
...
if OASIS7_PRODUCT_DOC_HEAD="$head_oid" ./scripts/doc-governance-check.sh >"$tmp_dir/partial-head.out" 2>&1; then
...
valid_range="$(GITHUB_EVENT_PATH="$tmp_dir/pull_request.json" GITHUB_EVENT_NAME=pull_request GITHUB_SHA="$head_oid" product_doc_range)"
[[ "$valid_range" == "$base_oid
$head_oid" ]]

printf '{}\n' >"$tmp_dir/empty.json"
if GITHUB_EVENT_PATH="$tmp_dir/empty.json" GITHUB_EVENT_NAME=unknown GITHUB_SHA="$head_oid" product_doc_range >"$tmp_dir/event.out" 2>&1; then
...
if GITHUB_EVENT_PATH="$tmp_dir/push-missing-head.json" GITHUB_EVENT_NAME=push GITHUB_SHA="$head_oid" product_doc_range >"$tmp_dir/push.out" 2>&1; then
...
if CI=true GITHUB_ACTIONS=true GITHUB_EVENT_PATH="" GITHUB_EVENT_NAME="" product_doc_range >"$tmp_dir/no-event.out" 2>&1; then
