...
Description:
  ...
  1. pass case: rotate one signer inside a slot while preserving baseline signer count/threshold
  2. block case: intentionally degrade one slot below its baseline signer count
  3. rejoin case: re-import the baseline-compatible pass manifest on top of the degraded world
  Note:
