...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'USAGE'
Usage:
  scripts/sync-m5-builtin-wasm-artifacts.sh [options]

Options are forwarded to scripts/sync-m1-builtin-wasm-artifacts.sh with:
  --module-ids-path crates/oasis7/src/runtime/world/artifacts/m5_builtin_module_ids.txt
  --hash-path crates/oasis7/src/runtime/world/artifacts/m5_builtin_modules.sha256
...
"$ROOT_DIR/scripts/sync-m1-builtin-wasm-artifacts.sh" \
  --module-ids-path "$ROOT_DIR/crates/oasis7/src/runtime/world/artifacts/m5_builtin_module_ids.txt" \
  --hash-path "$ROOT_DIR/crates/oasis7/src/runtime/world/artifacts/m5_builtin_modules.sha256" \
  "$@"
