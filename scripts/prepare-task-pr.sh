...
Generated task PR bodies include a GitHub auto-close link to the bound task issue
when a GitHub-backed task issue can be resolved. Explicit body files must carry
the same close linkage.
...
pattern = re.compile(rf"\b(close[sd]?|fix(e[sd])?|resolve[sd]?)\s+#\s*{issue_number}\b", re.IGNORECASE)
raise SystemExit(0 if pattern.search(body) else 1)
PY
}
...
if [[ -n "$TASK_ISSUE_NUMBER" ]]; then
  if [[ -n "$BODY_FILE" ]]; then
    if ! body_file_has_task_close_link "$BODY_FILE" "$TASK_ISSUE_NUMBER"; then
      die "--body-file must include a GitHub auto-close link for the bound task issue, for example: Closes #$TASK_ISSUE_NUMBER"
    fi
  else
...
Generated by ./scripts/prepare-task-pr.sh."
  ...
  die "cannot resolve GitHub task issue number for $LOCAL_ROLE_REVIEW_TASK_UID; refusing to create a task PR without a close link"
