...
SYSTEM_DESIGN_TRACEABILITY_CHECKER="$SOURCE_WORKTREE/scripts/system-design-traceability-check.py"
[[ -f "$SYSTEM_DESIGN_TRACEABILITY_CHECKER" ]] || die "system-design traceability checker is missing: $SYSTEM_DESIGN_TRACEABILITY_CHECKER"
SYSTEM_DESIGN_TRACEABILITY_COMMAND="$(render_cmd "$PRODUCT_DOC_PYTHON" "$SYSTEM_DESIGN_TRACEABILITY_CHECKER" \
  --repo-root "$SOURCE_WORKTREE" --base "$COMPARISON_HEAD" --head "$SOURCE_HEAD" --worktree)"
if ! SYSTEM_DESIGN_TRACEABILITY_OUTPUT="$(cd "$SOURCE_WORKTREE" && "$PRODUCT_DOC_PYTHON" "$SYSTEM_DESIGN_TRACEABILITY_CHECKER" \
  --repo-root "$SOURCE_WORKTREE" --base "$COMPARISON_HEAD" --head "$SOURCE_HEAD" --worktree 2>&1)"; then
  printf '%s\n' "$SYSTEM_DESIGN_TRACEABILITY_OUTPUT" >&2
  die "system design traceability gate failed for $COMPARISON_HEAD..$SOURCE_HEAD"
fi
...
if [[ -x "$PLANNER_SCRIPT" ]]; then
  ...
  if RUST_SCOPE_OUTPUT="$(cd "$SOURCE_WORKTREE" && "$PLANNER_SCRIPT" "${PLANNER_ARGS[@]}" 2>/dev/null)"; then
    ...
    if [[ "$LOCAL_REQUIRED_SCOPE" != "minimal" ]]; then
      ...
      RUN_PIXEL_WORLD_BRIDGE_LIB_TESTS="$(plan_kv_get_default "$RUST_SCOPE_OUTPUT" "run_pixel_world_bridge_lib_tests" "false")"
      RUN_PIXEL_WORLD_BRIDGE_WASM_CHECK="$(plan_kv_get_default "$RUST_SCOPE_OUTPUT" "run_pixel_world_bridge_wasm_check" "false")"
      ...
      RUN_OPERATIONAL_CONTRACTS="$(plan_kv_get_default "$RUST_SCOPE_OUTPUT" "run_operational_contracts" "false")"
      RUN_SITE_CONTRACT_TESTS="$(plan_kv_get_default "$RUST_SCOPE_OUTPUT" "run_site_contract_tests" "false")"
...
OASIS7_CI_RUN_PIXEL_WORLD_BRIDGE_LIB_TESTS=$RUN_PIXEL_WORLD_BRIDGE_LIB_TESTS \
OASIS7_CI_RUN_PIXEL_WORLD_BRIDGE_WASM_CHECK=$RUN_PIXEL_WORLD_BRIDGE_WASM_CHECK \
...
OASIS7_CI_RUN_OPERATIONAL_CONTRACTS=$RUN_OPERATIONAL_CONTRACTS \
OASIS7_CI_RUN_SITE_CONTRACT_TESTS=$RUN_SITE_CONTRACT_TESTS \
OASIS7_CI_RUN_CODEX_AGENT_CONFIG_VALIDATION=$RUN_CODEX_AGENT_CONFIG_VALIDATION \
...
CLEANUP_CMD_1="$(render_cmd \
  "$CANONICAL_REPO_ROOT/scripts/pm/finalize-task.sh" \
  --repo-root "$CANONICAL_REPO_ROOT" \
  --task-uid "${LOCAL_ROLE_REVIEW_TASK_UID:-unknown}" \
  --pr '<pr-number>' --resume --json)"
CLEANUP_CMD_2=""
