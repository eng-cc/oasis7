...
branch = __import__('subprocess').check_output(["git","branch","--show-current"], text=True, cwd=root).strip()
worktree_name = root.name

def parse_task(path: pathlib.Path):
    data = {}
    for line in path.read_text(encoding='utf-8').splitlines():
        m = re.match(r"^([a-z_]+):\s*(.*)$", line)
        if m and m.group(1) in {"task_uid","title","owner_role","worktree_hint","execution_log_path","status","last_started_at","last_closed_at","last_claim_type","last_verified_at","last_verification_status"}:
            v = m.group(2).strip().strip('"')
            data[m.group(1)] = "" if v == "null" else v
...
if explicit_uid:
    ...
    bound = [t for t in tasks if t.get("worktree_hint") in {branch, worktree_name}]
...
check(proj.exists(), "project.md missing; fix: create or point to module project doc")
if proj.exists():
    ptxt = proj.read_text(encoding='utf-8')
    check(f"Trace: .pm/tasks/{uid}.yaml" in ptxt, f"project.md lacks Trace for {uid}; fix: add 'Trace: .pm/tasks/{uid}.yaml' to task item")

elog = root / task.get("execution_log_path","")
check(elog.exists(), f"execution log missing: {elog.relative_to(root) if elog else '(none)'}; fix: run workflow-report --phase start or update execution_log_path")
if elog.exists():
