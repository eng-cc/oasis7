...
live={"task_uid":uid,"title":"t","issue_number":1,"issue_url":"https://github.com/o/r/issues/1","status":"committed","acceptance":[],"worktree_hint":"/tmp"}
...
def invoke_refresh(existing, node, canonical_project=None, expect_success=False):
    with tempfile.TemporaryDirectory() as td:
        root=pathlib.Path(td); (root/".pm/github-project-sync").mkdir(parents=True)
        record=dict(live); record["project_item_id"]="ITEM" if existing else ""
        project=canonical_project or {"owner":"o","number":1}
        mapping={"version":1,"project":project,"tasks":{uid:record}}
