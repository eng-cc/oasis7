#!/usr/bin/env python3
"""Validate immutable per-role pre-PR review return provenance."""
...
def main() -> int:
    ...
    for line_no, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        mandatory = ("slice_id","dispatch_receipt","activation","context_delivery","actual_runtime","artifact_digest","scope_verdict","risk_verdict","findings","residual_risk")
        ...
        if not re.fullmatch(r"[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}", dispatch_id, re.I):
            p.error(f"slice_id is not a strict runtime dispatch UUID for {role}")
    ...
    print(json.dumps({"status":"passed","roles":sorted(seen),"source_head":args.source_head}, sort_keys=True))
    return 0
