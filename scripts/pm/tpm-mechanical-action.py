...
ap=argparse.ArgumentParser(); ap.add_argument("--phase",required=True,choices=PHASES); ap.add_argument("--task-uid",required=True); ap.add_argument("--repo",type=Path,required=True); ap.add_argument("--state",type=Path,required=True); ap.add_argument("--json",action="store_true"); a=ap.parse_args()
