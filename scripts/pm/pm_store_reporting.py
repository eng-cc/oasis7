...
def build_workflow_checklist(
    ...
    if phase == "start":
        ...
        add(
            "prepare-pr-review",
            "创建 PR 前必须先完成本地相关角色 subagent review 并写入 `Pre-PR Local Role Review: passed` evidence packet；随后通过 `./scripts/prepare-task-pr.sh` 执行 PR preflight / create。普通 PR 创建后默认继续盯 GitHub required checks、review/approval、mergeability、PR comments 与 unresolved review threads，失败或存在 actionable comments 就修复/验证/推送或回复/resolve，通过且 comments/thread 已收口后合入并清理；只有明确用于 manual packaging/release CI 的 PR 才能停在人工打包 gate。",
