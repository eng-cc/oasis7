...
checks = [
    (
        ...
        [
            ...
            "Default subagent runtime is `gpt-5.4` with `reasoning_effort=medium`",
    ...
    (
        ...
        [
            ...
            "只读/聊天请求不默认进入 task/worktree bootstrap",
        ],
    ),
    (
...
scenarios = [
    ...
    {
        "id": "read_only_professional_judgment_uses_role_slice_without_task_bootstrap",
        "expected_route": "matching professional role slice without default-workflow-bootstrap unless writeback follows",
        "surface": "doc/engineering/workflow/source-of-truth.md",
        "required_markers": [
            "Read-only/chat-only requests still split by judgment type:",
            "Pure fact lookup, path lookup, command-output restatement",
            "Read-only professional/domain questions must be dispatched to the matching bounded professional role slice",
            "do not by themselves require `.pm` task creation, a task worktree, or PR chain",
            "The required sink for an unbound read-only professional slice is the role-tagged user-facing answer",
            "no `.pm` execution-log sink is required unless the work is already task-bound or repository writeback follows",
            "If the answer leads to repository writeback, create or enter the standard task worktree before edits.",
        ],
    },
    {
        "id": "unbound_read_only_professional_question_skips_router_entrypoint",
        "expected_route": "TPM direct dispatches matching role slice; router only applies after task truth or writeback need",
        "surface": ".agents/skills/repo-owned-workflow-router/SKILL.md",
        "required_markers": [
            "Read-only/chat-only requests do not enter this router unless they are already bound to task truth or need repository writeback.",
            "Already-bound read-only professional/domain judgment",
            "Apply this router step only when the read-only professional/domain judgment is already task-bound or writeback-driven.",
            "this router is not the entrypoint for unbound read-only professional questions",
            "TPM dispatches the matching bounded role slice directly and uses the role-tagged user-facing answer as the sink.",
        ],
    },
    {
    ...
    },
    {
        ...
        "expected_route": "TPM records gpt-5.4-medium as the default subagent model configuration",
        "surface": "doc/engineering/workflow/source-of-truth.md",
        "required_markers": [
            "Default subagent runtime is `gpt-5.4` with `reasoning_effort=medium`",
    ...
    },
    {
        ...
        "surface": ".agents/skills/repo-owned-workflow-router/SKILL.md",
        "required_markers": [
            "For task-bound or repository-changing work, TPM TODO decomposition and subagent slice contracts must be recorded in `.pm/tasks/<TASK-UID>.execution.md` before delegated execution begins.",
            "task-bound or repository-changing work",
            "mandatory `.pm` execution-log sink",
...
payload = {
    ...
    "expected_agent_behavior": [
        ...
        "read-only professional/domain questions use matching role slices without forcing task/worktree bootstrap unless repository writeback follows",
        ...
        "subagent slice contracts record gpt-5.4-medium as the default model configuration unless an override reason is present",
    ...
    "failure_signature": [
        ...
        "subagent model configuration markers disappear or no longer default to gpt-5.4-medium",
