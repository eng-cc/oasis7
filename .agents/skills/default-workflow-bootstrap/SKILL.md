...
- a new task is starting and you have not yet confirmed whether it is trivial
- the user said `做`, `继续`, `landing`, or otherwise expects end-to-end execution
- you need to decide whether a dedicated task worktree and `.pm` task must be created
...
2. For non-trivial work, verify workflow state in this order:
...
- Do not skip worktree / `.pm` task creation for non-trivial work unless the user explicitly authorized reuse.
...
- Expected result:
  ...
  - workflow behavior eval proves the default chain still routes through repo-owned truth
