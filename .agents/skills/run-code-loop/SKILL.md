---
name: run-code-loop
description: Use when the user explicitly requests a bounded code loop task or continuation of that task.
---
...
1. Use default-workflow-bootstrap for the one canonical task/worktree; bind loop `code`, manual request reference, fixed base, immutable inputs and owner through the PM adapter. Preserve legacy tasks unless the user explicitly requests migration with a new bootstrap epoch.
