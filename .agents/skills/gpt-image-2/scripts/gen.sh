...
# `--ignore-rules` keeps repo-specific AGENTS/skill rules from steering the
# nested Codex run into documentation lookups or code-native SVG fallbacks.
args=(exec --skip-git-repo-check --ignore-rules --sandbox read-only --color never --enable image_generation)
...
instruction="Generate an image using the built-in image generation capability enabled for this run."
if [[ ${#REF_IMAGES[@]} -gt 0 ]]; then
  instruction+=" Use the attached image(s) as visual reference / input for image-to-image."
fi
instruction+=$'\nRequirements: do not inspect repository files, do not read local skills or CLI docs, do not explain your steps, do not return SVG/HTML/code, and return only the generated image.\n\nRequest:\n'"$PROMPT"
