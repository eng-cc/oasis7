...
Pre-PR local role review is required for the frozen source head before promotion. The default v2 path creates a source-only plan from the verified impact projection, then runs trusted exact-head CI and professional review concurrently. Pre-PR Ready and promotion fail closed until the current CI identity and role-complete review identity join; review supplements, never replaces, GitHub checks, comments, requested changes, or mergeability. Explicit v1 is compatibility-only.
