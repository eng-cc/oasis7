...
- `subagent-slice-card` 或等价 handoff，明确每个专业角色 subagent 的边界
...
- 所有专业角色工作均以 subagent slice 形式出现，并有 write scope、return contract、formal sink 与 integration order。
...
- 是否为每个专业角色 subagent 写清 slice type、write scope、return contract、formal sink、integration order。
