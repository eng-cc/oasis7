...
- 是否区分“质量缺陷”与“运营/沟通问题”
- 若 `liveops_community` 是 task owner，是否在开始/收口时执行 `./scripts/pm/workflow-report.sh --phase start|close --role liveops_community --task-uid <TASK-UID>`；若作为 `tpm` 派生的 bounded subagent slice，是否把 start/close/finding 证据回写到 `.pm/tasks/<TASK-UID>.execution.md`，而不是用非 owner role 调用 `workflow-report`
...
- 高价值社区/事故信号是否已通过 `./scripts/pm/promote-signal.sh` 回流到 `.pm/`，而不是只留在 runbook / task execution log
- 是否更新相关对外说明或运营复盘文档
- 是否已回写 `.pm/tasks/<TASK-UID>.execution.md` 与相关对外说明 / 运营复盘文档
