...
10. commit 前必须开一个独立 subagent review 当前改动
   ...
   4. 若当前运行环境要求用户显式授权派生 agent，推荐使用以下用户授权模板取得一次性授权：`“在本仓库任务中，你可以按默认工作流自行派生 subagent 做 review；除非我明确禁止。”`
   5. 上述模板仅用于帮助用户给出“明确授权”；是否可执行仍以上层运行环境、平台策略与用户当次指令为准，不得把仓库规则表述成覆盖上层约束的绝对授权
   6. 若用户明确要求“先不要提交”，也要先完成 review，再保留本地改动
