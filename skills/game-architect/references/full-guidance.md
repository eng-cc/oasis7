...
This file preserves the detailed guidance that used to live in `game-architect/SKILL.md`. The entrypoint now uses progressive disclosure: read this file only when the task needs the detailed patterns, examples, or command catalogue.
...
- Architecture documents may supplement `prd.md`, GitHub task issue evidence comments, and handoff truth, but they do not replace GitHub-backed task truth.
