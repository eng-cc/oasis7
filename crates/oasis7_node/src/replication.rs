...
impl ReplicationRuntime {
    ...
    /// installed a checkpoint closure.  Probe tooling deliberately reads this
    /// from its own fresh replication root rather than accepting operator JSON.
    pub(crate) fn persist_checkpoint_verification_receipt(
        ...
        // Normal sync deliberately leaves no operator-consumable receipt.
        // A clean-root probe supplies an unpredictable nonce and accepts only
        // the receipt produced by that exact runtime invocation.
        ...
        // `rename` overwrites an existing receipt on POSIX.  A probe receipt
        // is immutable evidence for a single fresh root, so atomically create
        // the final name instead and fail closed if it already exists.
