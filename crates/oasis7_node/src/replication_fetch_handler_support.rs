...
use crate::replication::{
    ...
    load_latest_commit_head_index_height_from_root,
};
...
pub(super) fn attach_checkpoint_for_fetch_commit_if_boundary(
    ...
    world_id: &str,
    node_id: &str,
    _replication: &NodeReplicationConfig,
    request_height: u64,
    ...
    let Some(message_height) =
        parse_replication_commit_payload(message.payload.as_slice()).map(|payload| payload.height)
    else {
        return Ok(Some(message));
    };
    let should_export = if let Some(latest_height) =
        load_latest_commit_head_index_height_from_root(root_dir, world_id)?
    {
        should_export_checkpoint_for_fetch_commit(request_height, latest_height)
    } else {
        // A normal handler has a startup-reconciled index. If a legacy or
    ...
    let mut runtime = ReplicationRuntime::new_for_fetch_commit_attachment(_replication, node_id)?;
    runtime
