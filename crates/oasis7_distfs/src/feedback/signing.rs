...
use super::{
    ...
    FeedbackTombstoneRequest, WorldError, blake3_hex, decode_hex_array,
    feedback_create_content_hash, to_canonical_cbor,
};
...
fn sign_feedback_request(
    ...
    if expected_public_key_hex != payload.actor_public_key_hex {
        return Err(WorldError::DistributedValidationFailed {
            reason: format!(
                "signing key does not match actor public key: expected={} actual={}",
                expected_public_key_hex, payload.actor_public_key_hex
            ),
        });
    }
