...
pub use auth::{
    ...
    verify_collect_data_auth_proof, verify_fragment_refill_preview_auth_proof,
    ...
    ExclusiveDirectoryProcessLock, PromptControlAuthIntent, VerifiedPlayerAuth,
    HOSTED_REGISTRATION_ISSUER_PRIVATE_KEY_ENV, HOSTED_REGISTRATION_ISSUER_PUBLIC_KEY_ENV,
    HOSTED_REGISTRATION_REPLAY_LEDGER_PATH_ENV,
...
pub use gameplay_actions::{
    ...
    ACTION_CLAIM_AGENT, ACTION_CLAIM_FIRST_AGENT, ACTION_CLAIM_STARTER_OC,
    ACTION_RELEASE_AGENT_CLAIM, ACTION_SCHEDULE_ASSEMBLER_CONTROL_CHIP,
...
pub use protocol::{
    ...
    RefineQuotePreflight, RefineQuoteRequest, ViewerControl, ViewerControlProfile, ViewerRequest,
    ViewerResponse, ViewerStream, VIEWER_PROTOCOL_VERSION,
};
pub use rollback_audit_evidence::{
    build_unsigned_strict_audit_evidence, strict_audit_artifact_digest,
    strict_audit_manifest_digest, RollbackStrictAuditEvidenceInput,
};
#[cfg(not(target_arch = "wasm32"))]
pub use runtime_live::{
    runtime_agent_chat_echo_enabled_from_env, viewer_bootstrap_formal_release_runtime_world,
    viewer_bootstrap_generated_sidecar_runtime_world, ChainLinkPolicy, ViewerRuntimeLiveServer,
