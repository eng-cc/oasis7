        ...
        match effective_provider_base_url(config) {
            Ok(base_url) => {
                if let Some(transport) = provider_transport {
                    if validate_provider_base_url_for_transport(base_url.as_str(), transport)
                        .is_err()
                    {
                        issues.push(
            ...
            Err(_) => issues
                .push("agent provider URL is required when auto-discover is disabled".to_string()),
        }
...
fn provider_backed_is_requested(config: &LauncherConfig) -> bool {
    canonical_agent_decision_source(config.agent_decision_source.as_str())
...
fn validate_provider_base_url_for_transport(base_url: &str, transport: &str) -> Result<(), String> {
...
fn parse_http_base_url(base_url: &str, label: &str) -> Result<(String, u16), String> {
...
fn canonical_provider_execution_mode(raw: &str) -> Option<&'static str> {
...
fn effective_provider_base_url(config: &LauncherConfig) -> Result<String, String> {
...
fn parse_agent_provider_connect_timeout_ms(config: &LauncherConfig) -> Result<u64, String> {
    parse_positive_u64(
