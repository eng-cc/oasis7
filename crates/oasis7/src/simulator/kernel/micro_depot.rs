...
impl WorldKernel {
    pub fn micro_depot_install_quote(
        ...
        if supported_resource_kinds.as_slice() != ["data"] {
            reasons.push("invalid supported resource kinds; canonical micro_depot resources are [data]".to_string());
        }
        ...
        if let Some(stock) = self.model.agents.get(installer_agent_id).map(|agent| &agent.resources)
        {
            for debit in &commissioning.install_cost_resources {
                ...
                if available < debit.amount {
                    reasons.push(format!(
                        ...
                        micro_depot_resource_kind_label(debit.kind), debit.amount, available
                    ));
                }
            }
        }
        ...
        if projected_commissioning_service_uses == 0 && prospective_preview.is_some() && reasons.is_empty() {
            reasons.push("prospective service debit cannot be supported by commissioning inventory".to_string());
        }
        let low_use_warning = projected_commissioning_service_uses < break_even_uses || projected_commissioning_service_uses == 0;
        let recommendation_reason = if projected_commissioning_service_uses == 0 {
            ...
            format!("low-use warning: projected uses {projected_commissioning_service_uses} are below break_even {break_even_uses}")
        } else {
            format!("recommended: projected uses {projected_commissioning_service_uses} meet break_even {break_even_uses}")
        };
        ...
        quote.deployable = quote.reasons.is_empty() && quote.projected_commissioning_service_uses > 0;
        Ok(quote)
    }
