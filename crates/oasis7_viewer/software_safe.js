...
function buildAuthSurfaceModel() {
  ...
  const source = state.hostedAccess ? state.auth.available ? state.auth.source === "legacy_viewer_auth_bootstrap" ? "legacy_viewer_auth_bootstrap+hosted_access_hint" : "hosted_player_issue+browser_local_device_session" : "hosted_access_hint" : state.auth.available ? state.auth.source : "guest_only";
  return {
    deploymentHint,
    source,
    currentTier,
    ...
    tiers: [
      ...
      },
      {
        ...
        status: state.auth.available ? state.auth.source === "legacy_viewer_auth_bootstrap" ? "active_legacy_preview" : state.auth.registrationStatus === "registered" ? "active_hosted_session" : "issued_pending_register" : "not_issued",
...
function buildGameplaySummary(locale = state.uiLocale) {
  ...
  const acceptedIntentSummary = intentSummary || acceptedIntentId || localeText(
    locale,
    "还没有一条被正式接受的玩家意图",
