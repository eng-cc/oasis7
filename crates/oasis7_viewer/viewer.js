...
function createViewerAuthSurfaceModule({
    ...
    if (promptPolicy.availability === "public_player_plane_with_backend_reauth_preview") {
      ...
      if (state2.auth.registrationStatus === "registered") {
        return {
          ...
          reason: "hosted preview backend reauth is available after the browser-local player_session has completed runtime registration for prompt_control"
        };
      }
      return {
  ...
  function buildAuthSurfaceModel2() {
    ...
    const source = state2.hostedAccess ? state2.auth.available ? state2.auth.source === "legacy_viewer_auth_bootstrap" ? "legacy_viewer_auth_bootstrap+hosted_access_hint" : "hosted_player_issue+browser_local_ephemeral_key" : "hosted_access_hint" : state2.auth.available ? state2.auth.source : "guest_only";
    return {
      deploymentHint,
      source,
      currentTier,
  ...
  function buildHostedRecoveryHint2(locale = state2.uiLocale) {
    ...
    if (errorText.includes("released locally")) {
      return {
        ...
        cta: localeText2(locale, "重新申请 Hosted Player Session", "Re-acquire Hosted Player Session")
      };
    }
    ...
    return {
      ...
      cta: localeText2(locale, "申请 Hosted Player Session", "Acquire Hosted Player Session")
    };
  }
  return {
    authDeploymentHint,
