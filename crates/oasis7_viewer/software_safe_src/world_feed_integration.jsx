...
export function WorldFeedSurface({ core, locale, tr, onReloadSnapshot, onRetryFeed, observeState }) {
  const feed = () => {
    observeState?.();
    return core.state.worldFeed;
  };
  const retryFeed = () => {
  ...
  return (
    <WorldFeedPanel
      ...
      resolveModuleVisualEntity={(event) => {
        observeState?.();
        return core?.entityCollections?.().moduleVisualEntities.find((entry) => entry.id === event?.module_visual_entity_id) || null;
      }}
      onFocusModule={(event) => {
