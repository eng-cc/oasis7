...
export function TransferMaterialQuoteCard(props) {
  ...
  return <section class="panel panel--nested" data-testid="transfer-material-quote" data-quote-kind="preflight">
    ...
    <div class="panel__body stack"><div class="badge-row"><span class="badge badge--accent">{tr(locale(), "预估", "quote")}</span><span class="badge">{`${tr(locale(), "物料", "Material")}: ${materialLabel(quote().kind, locale(), tr)}`}</span><span class="badge">{`${tr(locale(), "距离", "Distance")}: ${display(quote().distance_km)} km`}</span></div>
      ...
      <div class="summary-grid"><Metric label={tr(locale(), "预计收到", "Expected received")} value={display(quote().expected_received_amount)} /><Metric label={tr(locale(), "预计损失", "Expected loss")} value={display(quote().expected_loss_amount)} /><Metric label={tr(locale(), "到达时间", "Arrival") } value={`${display(quote().ticks_until_arrival)} ${tr(locale(), "步后", "ticks")}`} /><Metric label={tr(locale(), "预计就绪", "Ready at")} value={tr(locale(), `第 ${display(quote().ready_at)} 步`, `Tick ${display(quote().ready_at)}`)} /><Metric label={tr(locale(), "优先级", "Priority")} value={priorityCopy(quote().effective_priority, locale(), tr)} /><Metric label={tr(locale(), "在途容量", "Transit capacity")} value={`${display(quote().inflight_before)} / ${display(quote().inflight_capacity)}`} /><Metric label={tr(locale(), "来源库存", "Source after")} value={`${display(quote().source_amount_before)} → ${display(quote().source_amount_after)}`} /><Metric label={tr(locale(), "目的地库存", "Destination after")} value={`${display(quote().destination_amount_before)} → ${display(quote().destination_expected_amount_after)}`} /></div>
...
export function TransferMaterialQuotePanel(props) {
  const [fromLedger, setFromLedger] = createSignal("site:source"); const [toLedger, setToLedger] = createSignal("site:destination"); const [kind, setKind] = createSignal("iron_ingot"); const [kindInput, setKindInput] = createSignal(null); const [amount, setAmount] = createSignal("20"); const [distance, setDistance] = createSignal("200"); const [priority, setPriority] = createSignal(""); const [requesting, setRequesting] = createSignal(false); const [localError, setLocalError] = createSignal(""); const locale = () => props.locale; const tr = props.tr; const remote = () => props.requestState || {};
