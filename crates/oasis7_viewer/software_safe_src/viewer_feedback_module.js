    ...
    if (feedback.stage === "error") {
      if (code === "llm_init_failed") {
        ...
        description.summary = isLocaleZh(locale)
          ? "当前栈没有可用的远程 provider bridge，因此无法开始聊天。"
          : "Chat cannot start because this stack has no usable remote provider bridge.";
        description.detail =
          isLocaleZh(locale)
            ? "请确认 launcher 使用 --newapi-user-ref 或等价 provider bearer selector，并且远程 provider bridge 的 /v1/provider/info 可访问。"
            : "Start the launcher with --newapi-user-ref or an equivalent provider bearer selector, and verify the remote provider bridge /v1/provider/info endpoint.";
        return description;
      }
