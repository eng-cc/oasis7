...
function pageStateScript() { return String.raw`(() => { const state = window.__AW_TEST__?.getState?.() || {}; const canvas = document.querySelector('#pixel-world-embedded-runtime-canvas'); const dto = window.__OASIS7_PIXEL_WORLD_RENDER_DTO__?.() || null; return JSON.stringify({ runtimeStatus: state.pixelWorldRuntimeStatus, runtimeSource: state.pixelWorldRuntimeSource, fatal: state.pixelWorldFatal || state.lastError || null, canvas: Boolean(canvas), fixture: document.querySelector('.pixel-world-host')?.dataset.visualFixture || null, modules: (dto?.module_visual_entities || []).map(({ id, pos }) => ({ id, pos })), control: Boolean(window.__OASIS7_MODULE_VISUAL_FIXTURE_CONTROL__) }); })()`; }
...
try {
  ...
  for (const [name, width, height] of [["desktop", 1440, 900], ["narrow", 390, 844]]) {
    ...
    assert(state.modules.map((entity) => entity.id).join(",") === "module-absolute,module-agent,module-location", "three projected markers are not present in stable order", state);
    ...
    const updated = await evalJson(String.raw`(() => { window.__OASIS7_MODULE_VISUAL_FIXTURE_CONTROL__.update({"module-update":{"entity_id":"module-update","module_id":"fixture-module","kind":"opaque_fixture_kind","anchor":{"type":"absolute","data":{"x_cm":1850000,"y_cm":3600000,"z_cm":0}}}}); return JSON.stringify(true); })()`);
    assert(updated === true, "module fixture update control failed");
    const updatedState = await evalJson(String.raw`(async()=>{const deadline=Date.now()+5000; while(Date.now()<deadline){const state=JSON.parse(${pageStateScript()}); if(state.modules.map((entity)=>entity.id).join(',') === 'module-update') return JSON.stringify(state); await new Promise((resolve)=>setTimeout(resolve,50));} throw new Error('updated marker was not rendered');})()`);
    ...
    summary.viewports[name] = { width, height, state, clearedState, updatedState, beforePng, clearedPng, updatedPng, consolePath };
  }
