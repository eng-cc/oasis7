...
function pageStateScript() { return String.raw`(() => { const state = window.__AW_TEST__?.getState?.() || {}; const canvas = document.querySelector('#pixel-world-embedded-runtime-canvas'); const dto = window.__OASIS7_PIXEL_WORLD_RENDER_DTO__?.() || null; return JSON.stringify({ runtimeStatus: state.pixelWorldRuntimeStatus, runtimeSource: state.pixelWorldRuntimeSource, camera: state.pixelWorldCamera || null, fatal: state.pixelWorldFatal || state.lastError || null, canvas: Boolean(canvas), fixture: document.querySelector('.pixel-world-host')?.dataset.visualFixture || null, modules: (dto?.module_visual_entities || []).map(({ id, kind, pos }) => ({ id, kind, pos })), control: Boolean(window.__OASIS7_MODULE_VISUAL_FIXTURE_CONTROL__) }); })()`; }
...
try {
  ...
  for (const [name, width, height] of [["desktop", 1440, 900], ["narrow", 390, 844]]) {
    ...
    summary.viewports[name] = { width, height, state, closeState, closePanState, overviewState, closeViewportPng, overviewViewportPng, clearedState, updatedState, beforePng, beforeCanvasPng, beforeViewportPng, clearedPng, clearedCanvasPng, clearedViewportPng, updatedPng, updatedCanvasPng, updatedViewportPng, canvasDimensions: { before: beforeCanvasDimensions, cleared: clearedCanvasDimensions, updated: updatedCanvasDimensions }, consolePath };
  }
