...
function pageStateScript() { return String.raw`(() => { const canvas = document.querySelector('#pixel-world-embedded-runtime-canvas'); const state = window.__AW_TEST__?.getState?.() || {}; const gl = canvas?.getContext('webgl2') || canvas?.getContext('webgl'); const debug = gl?.getExtension('WEBGL_debug_renderer_info'); return { rendererReady: document.querySelector('.pixel-world-canvas')?.dataset.rendererReady === 'true', runtimeStatus: state.pixelWorldRuntimeStatus, runtimeSource: state.pixelWorldRuntimeSource, fatal: state.pixelWorldFatal || state.lastError || null, canvas: canvas ? { width: canvas.width, height: canvas.height, rect: (() => { const r = canvas.getBoundingClientRect(); return { width:r.width, height:r.height }; })() } : null, browserEnv: { userAgent: navigator.userAgent, devicePixelRatio: window.devicePixelRatio, webglRenderer: debug ? gl.getParameter(debug.UNMASKED_RENDERER_WEBGL) : null, webglVendor: debug ? gl.getParameter(debug.UNMASKED_VENDOR_WEBGL) : null }, fixture: document.querySelector('.pixel-world-host')?.dataset.visualFixture || null }; })()`; }
