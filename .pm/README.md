...
- 建议把 `workflow-report` 作为 worktree 创建后的第一条 PM 命令；收口时优先使用 `task-closeout.sh` 完成 close-phase，再在 commit 后立即进入 `prepare-task-pr.sh`。普通 PR 创建后默认继续盯 GitHub required checks、review/approval、mergeability 与 unresolved review threads，失败就修复/验证/推送，通过后合入并清理；只有明确用于 manual packaging/release CI 的 PR 才能停在人工打包 gate。`prepare-task-pr.sh` 还会基于当前 changed paths 给出一条本地 required 验证建议与 planner `reason_summary`，但这些输出只负责推荐/解释，不自动执行，也不改写 `./scripts/ci-tests.sh required/full` 的既有语义。
