...
- 模式总口径（`PRD-CORE-009`）：
  ...
  - `player_parity` / `headless_agent` / `debug_viewer` 是 `agent_direct_connect` 的 execution lane；当前默认 provider implementation 仍为 `openclaw_local_http`，这些字段只描述 Agent 的执行或观战方式，不构成额外玩家访问模式。
...
- 若需要直接以纯 API 客户端操作 live 会话，可使用 `cargo run -q -p oasis7 --bin oasis7_pure_api_client -- ...`。
  - 该链路属于 `pure_api` 玩家访问模式；若同时牵涉 `agent_direct_connect`（当前 provider implementation=`openclaw_local_http`），应额外标注实际 execution lane。
  - 推荐最小链路：
```bash
