...
- 2026-06-24 addendum：Viewer entry reason contract 已统一到 `direct_viewer_entry`；本证据文件中的 URL 示例同步移除旧 reason label，避免后续治理继续把旧 label 当作当前模式。
- 因此当前可确认的真实状态是:
...
- 默认 `/` 最终 URL:
  - `http://127.0.0.1:4273/software_safe.html?...&render_mode=software_safe&software_safe_reason=direct_viewer_entry`
