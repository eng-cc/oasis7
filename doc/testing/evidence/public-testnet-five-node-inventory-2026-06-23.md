...
4. Update observers one at a time. For the macOS arm64 observer, preflight the package/host/launchd target and back up runtime/config/provenance first; stop the original service; then create and verify the persistent-state backup while it is stopped. State roots must be unique, non-nested, non-symlink directories below—but not equal to—the node root, and must not overlap runtime, config, deployment provenance, or launchd plist paths. Binary or provenance mutation is forbidden until both backup phases pass.
