...
- [x] required-gate-ondemand-launcher-web-build (PRD-TESTING-CI-TIERED-002/003) [test_tier_required]: 为 `required-gate` 补 launcher Web build 的按需 planner 输出、workflow `trunk` 安装与 `ci-tests.sh required` 接线，仅在 launcher/shared runtime 路径命中时执行 `oasis7_client_launcher` Web dist 构建。 Trace: .pm/tasks/task_3778b0e747b249bc85b92b942a32b3fd.yaml
...
- 补充备注（2026-04-22）：`required-gate` 现已在 launcher/shared runtime 路径命中时额外输出 `run_launcher_web_build` / `needs_trunk`，workflow 仅在该分支安装 `trunk` 并向 `scripts/ci-tests.sh required` 透传 `OASIS7_CI_RUN_LAUNCHER_WEB_BUILD=true`；本地显式 `required` 默认语义不变。
