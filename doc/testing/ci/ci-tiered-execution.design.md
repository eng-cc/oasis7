...
| --- | --- | --- | --- | --- | --- |
| [Cargo package scope and impact-scoped integration verification](../../engineering/workflow/source-of-truth.md#cargo-package-scope-and-impact-scoped-verification) | [ci-tiered-execution.design.md#package-aware-exact-integration-target](ci-tiered-execution.design.md#package-aware-exact-integration-target) | 文档必须把批准目标的 package identity、`H/B/T` 与 `B -> T` impact selection 映射到 CI 分级，并把当前 path-based planner 标为未完成 enforcement。 | [ci-required-scope-audit-contract.test.sh](../../../scripts/ci-required-scope-audit-contract.test.sh) 的 required-tier planner/current-policy contract；本次还运行 `./scripts/doc-governance-check.sh` 与 `git diff --check`。 | 当前 task evidence；后续实现需另行提供 package metadata、exact integration 与 trusted CI receipt 证据。 | 本次不证明 planner 已执行 package-aware selection、`H/B/T` binding、真实 integration revalidation 或 full escalation enforcement。 |
