...
- Edge Cases & Error Handling:
  ...
  - release Web node-only config 污染：若 `viewer-primary-web-entry-regression.sh` 在 root 原本没有 canonical `config.toml` 的 runner/worktree 内启动 launcher/chain runtime，则自动生成的 `[node]` keypair `config.toml` 不得遗留并污染同一 workflow 后续 `run-game-test`；`release-gate-web-strict.sh` 的 software-safe phase 也必须显式跳过 remote provider contract preflight，只验证 stack bootstrap 后的 UI/blocker contract，避免在无 remote provider selector 的 release runner 上误判成 Web 回归。
