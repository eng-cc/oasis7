...
| 脚本迁移验证 | `run-launcher-stack.sh`、历史 `viewer-release-qa-loop.sh` | 运行标准入口并确认行为一致 | `legacy-migrated -> verified` | 先 smoke 再 QA loop | 脚本维护者可复核 |
...
- Tool Requirements: `agent-browser` CLI 用于真实浏览器操作；仓库脚本 `run-launcher-stack.sh`、`s10-five-node-game-soak.sh`、`p2p-longrun-soak.sh`。历史审查曾使用 `viewer-release-qa-loop.sh`，该脚本现已删除。
