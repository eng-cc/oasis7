...
- 涉及视觉方向、交互手感、玩家屏幕流程或体验取舍时，模型视觉评审必须回流 `game_visual_interaction_designer`；模型可替代 routine 截图检查，但不替代该角色的方向裁决。
