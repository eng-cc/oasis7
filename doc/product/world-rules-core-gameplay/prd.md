...
- `industrial_externality` 是独立于材料 batch 的 quality/custody、byproduct identity/disposition、maintenance、production receipt 与 delivery/terminal settlement 的 operation facet；它不能静默改写材料数量/质量、终端价值、需求或稳定窗口。已有 WIP、in-transit、buffer 必须沿原 root/revision lineage 取得一次 profile 支持的处置，不能因外部性预览或重连自动迁移。
...
站点候选被阻塞后，玩家必须能读到最早根因、已占用/已消费价值、受影响的输入或输出边、稳定窗口影响和下一次权威复查点，并只看到当前 profile 支持的等待容量、补料/补电、改路、改用其他站点、降载或延期动作。不存在合法站点或安全恢复路径时，必须明确 `no_safe_site_fallback`（或等价产品语义）并停止；不能把推荐、自动迁厂、后台重试或已接受目标伪装成工厂已建成、物流已连通或生产已开始。已存在的 accepted-unstarted、WIP、in-transit、buffer 工作不因新的站点候选自动迁移；若未来专业 profile 支持 relocation，必须沿既有 cutover/处置合同逐项产生 parent-linked 结果，否则不得在产品层展示迁厂动作。
...
该闭环的 `test_tier_required` 至少覆盖：同一目标下两个合法站点（例如原料近但输出路径拥挤、终端近但输入损耗较高）可从预览比较并显示成本、容量、损耗、power、终端适配、机会成本与下一步；站点、输入/输出路径或容量在报价后漂移时原子拒绝或重新报价且无设施/资源/物流副作用；站点因果变化建立新 candidate 并从 `W=0` 开始，旧工作不自动迁移；无合法站点时显示阻塞根因和真实恢复路径；重复提交、重连、Agent retry、snapshot restore 与 replay 不复制建厂、路径 reservation、输入 sink、稳定进度或奖励，Viewer 与 pure API 对候选、root blocker、`next_action` 和 `next_recheck` 保持一致。`test_tier_full` 覆盖三站点以上的共享边/终端容量争用、拓扑变化期间的 WIP/in-transit/buffer 处置、持久化恢复与支持的 relocation/cutover profile。
