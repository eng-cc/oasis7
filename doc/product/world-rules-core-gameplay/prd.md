...
`BuildFactory` 提交必须绑定所选 site 与当时有效的输入/输出拓扑上下文，并从新鲜权威状态重新校验 owner、location/chunk、factory kind、输入路径、输出路径、容量、power、terminal 和治理/权限前置。报价后任一会改变产出因果的站点或拓扑事实发生漂移时，只能按当前条件重新报价或原子拒绝，不得静默切换站点、自动改道、降级路线、隐藏损耗或生成部分设施。站点/拓扑变化若改变输入来源、物流边、能源、输出去向或 terminal purpose，则属于 parent-linked 新 factory/capability candidate，稳定窗口从 `W=0` 开始；只改变展示元数据且不影响产出因果时才可保留原 identity，无法证明时按因果变化处理并 fail closed。
