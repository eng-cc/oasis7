...
**What。**长期不活跃只允许沿着 `可读通知 -> 保护 -> 受限持续 -> 可恢复主张/申诉 -> 已解决` 的单向、可审计结果链处理。estate 与可撤销 delegation 是受限持续的两种可选制度容器，不是独立的“无人认领”状态；它们只在保护结束且仍有明确的连续性、风险隔离或价值保全需要时才可进入。任何阶段的动作都必须绑定该阶段的触发事实、范围、未决义务和 receipt，并在结果未确认时明确保持待决。
...
| 受限持续（estate / 可撤销 delegation） | 保护后仍存在明确的维护、风险隔离或价值保全需要，且其范围、授权来源、可撤销性和未决义务已可读记录。 | 在授权范围内提出有限维护、风险隔离或有限重启；相关主体可查看并质疑范围，提交 reclaim 或申诉。 | 不能把受限授权扩张为永久所有权、绕过合同/托管/责任处理，或处置范围外的资产、Agent 与业务。 | 授权撤销、范围不成立、reclaim/申诉成功或处置前提失败时，回到保护或待决处置；已经确认的最小安全动作及其 receipt 不被抹除。 |
| 可恢复主张 / 申诉待决 | 有资格主体以当前有效证据提出 reclaim 或程序性申诉；世界保留原阶段的约束，等待权威结果。 | 查看请求状态、补充允许的证据、撤回自己的待决请求，或在仍开放时提出相邻的程序性异议。 | 不能因提交而自动取回控制/资产、暂停全部既有义务，或以重连、短暂转让、重复请求取得优先级。 | 拒绝、过期或撤回只终止该请求；成功结果仅在已确认范围内恢复或更正，未结清义务继续可见且不得被绕过。 |
