...
- 若权威状态没有可用路径、只提供旧式摘要，或无法构成完整的可比较选项集，玩家必须看到“尚未发布/不可用”的真实状态或既有摘要，并继续看到当前 blocker 与下一次可决策点；客户端、Agent 建议或历史快照不得臆造缺失选项。旧式摘要可以在明确标注为兼容信息的区域保留权威来源原文（包括旧路径标签），但它不属于结构化比较项、不能满足 AS-8，也不能替代可读降级标签。
