...
- 静态 docs hub 与 Viewer 手册的 CN/EN 镜像是公共发现面的派生呈现，不是独立事实源。涉及手册、模式、限制、恢复或当前状态的变更，必须先由根 `README.md` 与对应专业 canonical source 决定，再在相关镜像中成对同步；镜像与 canonical 不一致时应同时纠正、收窄或标记失效，不得以历史静态文案背书。
