...
- 区域 charter、地点 tenure 与公共融资已由世界规则与玩法系统的区域专题接收，不构成本模块 authority。工业/市场/保供产品语义已由世界规则与玩法系统接收；对应专题和稳定条款由该模块根入口列出，本模块不重复声明 authority。 [`工业/市场（迁移 provenance）`](governed-industry-market-and-emergency-supply.prd.md) 的专业 PRD-GAME-018 等 active backlinks 已闭合至 GR-009..011、SC31-009/010/011 与 ES-001..004；该页仅保留为 `superseded` 历史 provenance，不构成本模块产品 authority、路线图、active topic 或验收。删除仍需独立 S1 退休 gate、历史 provenance/readback、inventory 更新及 owner/QA 批准，本索引不触发删除。[`普通治理（迁移 provenance）`](global-governance-organization-continuity-and-constitutional-guardrails.prd.md) 的 GG-2/GG-3 已由世界规则与玩法系统的 GCB-004/005 接收；该 `superseded` 源仍按独立 S1 gate 保留并通过治理/链接检查，各页迁移头记录接收 owner、已吸收切片与删除条件。连续性 `retired` 源的 CR-3/CR-4/CR-5/CR-6 由世界规则 GCB/ES/CC 与根 SC-34 接收，但旧页继续保留至 S1 清理专业/历史 backlinks。原 frontier 与区域能力/扩展迁移债务已由世界规则与玩法系统吸收，不再保留在本模块索引。
