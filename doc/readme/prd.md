...
- PRD-README-049: As a 站点访客 / 外部评审者, I want the public site to reflect the same framing as the whitepaper overview and expose a direct entry to it, so that I can understand the project's vision, two-stage path, and preview boundary from the landing surfaces before I fall back to repo-native docs.
- PRD-README-050: As a `liveops_community`, I want a reusable Xiaohongshu distillation-emoji post pack with fixed copy, panel-by-panel lines, and GPT Image 2 generation plan, so that we can turn the current “被蒸馏 / 蒸馏同事” discussion into a safe, humorous, shareable post without drifting into employer callout, layoff insinuation, or generic AI panic.
- Critical User Flows:
...
- AC-39: 若小红书内容链路继续追当前“被蒸馏 / 蒸馏同事”话题，必须补齐一份表情包型素材包，明确推荐标题、发布配文、评论区引导、逐张表情包文案、统一视觉方向、GPT Image 2 基准图与扩展图提示词策略，并把讨论收口到“打工人对知识沉淀/可替代性的共鸣式自嘲”，而不是滑向具体公司影射、裁员暗示、攻击同事、泛 AI 恐慌或技术科普长帖。
...
| PRD-README-050 | xiaohongshu-distillation-emoji-post-pack | `test_tier_required` | 小红书第十五篇素材包明确“被蒸馏”表情包主题的推荐标题、发布配文、逐张表情包文案、GPT Image 2 出图方案、评论区引导与禁滑坡边界，并固定为可直接继续出图的文档包 | 小红书内容链路从项目 token 成本继续推进到 AI 时代职场共鸣梗，让账号在热点讨论里保持“项目判断 + 打工人自嘲 + 可转发表情包”的原生表达，而不是退回技术解释帖 |
- Decision Log:
...
| DEC-RM-049 | 站点首页和文档中心显式暴露白皮书总览入口，并把首页路线说明收口到“第一阶段开放世界沙盒游戏，后续演进为开放世界游戏平台” | 继续把白皮书只藏在仓库 README / governance 索引里；或在站点首页继续只讲局部玩法而不解释阶段路径 | 白皮书已经补上“第一次接触项目时的单页全貌”，如果站点仍不给入口，公开读者还是会停在首页碎片文案里。把总览入口和两阶段路线放到站点，可让对外 landing surface 与现有 explanation 文档形成前后衔接，同时不越过技术预览边界。 |
| DEC-RM-050 | 第十五篇采用“被蒸馏”表情包路线，用统一角色和系列化大字梗图承接热点，而不是写成长篇技术解释或职场控诉 | 把话题写成模型蒸馏科普；直接影射具体公司/同事/裁员场景；或把每张图都做成不同画风的散图 | 这波话题在中文平台真正有传播力的是“打工人看一眼就懂”的共鸣式自嘲。固定系列角色、统一视觉底板和一句话梗图，更符合小红书保存/转发习惯，也能把风险控制在“好笑但不冒犯”的范围内。 |
