# oasis7 Xiaohongshu Blogger and WeChat Official Account Oasis Coin Incentive Pack（2026-04-12）（项目管理）

- 对应设计文档: `doc/readme/governance/readme-xiaohongshu-wechat-promoter-oasis-coin-incentive-pack-2026-04-12.design.md`
- 对应需求文档: `doc/readme/governance/readme-xiaohongshu-wechat-promoter-oasis-coin-incentive-pack-2026-04-12.prd.md`

审计轮次: 2

## 任务拆解（含 PRD-ID 映射）
- [x] XWPI-1 (PRD-README-XWPI-001/002): 完成专题 PRD / design / project 建档，明确小红书博主与微信公众号两类对象的评分结构、固定 `300 / 800 / 1500 OC` 档位、审批链和反作弊边界。
- [x] XWPI-2 (PRD-README-XWPI-001/002/003): 输出执行版绿洲币激励操作包，覆盖笔记/文章分层、计分、固定绿洲币额度、证据字段、safe phrase / forbidden phrase 与 FAQ。
- [x] XWPI-3 (PRD-README-XWPI-002/003/004): 对齐 `readme` 模块主追踪、文件级索引与 PM task execution log，确保后续 `liveops_community` 可直接复用。

## 依赖
- `doc/readme/prd.md`
- `doc/readme/project.md`
- `doc/readme/prd.index.md`
- `doc/readme/governance/readme-limited-preview-contributor-reward-pack-2026-03-22.prd.md`
- `doc/readme/governance/readme-limited-preview-contributor-reward-ledger-2026-03-22.prd.md`
- `doc/readme/governance/readme-xiaohongshu-liveops-runbook-2026-03-23.md`
- `doc/p2p/token/mainchain-token-initial-allocation-and-early-contribution-reward-2026-03-22.prd.md`

## 状态
- 更新日期: 2026-04-12
- 当前阶段: 已完成
- 阻塞项: 无
- 下一步: 用真实小红书笔记和微信公众号文章跑第一轮审核，验证固定 `300 / 800 / 1500 OC` 档位是否过粗，再决定是否需要补更细的 ledger 字段或单独的 producer override 专题。
