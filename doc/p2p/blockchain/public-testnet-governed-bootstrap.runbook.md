...
Readback is independent and precedes launch. Keep the existing pair unchanged until controlled cutover; the fixed order is storage-205 then sequencer-204: stop/read back one member, stage/start/read back it, then handle the second, never both at once. The executor records `max_simultaneously_stopped_validators=1`, live-peer readback before each reset, and rebuilt-member readiness before the next reset.
