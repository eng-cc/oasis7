...
- 当前缺口:
  ...
  - `shared_access / rollback_target_ready` draft 已生成；`mixed_topology_baseline` 现在也已有正式 `partial` 证据文档，但仍等待更强 same-window mixed-topology 真值或 dedicated lab 裁决来升到 `pass`
  - 没有正式 `staging/canary`
