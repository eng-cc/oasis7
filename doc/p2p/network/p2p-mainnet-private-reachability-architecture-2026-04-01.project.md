...
- 本轮已交付:
  ...
  - 2026-04-08 same-window triad reconfirm：`doc/testing/evidence/p2p-real-env-triad-reconfirm-2026-04-08.md` 已重新采到本机 observer、ECS sequencer、ECS storage 三端 live sample。observer 当前高位样本为 `committed_height=6597 -> 6602 / network_committed_height=61840 / known_peer_heads=1 / last_error=null`；storage 继续推进到 `committed_height=61842`；sequencer 仍卡在 `committed_height=0 / network_committed_height=0`，并稳定暴露 `execution driver received stale height: context=57536 state=57560`。同轮仓库内已为 `oasis7_chain_runtime` execution bridge 补上“从 exact execution record 回收 stale state”的恢复路径，并新增定向回归；但在把新二进制实际部署到 ECS sequencer 前，这组 real-env evidence 仍只能记为 blocked residual，而不是 pass uplift。
- 完成定义:
