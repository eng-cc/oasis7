...
- [ ] P2PARCH-6 (PRD-P2P-024-D/E) [test_tier_required + test_tier_full]: `qa_engineer` 建立 mixed-topology 套件，覆盖家宽/NAT、CGNAT、relay exhaustion、sentry loss、bootstrap poisoning、path failover。
  ...
  2026-04-07--08 real-env triad incident/rollout chain is consolidated in `doc/testing/evidence/p2p-real-env-triad-incident-provenance-2026-07-31.md`. Its historical sequence was observer peer-head-zero -> observer gap-sync blob availability -> sequencer stale execution -> storage challenge/blob deadlock -> fetch-commit source-readiness and convergence residual. It retains the release/hash/run locators and regressions, but is historical provenance only: the terminal triad disposition stayed `blocked` and is not current readiness authority.
- [x] iroh-inspired-reachability-doc-truth-alignment (PRD-P2P-024/PRD-P2P-025) [test_tier_required]: 将 iroh-inspired 后续三项落成设计真值，不引入 iroh 依赖、不替换 libp2p，并明确三者的单一真值链。 Trace: .pm/tasks/task_43a21163092541809de36036403d7c97.yaml
  - 文档落点:
    - `peer reachability contract normalization`: 在本专题 PRD/design 中定义 `PeerReachabilityContract` 内部归一化视图，规定 signed peer record、`TransportPath`、network-tier policy、status projection 与 matrix summary 之间的边界。
    - `path behavior matrix taxonomy`: 冻结 `exact / proxy / manual_lab / real_env / unsupported` evidence class、path expectation、NAT/degradation labels 与 claim boundary；matrix 只拥有 evidence/claim 语言，不拥有 runtime reachability truth。
    - `reachability/status observability summary`: 冻结 bounded operator summary 字段建议，并规定只能投影到现有 `/v1/chain/status.observability` / triad summary，不新增第二套 net health endpoint。
  - 后续实现 workstreams:
    - `P2PARCH-6-matrix-taxonomy-implementation`（owner: `qa_engineer`）: 更新 `scripts/p2p-mixed-topology-matrix.sh` / smoke，使 summary 输出 taxonomy fields，并保持 `required` only exact。
    - `P2PARCH-status-observability-operator-summary`（owner: `runtime_engineer`）: 在 status/triad report 中实现 selected path kind/age、transition counters、path mix、fallback reason、reachability confidence 的 bounded projection。
    - `P2PARCH-peer-reachability-contract-normalization`（owner: `runtime_engineer`）: 在 `oasis7_net` 内部把 signed peer record + transport paths + role/publish policy 归一化到单一 contract，再让 selection/status/debug 消费该 projection。
  - 验收边界:
    - 不复制 iroh address/router/net-report 类型；只吸收 identity-first、selected-path、matrix evidence 与 bounded observability 的设计模式。
    - 不把 `proxy` mixed-topology drill 写成 physical NAT/CGNAT proof。
    - 不让 status observability 输出 unbounded per-peer metric labels。
- [ ] P2PARCH-7 (PRD-P2P-024-E) [test_tier_required + test_tier_full]: `producer_system_designer` + `liveops_community` + `qa_engineer` 把 shared-network / release-train / claim gate 升级为 mixed-topology 正式门禁。
  ...
  当前已把 `mixed_topology_baseline` 从“缺草稿”推进成正式 legacy evidence：`doc/testing/evidence/legacy-shared-devnet-provenance-2026-07-26.md` 索引的 retained mixed-topology record 钉住 `P2PARCH-6` baseline、same-window shared-devnet follow-up/short-window 证据与 proxy 边界；`shared-devnet-blocker-packet.sh` 继续负责生成 `shared_access / mixed_topology_baseline / rollback_target_ready` 三份 blocker 文档，方便后续把 same-window mixed-topology 证据继续回填到 shared-devnet gate，而不冒充已 `pass`。若要正式升到 `pass`，除 same-window evidence 外还必须固定 producer/QA 审计通过的 pass-uplift decision ref。
...
- 本轮已交付:
  ...
  - historical real-env provenance：`doc/testing/evidence/p2p-real-env-triad-incident-provenance-2026-07-31.md` preserves the credentialed same-window triad reconfirmation and later rollouts; its terminal April disposition is `blocked`, not a current readiness claim.
  ...
  - 2026-04-08 observer/blob/challenge-gate/route follow-ups：the consolidated provenance record retains the observer `known_peer_heads=1` transition, provider-route fallback, replication-persisted cursor root cause, release hashes, and the narrowing to `fetch-commit` source readiness. These are historical incident findings; they do not replace a fresh current triad assessment.
- 完成定义:
