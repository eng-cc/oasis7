...
- Proposed Solution: 在仓库内冻结一套 triad 级完整监控体系，复用现有 status/traffic/wasm 能力，新增 host/process 采样与统一汇总层，输出单次可审计的 `snapshot + host/process + traffic + wasm + merged summary` 证据包。
- Success Criteria:
  ...
  - SC-3: triad 级 merged summary 能同时回答“链是否健康”“哪台机资源吃紧”“traffic 主要在哪条 lane”“WASM 是否退化”。
...
- Critical User Flows:
  ...
  2. Flow-P2P-TOS-002: `summary reports pass_candidate / pass_with_resource_alerts / blocked -> operator reads node-level CPU/load/memory/storage + chain status deltas -> evidence doc references canonical output paths`
  3. Flow-P2P-TOS-003: `fixture test feeds synthetic snapshot/host/traffic/wasm inputs -> merged summary contract stays stable without real ECS dependency`
...
| merged summary | `overall.status`、per-node `alerts`、resource + chain + wasm + traffic 摘要 | 读取四类输入并输出统一 JSON/MD | `inputs_ready -> merged` | snapshot claim 先定底线，resource alerts 再叠加 | producer/QA/runtime 共用 |
| fixture 回归 | host fixture / observability fixture | shell test 调 summary helper | `fixture -> pass/fail` | assert JSON contract 关键字段 | CI / 本地回归可读 |
- Acceptance Criteria:
  ...
  - AC-4: 监控体系必须保留 repo-owned artifacts，不依赖 Prometheus/OTel/外部 TSDB。
  - AC-5: `testing-manual.md` 必须给出 canonical 命令与产物路径。
  - AC-6: fixture 回归至少覆盖 host summary 与 merged summary 两层。
- Non-Goals:
...
- Architecture Overview: 以 repo-owned shell/python 工具为主，保持“一个控制机执行，多台节点取样，本地汇总出机器可读 summary”的结构；链状态/traffic/wasm 继续复用现有脚本，新增 host/process 采样和 triad merged summary helper。
- Integration Points:
...
| PRD-P2P-025-B | triad-observability-stack-T2 | `test_tier_required` | merged fixture -> observability summary helper 输出 overall/node summaries | triad merged summary contract |
