...
## 状态
- 更新日期: 2026-05-22
- 最新完成: `formal-public-testnet-local-observer-contract-sync`（已补 repo-owned `scripts/p2p-public-testnet-local-observer-sync.sh`，可直接从 fresh ECS `node.env` 推导并安装本机 `oasis7-testnet-observer.service` 应使用的 two-validator validator/signer/bootstrap/manifest 合同，同时在正式证据里明确当前会话对 `/opt/oasis7/p2p-testnet-local/**` 只有读权限，没有写权限，因此这一步只能先交付 sync 路径，不能伪报 live apply 已完成。）
