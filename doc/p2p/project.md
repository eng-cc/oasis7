...
- [x] formal-public-testnet-ecs-evidence-harvest (PRD-P2P-028) [test_tier_required]: 将 current live candidate manifest / bundle / bootstrap peers 镜像成 repo-owned readiness 输入，并用 same-window ECS + local triad freshness audit 复核当前 lane 真值；若 public endpoint 仍可达但 runtime/config 已漂移，则必须把 `public_rpc_ready/explorer_public_ready/faucet_guard_ready/runtime_bootstrap` 收紧回 `partial/block`。 Trace: .pm/tasks/task_49d6af52e31d404eb80999993eb71b98.yaml
  ...
  - 验收命令 (`test_tier_required`):
    ...
    - `./scripts/doc-governance-check.sh`
    - `git diff --check`
- [x] formal-public-testnet-local-observer-contract-sync (PRD-P2P-028) [test_tier_required]: 为 root-owned 本机 `oasis7-testnet-observer.service` 补 repo-owned local contract sync 脚本，从两台 ECS 的 two-validator env 推导本机 observer 应使用的 validator/signer/bootstrap/manifest 合同，并显式记录当前只能先交付 sync 路径、不能伪报 live apply 已完成。 Trace: .pm/tasks/task_dfb4d70a28884617b1506fa6570b34fc.yaml
...
## 状态
- 更新日期: 2026-05-22
- 最新完成: `formal-public-testnet-local-observer-contract-sync`（已补 repo-owned `scripts/p2p-public-testnet-local-observer-sync.sh`，并在拿到本机 sudo 权限后实际把 `oasis7-testnet-observer.service` 收口到 two-validator validator/signer/bootstrap/manifest 合同。当前 local status 已加载 `network_tier.tier=public_testnet`，说明旧的 `network_tier=null`/三 validator drift 已清掉；剩余 local blocker 已切换成 `fetch-commit authorization failed`，不再是 contract 未生效。）
- 最新完成: `formal-public-testnet-ecs-evidence-harvest`（已把 current live `public_testnet` candidate 的 manifest / bundle / bootstrap peers 镜像成 repo-owned readiness 输入，并执行 2026-05-22 same-window ECS + local freshness audit。新证据表明公网 RPC / explorer / faucet 入口仍可达，但本机 `oasis7-testnet-observer.service` 仍跑在未加载 `NETWORK_TIER_MANIFEST_PATH` 的旧启动链路上，且保留三 validator / 三 signer 合同；两台 ECS 则已收口到两 validator manifest。与此同时，ECS sequencer 当前还报 `execution driver missing predecessor record for non-contiguous committed height`。因此新 lanes TSV 已将 `public_rpc_ready/explorer_public_ready/faucet_guard_ready` 收紧到 `partial`、`runtime_bootstrap` 收紧到 `block`，aggregate readiness 继续保持 `block`，且下一步不该是“加更多 ECS”，而是先修 live runtime/config drift。）
