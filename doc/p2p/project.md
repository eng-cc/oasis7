...
- 最新补充（2026-05-31 / mainstream sync recovery parity）: 平滑升级不再只依赖 systemd active。新增 `scripts/p2p-upgrade-preflight.sh`，升级前检查 status 端点的 replication cursor、peer-head freshness、gap-sync blocked 和 policy lag；runtime status 也会暴露首缺口 repair attempt summary，用于定位 fetch-commit 是否走过 generic / connected-peer repair 路由；未完成 verified sync recovery 的节点会 hold 本地 consensus participation，避免落后节点继续 proposal/attestation。该能力对齐主流链的 preflight + repair visibility + unsafe-validator quarantine 边界，并新增显式 trusted checkpoint/state-sync fallback 门槛：status 暴露所需 checkpoint 高度与 snapshot 可用性，preflight 仅在 operator 显式提供覆盖缺口的受信 checkpoint 高度/hash 时允许带 warning 放行；可通过 `--trusted-checkpoint-manifest` 输入治理批准的 checkpoint manifest，并校验手工 height/hash 与 manifest 一致、唯一 signer 数满足 `min_signatures`、拒绝重复 signer，且支持 `validator_stakes` + `threshold_bps` 的 stake-weighted 阈值检查；`--state-sync-bundle-manifest` 与 `--state-sync-bundle-dir` 会把恢复 bundle 绑定到同一 checkpoint height/hash，并要求本地 snapshot/journal 文件存在且可按 sha256 校验；新增 `--require-state-sync-bundle` 后，checkpoint fallback 必须同时具备 bundle manifest、bundle dir、snapshot path/sha256 与 state_root，否则以 `state_sync_bundle_required` 阻断；checkpoint manifest 的每个签名条目也必须绑定 canonical checkpoint payload sha256；新增 `--verify-trusted-checkpoint-signatures` 后会用 openssl Ed25519 对 `signature_hex` 与 `public_key_path` 执行真实签名验签；新增 `--validator-set-manifest` 后会独立重算 validator-set hash 与 stake-root，并绑定 signer membership/public key；新增 `--verify-state-sync-bundle-semantics` 后会重算 JSON snapshot state root 并校验 journal checkpoint height/hash/state-root；state-sync bundle manifest 支持 `chunks`/`snapshot_chunks`，会逐个校验 chunk 文件 sha256 并重算 chunks root；新增 `--generate-restore-command-plan` 后会在 recovery plan 中输出 stop/backup/restore snapshot/restore journal/restart/post-check 的 dry-run 命令数组，且要求 `--restore-allow-dir` 限定 data/backup 路径，同时生成 rollback command plan；新增 `--restore-script-dir` 后会输出带环境确认锁的 restore/rollback shell 脚本并把 sha256 回写 recovery plan；新增 `--execute-restore-scripts` 后，只有设置 `OASIS7_ALLOW_RESTORE_EXECUTION=I_UNDERSTAND_THIS_CAN_REPLACE_NODE_STATE` 才会执行 restore 脚本，并在备份前生成 source sha256 manifest、备份后生成 data-before-state-sync.sha256 且自动 cmp 比对，且把执行日志、状态文件、状态和退出码回写 recovery plan；默认仍不会自动执行危险操作；新增 `--auto-rollback-on-restore-failure` 后，restore 失败会执行 rollback 脚本并回写 rollback 日志、状态文件和退出码，防止错包、缺件、内容篡改或跨 checkpoint 拼接签名元数据恢复；`--recovery-plan-dir` 可输出每节点 dry-run 恢复计划，记录 required height、checkpoint hash、manifest/source/signature count、唯一 signer 数、validator set/stake root、approved/required/total stake、bundle hash/snapshot/state root/path/sha256、阻断原因与人工恢复步骤；仍不自动替换本地账本。Trace: `.pm/tasks/task_9051849e0c92424bb7f0ca972a7935cc.yaml`。
