...
产品层归属：`p2p` 是“权威世界基础设施”的分布式状态专业域权威，不形成并列产品入口。产品总导航见 `doc/product/README.md`，组合承诺见 `doc/product/world-infrastructure/prd.md`。
...
## 高密度提示
- 本页不维护容易漂移的文件数量或状态快照；当前模块库存、热点子目录与治理状态统一以 `./scripts/doc-inventory-report.sh` 为准。
