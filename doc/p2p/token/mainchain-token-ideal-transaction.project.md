...
- [x] `ideal-transaction-doc-freeze` (`PRD-P2P-ITX-001/002/003`): the former dated topic is absorbed into this stable authority; historical process remains traceable through Git and GitHub task evidence.
- [x] `ideal-transaction-v2-context`: chain id, transaction version/type and validity reached submit, signing, node verification, explorer and launcher.
- [x] `ideal-transaction-asset-metadata`: asset, memo and network metadata reached the same closure.
- [x] `ideal-transaction-live-chain-status`: chain/network identity is live-runtime only, with no launcher fallback.
- [x] `ideal-transaction-fee-quote-metadata`: payload hash, fee quote and client-request metadata are propagated without fee execution.
- [x] `ideal-transaction-phase-two-scope`: payer, sponsor, priority and actual fee economics stay out of Phase 1.
