...
- 最新完成: `chain-status-transaction-and-finality-metrics`（已在同一 `/v1/chain/status` 真值下补齐 `transactions` lifecycle/confirmation latency summary、pending proposal `opened_at_ms`/stake progress、queue 与 submit buffer payload bytes、以及 `consensus.recent_finality_latency`；节点运营者现在可以继续区分 transfer 长时间未确认、submit buffer/队列积压和近期 finality 变慢，而不必翻 transfer history 或原始 runtime 日志。）
