...
- [x] TASK-WORLD_RUNTIME-051 (PRD-WORLD_RUNTIME-001/010/011/012) [test_tier_required]: 收口 `world-runtime` 其余活跃 module/testing/governance 与 numeric-correctness 专题中仍把旧 `oasis7*` crate/path/command 写成当前实现载体的口径，统一到 `oasis7*`。
  - 产物文件:
    ...
    - `doc/world-runtime/project.md`
  - 验收命令 (`test_tier_required`):
    - `rg -n "oasis7(_consensus|_node|_wasm_abi)?|crates/oasis7|crates/oasis7_consensus|crates/oasis7_node|crates/oasis7_wasm_abi|cargo test -p oasis7" doc/world-runtime/module/module-storage.project.md doc/world-runtime/module/player-published-entities.prd.md doc/world-runtime/module/module-subscription-filters.project.md doc/world-runtime/governance/governance-events.md doc/world-runtime/testing/testing.md doc/world-runtime/runtime/bootstrap-power-modules.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase3.prd.md doc/world-runtime/runtime/runtime-numeric-correctness-phase3.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase4.prd.md doc/world-runtime/runtime/runtime-numeric-correctness-phase4.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase5.prd.md doc/world-runtime/runtime/runtime-numeric-correctness-phase5.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase9.prd.md doc/world-runtime/runtime/runtime-numeric-correctness-phase9.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase10.prd.md doc/world-runtime/runtime/runtime-numeric-correctness-phase10.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase11.prd.md doc/world-runtime/runtime/runtime-numeric-correctness-phase11.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase12.prd.md doc/world-runtime/runtime/runtime-numeric-correctness-phase12.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase13.prd.md doc/world-runtime/runtime/runtime-numeric-correctness-phase13.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase14.prd.md doc/world-runtime/runtime/runtime-numeric-correctness-phase14.project.md doc/world-runtime/runtime/runtime-numeric-correctness-phase15.prd.md`
    - `./scripts/doc-governance-check.sh`
    - `git diff --check`
...
- 模块进展补充（2026-05-31 / mainstream sync recovery parity）: 为主流链式平滑升级补齐运行时护栏：gap-sync 首个缺失 commit 的 fetch-commit repair 现在在 consensus/status 中暴露结构化 route summary，`scripts/p2p-upgrade-preflight.sh` 会在升级前阻断 replication gap、peer-head 不可用和高度 lag 超标的节点；同时未通过 verified sync boundary 的节点会 hold 本地 proposal/attestation 参与，并在 status 中暴露 `consensus_participation_held` 与 hold reason。state-sync fallback 已补为显式 operator-gated 骨架：status 暴露 `state_sync_fallback_required`、snapshot 可用性、受信 checkpoint 最低高度和原因，preflight 只接受覆盖该高度且带 hash 的 trusted checkpoint fallback；`--trusted-checkpoint-manifest` 可读取治理批准的 checkpoint manifest，校验唯一 signer 数满足 `min_signatures` 并拒绝重复 signer，同时支持 `validator_stakes` + `threshold_bps` 的 stake-weighted 阈值检查；`--state-sync-bundle-manifest` 与 `--state-sync-bundle-dir` 会把恢复 bundle 绑定到同一 checkpoint height/hash，并要求本地 snapshot/journal 文件存在且可按 sha256 校验；新增 `--require-state-sync-bundle` 后，checkpoint fallback 必须同时具备 bundle manifest、bundle dir、snapshot path/sha256 与 state_root，否则以 `state_sync_bundle_required` 阻断；checkpoint manifest 的每个签名条目也必须绑定 canonical checkpoint payload sha256；新增 `--verify-trusted-checkpoint-signatures` 后会用 openssl Ed25519 对 `signature_hex` 与 `public_key_path` 执行真实签名验签；新增 `--validator-set-manifest` 后会独立重算 validator-set hash 与 stake-root，并绑定 signer membership/public key；新增 `--verify-state-sync-bundle-semantics` 后会重算 JSON snapshot state root 并校验 journal checkpoint height/hash/state-root；state-sync bundle manifest 支持 `chunks`/`snapshot_chunks`，会逐个校验 chunk 文件 sha256 并重算 chunks root；新增 `--generate-restore-command-plan` 后会在 recovery plan 中输出 stop/backup/restore snapshot/restore journal/restart/post-check 的 dry-run 命令数组，且要求 `--restore-allow-dir` 限定 data/backup 路径，同时生成 rollback command plan；新增 `--restore-script-dir` 后会输出带环境确认锁的 restore/rollback shell 脚本并把 sha256 回写 recovery plan；新增 `--execute-restore-scripts` 后，只有设置 `OASIS7_ALLOW_RESTORE_EXECUTION=I_UNDERSTAND_THIS_CAN_REPLACE_NODE_STATE` 才会执行 restore 脚本，并在备份前生成 source sha256 manifest、备份后生成 data-before-state-sync.sha256 且自动 cmp 比对，且把执行日志、状态文件、状态和退出码回写 recovery plan；默认仍不会自动执行危险操作；新增 `--auto-rollback-on-restore-failure` 后，restore 失败会执行 rollback 脚本并回写 rollback 日志、状态文件和退出码，防止错包、缺件、内容篡改或跨 checkpoint 拼接签名元数据恢复；`--recovery-plan-dir` 会生成每节点 dry-run 恢复计划，写明 checkpoint、manifest/source/signature count、唯一 signer 数、validator set/stake_root、approved/required/total stake、bundle hash/snapshot/state_root/path/sha256、阻断原因、snapshot 可用性与人工恢复步骤。该切片仍不自动跳过缺失 commit，也不做危险重置或本地账本替换。Trace: `.pm/tasks/task_9051849e0c92424bb7f0ca972a7935cc.yaml`。
- 模块进展补充（2026-06-01 / mainstream sync recovery parity execution guardrails）: testnet 演练后继续补齐恢复执行层：restore script 执行前会 re-check 必需工具链、snapshot/journal/chunk sha256 与 chunks root，捕获 `systemctl show/status` 服务状态快照，备份 source/backup sha256 与 `path/type/size/mode/uid/gid` metadata manifest 并自动比对；生成 restore command plan 前会拒绝 shell-unsafe service name、data/backup/bundle 路径及 snapshot/journal/chunk 相对路径。fake execution drill 覆盖成功恢复、snapshot 篡改、chunk 篡改、restore 失败自动 rollback；真实 ECS/testnet 状态仍未被替换。
- 模块进展补充（2026-06-13 / local LetAI provider playtest recovery）: 本地真实 LetAI provider-backed 试玩链路已补齐 60s chat probe / runtime provider decision budget、LetAI chat completion 瞬态空 SSE/timeout/remote-close 重试、provider bridge fallback timeout 单位与参数兼容，并为 viewer blocked-control feedback 增加本地 LetAI bridge 日志与代理/上游检查提示；验证覆盖 targeted Rust/Python/script checks 与一次真实 `run-local-letai-game-test.sh` stack-ready 证据。Trace: .pm/tasks/task_4b3ddd0cf17d4f568fe4249e5a80da0b.yaml。
