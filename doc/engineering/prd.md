...
- Success Criteria:
  ...
  - SC-36B: 专业角色 subagent slice 默认模型配置必须固定为 `gpt-5.4` + `reasoning_effort=medium`（`gpt-5.4-medium`）；任何非默认模型或 reasoning effort 必须在 slice contract 中记录用户要求或任务理由。
...
- Critical User Flows:
  ...
  17. Flow-ENG-017: `TPM 准备派发专业角色 slice -> 默认填入 model=gpt-5.4 / reasoning_effort=medium -> 若要覆盖则在 slice contract 写明 override reason -> 再启动 subagent`
