...
- Success Criteria:
  ...
  - SC-17: 任务执行证据 canonical sink 统一为 GitHub task issue evidence comments，多角色执行时单条记录必须显式标注角色。
  - SC-18: GitHub task issue evidence / handoff 中的角色字段由 `.agents/roles/*.md` 白名单约束，新增别名违规数为 0。
  - SC-19: 当任务需要其他伙伴协作时，执行主体必须切换到标准角色视角并加载对应职责卡；implementation-freeze commit 后独立 review 只用于暴露风险/回归/缺测，不得替代 owner / role 协作语义。
  ...
  - SC-23: 默认评审边界必须是先冻结 implementation commit，再校验 source-bound Task UID / frozen-head identity 并创建 `./scripts/prepare-task-pr.sh --draft-candidate --create`；draft candidate 创建后才基于同一 immutable head 完成 exact-head CI 与本地相关角色 subagent review evidence packet，在有效 findings 整改或基于证据驳回后完成 transactional task-closeout，再以 fresh ci_ready_receipt 通过 `./scripts/prepare-task-pr.sh --promote-draft <fresh ci_ready_receipt.json>` 进入 GitHub PR，并以 required checks、requested changes、PR comment/thread closeout、mergeability 与 GitHub merge path 作为服务端正式 review/merge 流程；普通 PR 创建后必须继续盯 required checks/mergeability/comments，`REVIEW_REQUIRED` 只作为状态信息回报、不作为 block 项，`mergeStateStatus=BEHIND` 也只表示落后基线、若 GitHub merge path 仍接受则不必先 rebase；若 `mergeStateStatus=BLOCKED` 仅因缺少 review approval，且用户/task policy 明确授权跳过 approval，则复查 gates 后可作为正常流程使用 repo admin merge path；失败修复并重推，actionable comments 回复/整改/resolve 后，才能合入与清理。
  - SC-24: 本地不再要求额外的 `codex-review-snapshot` 或其他 uncommitted review 脚本；draft candidate 创建不得等待 role review，exact-head CI 后必须由 TPM 新建/派发涉及到的相关专业角色 subagent 完成本地 review，并将 findings/no_findings/residual_risk 与整改处置写入 GitHub task issue evidence comments；完成 findings 处置后执行 transactional task-closeout，形成 `Pre-PR Ready` / draft promotion gate，而不是把 role review 或 task-closeout 作为 draft candidate 创建前置条件。
  - SC-24A: 根 `AGENTS.md`、engineering 主 PRD、`self-evolution` 正式追踪、`.pm/README` 与 `workflow-report` close checklist 必须对该流程维持单一口径：默认执行“implementation-freeze commit -> `./scripts/prepare-task-pr.sh --draft-candidate --create`（创建 frozen-head draft candidate）-> exact-head CI / detached immutable verification / 必要测试门禁 -> pre-PR local role subagent review -> findings 处置（实现变更则重新 freeze/verify/review）-> selected-task audit + transactional evidence-only task-closeout -> optional evidence-only commit（若改变 HEAD，则重新 exact-head CI / review / closeout 并生成 fresh receipt）-> `./scripts/prepare-task-pr.sh --promote-draft <fresh ci_ready_receipt.json>` -> GitHub PR watch/fix/merge -> cleanup”；若 owner 选择手工拆步，也必须与该 helper 的底层顺序保持等价。只有明确用于 manual packaging/release CI 的 PR 才允许停在人工打包 gate。
  ...
  - SC-27: `.pm` 的 stage/gate、signal、task 与 memory `source_ref(s)` / `updated_from` 不得再把 `doc/devlog/*.md` 当运行态真值；历史 `doc/devlog/*.md` 仅保留归档职责，运行态证据统一来自 GitHub task issue evidence comments、正式文档或其他显式 evidence。
  ...
  - SC-36B: 专业角色 subagent slice 默认继承用户/父线程当前 runtime，仓库不得用顶层配置固定根 TPM；每个 slice 必须记录 requested model/reasoning 及理由、surface 可观测时的 actual 值，或无法选择/观测时的 inherited/unverified capability 原因，且未经观测不得把 requested 值写成 actual。
...
- User Stories:
  ...
  - PRD-ENGINEERING-006 (historical): legacy migration owner scopes were completed and retired; current task execution truth uses GitHub task issue evidence comments.
...
- Critical User Flows:
  1. Flow-ENG-001: `implementation-freeze commit -> ./scripts/prepare-task-pr.sh --draft-candidate --create -> exact-head CI / detached immutable verification / 必要测试门禁 -> pre-PR local role subagent review -> findings 处置（实现变更则重新冻结）-> transactional evidence-only task-closeout -> optional evidence-only commit（若改变 HEAD，则重新 exact-head CI / review / closeout 并生成 fresh receipt）-> ./scripts/prepare-task-pr.sh --promote-draft <fresh ci_ready_receipt.json> -> normal PR 进入 required checks + mergeability + comments watch（REVIEW_REQUIRED 仅回报不阻塞；review-approval-only BLOCKED 可按授权 admin merge）-> 失败修复重推 -> comments 处理/resolve -> 通过后合入与清理`
  ...
  14. Flow-ENG-014: 新需求 -> 新建独立 worktree（若 owner/title/source refs 已明确，则优先通过 `new-task-worktree.sh --pm-*` 在目标 worktree 内原子完成 GitHub-backed PM bootstrap；新 task worktree 必须把 ignored `target` 链接到 repo-family shared cargo target cache，减少多 worktree 重复编译产物；若 canonical `main` worktree 根存在本地 `config.toml`，则同步复制到新 task worktree，避免 active-LLM / harness 复现因缺配置偏离 `main`）-> 创建并提升 GitHub-backed task -> workflow-report start -> 执行与回写 -> implementation-freeze commit -> ./scripts/prepare-task-pr.sh --draft-candidate --create -> exact-head CI / detached immutable verification / 必要测试门禁 -> pre-PR local role subagent review -> findings 处置（实现变更则重新冻结）-> selected-task audit + transactional task-closeout -> optional evidence-only commit（若改变 HEAD，则重新 exact-head CI / review / closeout 并生成 fresh receipt）-> ./scripts/prepare-task-pr.sh --promote-draft <fresh ci_ready_receipt.json> -> 记录 PR purpose decision；normal PR 盯 CI/mergeability/comments，`REVIEW_REQUIRED` 仅回报不阻塞，`mergeStateStatus=BEHIND` 若仍可直接 merge 也不强制 rebase，review-approval-only `BLOCKED` 可在授权后 admin merge，失败修复重推，comments 先处理/resolve；manual packaging/release CI PR 停在人工 gate -> merge/cleanup -> 若 project 仍有后续 task，则重新新建下一个 worktree/task
  14A. Flow-ENG-014A lifecycle hardening: bootstrap 用 journal 幂等恢复并在远端部分成功后保留 worktree；ready/done 绑定同一 frozen head 的 provenance review 与 merged/non-PR proof；exact head/base PR 可恢复 record；full PR gate 覆盖 checks、mergeability、requested changes、general comments、top review bodies、threads；manual packaging 和 `user_requested_merge_hold` 都阻止 merge；cleanup 只允许 merged + done + main-contained + clean 的非 force 删除。
  ...
  16. Flow-ENG-016: `只读问题 -> 先进入 default-workflow-bootstrap 并绑定标准 task/worktree 真值 -> 在已绑定 task 内判断是否只是客观事实读取 -> 若是则 TPM 直接复述证据 -> 若需要专业判断则派发对应 bounded role slice -> TPM 合流并标注角色来源 -> 若后续要改仓库则沿同一 canonical task/worktree 继续执行`
  17. Flow-ENG-017: `TPM 准备派发专业角色 slice -> 默认填入 inherit current parent selection -> 记录 requested model/reasoning 及理由 -> surface 可观测则记录 actual，否则记录 inherited/unverified capability 原因 -> 不把未观测 requested 值当 actual -> 再启动 subagent`
...
| 角色协作工作流 | owner role、角色视角切换、职责卡加载、handoff 触发条件、执行顺序、QA/LiveOps 回流、pre-PR local role review、GitHub PR watch/fix/comment-closeout/merge 默认流程 | 当需要其他伙伴协作时，先切换到对应标准角色视角并加载职责卡，再按工作流执行；必须先冻结 implementation commit，校验 task/frozen-head identity 并通过 `./scripts/prepare-task-pr.sh --draft-candidate --create` 创建 frozen-head draft candidate，再在该 immutable head 上完成 exact-head CI 与本地相关角色 subagent review evidence packet，完成 findings 处置与 task-closeout 后以 fresh ci_ready_receipt 通过 `./scripts/prepare-task-pr.sh --promote-draft <fresh ci_ready_receipt.json>` 进入 GitHub PR 的 required checks、requested changes、comment/thread closeout 与 mergeability 路径，并对 normal PR 持续 watch/fix/comment-closeout/merge/cleanup；`REVIEW_REQUIRED` 仅回报不阻塞，review-approval-only `BLOCKED` 可在授权后走 repo admin merge path | `defined -> adopted -> audited` | 默认按需求进入顺序执行，跨角色任务先定 owner 再流转；draft candidate 创建先于 exact-head CI / role review，role review 是 Pre-PR Ready / promotion 的 required evidence gate，但不得替代 GitHub PR 服务端 required checks、requested changes、comment closeout 或 mergeability 边界；未明确 manual packaging/release CI purpose 时不得停在 PR 创建后 | 全体贡献者遵守，治理维护者可演进 |
...
- Acceptance Criteria:
  ...
  - AC-18B: 根 `AGENTS.md` 不得再把本地额外 review 脚本写成默认硬门禁；正式流程必须要求 PR 创建前本地相关角色 subagent review evidence，并保留 GitHub PR required checks、requested changes、comment closeout、mergeability 与 merge path 作为服务端保护边界；`REVIEW_REQUIRED` 不得被写成 block 项，review-approval-only `BLOCKED` 在明确授权时必须被描述为正常 admin merge 路径。
  - AC-19: 根 `AGENTS.md` 的 GitHub task issue evidence 规则需明确“一任务一 evidence stream、不按角色拆 task、条目级标角色”的约束。
  - AC-20: 文档治理门禁需阻断 GitHub task issue evidence / handoff 中未在 `.agents/roles/*.md` 注册的角色名。
  ...
  - AC-23: GitHub task issue、Project item、mapping、working_memory、stage blocker 与 codex session 映射全部以 `task_uid` 作为唯一主键；`TASK-PM-xxxx` 顺序号、`legacy_ids` 与 `display_id` 不得再作为正式字段或路径依赖。
  - AC-24: `.pm/registry/tasks.yaml` 与 role backlog 若保留，必须退化为由 canonical task 对象扫描重建的视图，且重建过程不要求 `next_sequence`、不要求中心抢号。
  - AC-25: `.pm` 的 stage/gate、signal、task 与 memory `source_ref(s)` / `updated_from` 只允许引用 GitHub task issue evidence comments、`doc/devlog/**` 之外的正式文档或其他显式 evidence；若仍指向 `doc/devlog/*.md`，lint/promote/set-stage 必须阻断。
  ...
  - AC-35: Codex role-config validator 对全部 11 个 role-card projection 做确定性 registry/adapter 比较并拒绝 body-swap/假 hash 绕过；`claim-ready` 在验证期间发生 HEAD/index/path/type/mode/content/symlink 漂移时必须 fail closed；普通 audit/claim 不得改写 `tasks.json`，refresh-capable audit 必须报告 title/acceptance drift；task-wide completion 使用覆盖 workflow/doc/skill/syntax/diff 的单一 composite command。
- Non-Goals:
...
- Edge Cases & Error Handling:
  ...
  - 归档源污染：若 `.pm` runtime `source_ref(s)` / `updated_from` 仍引用 `doc/devlog/**`，必须视为把历史归档误当运行态真值，lint/promote/set-stage 直接阻断并要求改为 GitHub task issue evidence comments、正式文档或显式 evidence。
...
| PRD-ENGINEERING-027 | historical `world-simulator-viewer-path-governance`（已退役） | historical-only | 当前检查 `doc/world-simulator/viewer/README.md` 首读分流、维护触发器、`doc/world-simulator/README.md` / `doc/world-simulator/prd.index.md` 上游可达性与 `doc-governance-check`；原三件套从 git history 与 GitHub task evidence 追溯 | `world-simulator/viewer` 热点子域入口与 `PRD-ENGINEERING-025` 第二条 follow-up 的长期承接 |
| PRD-ENGINEERING-028 | historical `p2p-node-path-governance`（已退役） | `test_tier_required` | 当前检查 `doc/p2p/node/README.md` 首读分流、维护触发器、`doc/p2p/README.md` / `doc/p2p/prd.index.md` 上游可达性与 `doc-governance-check`；原三件套从 git history 与 GitHub task evidence 追溯 | `p2p/node` 热点子域入口与 `PRD-ENGINEERING-025` 第三条 follow-up 的长期承接 |
...
| PRD-ENGINEERING-030 | historical `readme-governance-path-governance`（已退役） | historical-only | 当前检查 `doc/readme/governance/README.md` 首读分流、维护触发器、`doc/readme/README.md` / `doc/readme/prd.index.md` 上游可达性与 `doc-governance-check`；原三件套从 git history 与 GitHub task evidence 追溯 | `readme/governance` 热点子域入口与 `PRD-ENGINEERING-025` 第五条 follow-up 的长期承接 |
...
| DEC-ENG-019 | 执行证据收敛为 GitHub task issue evidence comments，并在条目级强制标注角色 | 继续维护集中式日表、按角色拆分日志文件或复活 repo-local task execution files | GitHub-backed task envelope 已经以 task 为基本执行单元，issue evidence stream 更利于追溯、lint 与多 worktree 并发隔离。 |
...
| DEC-ENG-022 | `doc/devlog/*.md` 仅保留历史归档职责，`.pm` runtime `source_ref(s)` / `updated_from` 统一切到 GitHub task issue evidence comments、正式文档或显式 evidence | 继续让 `doc/devlog/*.md` 同时承担归档与 `.pm` 运行态真值 | 集中式日表已退役为归档层；若继续把它写进 stage/signal/task/memory 的 runtime 引用，会让 `.pm` 当前态重新依赖历史流水，削弱 worktree 隔离、lint 可判定性与 task-local traceability。 |
