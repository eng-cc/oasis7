...
- Success Criteria:
  ...
  - SC-3: `dispatching-parallel-agents` 与 `subagent-driven-development` 必须被翻译成 repo-owned 默认角色编排层与 bounded subagent-driven execution：`tpm` 负责 orchestrate，其他专业角色默认作为 subagent 参与分析 / 实现 / 验证 / 补充 review，但不得打破单 owner role、单 GitHub-backed task truth、单 canonical worktree 与 GitHub PR review 主链。
  ...
  - SC-5: 外部 workflow 借鉴不得引入新的运行态真值系统；默认 subagent 编排只能叠加在现有 GitHub-backed task evidence、`project.md`、workflow source-of-truth 与 GitHub PR review 之上。
  ...
  - SC-9: 从 `brainstorming` salvage 的 ideation discipline 必须被翻译成 repo-owned bounded brainstorming surface：只在范围仍模糊、需要 2-3 方案对比、或问题本身偏视觉/结构时启用；结论必须回写 `prd.md` / `project.md` / handoff / GitHub task issue evidence comments，不能停留在聊天里，也不能强制逐段审批。
  ...
  - SC-13: `using-superpowers` 的“对话一开始先进入正确流程”体验若要继续借鉴，必须翻译成 repo-owned `default-workflow-bootstrap`：只负责仓库状态影响判断、标准 task worktree / GitHub-backed task truth / owner role 真值检查、formal doc 入口选择与 handoff 到 workflow router；它不得替代 `AGENTS.md`、`doc/engineering/workflow/source-of-truth.md`、`prd.md`、`project.md`、GitHub task issue evidence comments 或 GitHub PR review。
...
- User Stories:
  ...
  - PRD-ENGINEERING-AWB-005: As a workflow maintainer, I want repo-owned planning surfaces to require affected paths, atomic validation steps, and a lightweight self-check, so that `writing-plans` discipline strengthens execution without replacing `prd.md` / `project.md` / GitHub-backed task evidence.
...
- Critical User Flows:
  ...
  4. Flow-AWB-004: Viewer Web 新一轮结构/视觉专题开始前，若问题本身包含 wireframe/IA/布局对比，则可先启用 visual companion 产出浏览器侧 mockup、Image2 target image 或 native/browser screenshot compare；若已存在绑定 GitHub-backed task，visual companion 只能作为当前 task 内 evidence，不创建第二 task/worktree/PR；若还没有实现 task，则确认方向后再创建实现 task，并继续按现有 `agent-browser` / repo-owned UI regression 收口。
  5. Flow-AWB-005: 当一个 task 需要多角色协作时，`tpm` 默认派生所需专业角色 subagent -> 将分析、实现、验证、补充 review 切成 role-owned slices -> 为每个 subagent 明确目标、输入、输出、验证方式与 write scope -> 若 write scope 不互斥则串行，若互斥则可限域并行 -> 所有结果统一回收到同一 owner / GitHub-backed task truth / canonical worktree / PR。
  6. Flow-AWB-006: 外部 workflow 若要求替换现有 owner role、GitHub PR review 默认边界或 GitHub-backed task truth，则直接标记 rejected；若能翻译成“默认角色 subagent 编排”或“bounded subagent-driven execution”且不替代正式真值，则允许 adopted；若只是 distribution/packaging 问题，则列入 deferred。
  ...
  8. Flow-AWB-008: 当任务已经具备 `project.md` / handoff / GitHub-backed task truth 且准备开始实施时，owner 先做一次 execution gap review，确认影响面、步骤、验证和命名一致性，再按原子步骤逐步执行并在每一步后读取实际验证结果；若遇到 scope drift 或重复失败，则先报告 blocker 而不是继续猜测。
  ...
  10. Flow-AWB-010: 当任务方向仍模糊、范围过大，或本质上是产品 / 架构 / UI 取舍题时，owner 先做 bounded brainstorming -> 判断是否需要拆 scope、是否需要 2-3 方案对比、是否值得用 visual companion -> 选定推荐方向后再回写 `prd.md` / `project.md` / handoff / GitHub task issue evidence comments，并进入现有 implementation/verification 主链。
  ...
  13. Flow-AWB-013: 当一个会改变仓库状态的新 task 启动时，owner 先通过 repo-owned `default-workflow-bootstrap` 确认标准 worktree / GitHub-backed task truth / owner role 真值与正式文档入口；若 task truth 缺失则先创建，再交给 workflow router 选择 bounded brainstorming、TDD、execution、verification 或 closeout 阶段。
...
| Repo-owned default workflow bootstrap contract | `repository_state_impact`、`workspace_state`、`reuse_allowed`、`worktree_action`、`owner_role`、`task_truth`、`routed_next_phase` | 让所有用户请求先进入 repo-owned bootstrap，建立或复用 task/worktree/GitHub task issue truth 后再路由到下一个 workflow surface | `unclassified -> bootstrapped -> routed -> phase_active` | 只处理 workflow setup，不替代 routing/execution/closeout；read-only/chat-only 和纯客观事实读取也不得绕过 bootstrap，只有已绑定 task 内的事实复述才可走最小记录快路径 | `tpm` 或当前 owner 触发；正式真值仍由 `AGENTS.md` / workflow source-of-truth / GitHub task issue evidence comments / `prd.md` / `project.md` 维护 |
...
| Repo-owned workflow router contract | `current_phase`、`selected_skills`、`skipped_skills`、`routing_reason`、`writeback_surface` | 对已建立 task truth 的仓库变更 task 先判断当前应进入哪一个 repo-owned workflow phase，并按顺序路由到 bounded brainstorming、TDD、execution、verification 与 closeout skill | `unrouted -> routed -> phase_active -> next_phase_ready -> closed` | 先判定当前阶段与风险面，只路由必要 skill；不得把所有 skill 强制串成 ceremony | `tpm` 或当前 owner 触发；正式真值仍由 `AGENTS.md` / workflow source-of-truth / GitHub task issue evidence comments / `prd.md` / `project.md` 维护 |
...
| `dispatching-parallel-agents` | adopted | repo-owned 默认 `tpm` orchestrator + professional role subagents，要求单 owner / 单 GitHub-backed task truth / 单 canonical worktree / GitHub PR review 真值 | 可借其拆分原则，但必须翻译成受限的 repo-native 角色编排，而不是无边界 swarm。 |
...
| `subagent-driven-development` | adopted | repo-owned bounded subagent-driven execution：`tpm` 在单 owner / GitHub-backed task truth / worktree / PR 真值内派生专业角色 subagent 处理分析、实现、验证与补充 review 切片 | 只借执行切片、上下文最小化与实现/验证分工；fresh subagent-per-task + 本地双阶段 review ritual 继续保持 rejected。 |
...
- Acceptance Criteria:
  ...
  - AC-7B: 对行为变更且存在稳定自动化测试面的实现任务，root workflow 与 handoff template 必须显式要求 `behavior contract / target test surface / RED command or skip reason`；若跳过 RED，必须能从正式文档或 GitHub task issue evidence comments 回放原因。
...
- Non-Functional Requirements:
  ...
  - NFR-AWB-3A: 默认角色 subagent 编排与 bounded subagent-driven execution 不得改变 GitHub-backed task lifecycle、GitHub task issue evidence comments 记录责任人与 GitHub PR review 正式边界；本地 `.pm` 状态/日志只作为迁移或脚本桥接时不得反向覆盖 GitHub task truth。
  ...
  - NFR-AWB-3D: bounded brainstorming contract 不得把 universal brainstorming gate、逐段审批或单独 spec 流程写回 root 默认门禁；它只能约束“方向仍模糊、范围过大或问题本身偏视觉/结构”的任务，并且产出必须回写到现有 `prd.md` / `project.md` / handoff / GitHub task issue evidence comments。
  - NFR-AWB-3E: repo-owned workflow router 不得取代 root workflow 真值；它只能编排当前已经 adopted 的本地 workflow surface，并且阶段切换必须可回放到 `AGENTS.md`、`.agents/skills/README.md`、handoff、`project.md` 或 GitHub task issue evidence comments。
  ...
  - NFR-AWB-6: execution surface tightening 不得绕开 `project.md` / GitHub-backed task evidence / GitHub PR review，也不得把 step-level verification 替换成事后总结式宣称。
- Security & Privacy:
...
- Technical Risks:
  ...
  - 风险-4: 若默认角色 subagent 没有 write scope / owner / return contract 约束，会造成 overlapping writes、task truth 漂移或 review 责任不清。
...
| PRD-ENGINEERING-AWB-002 | `completion-claim-verification-followup`、`pm-step-evidence-lint-defaulting`、`workflow-enforcement-audit-followup` | `test_tier_required` | fresh verification claim checklist/helper/smoke、GitHub task issue evidence comments 中的 step evidence、GitHub-backed task evidence 持久化、低层 `move-task done` 绕过阻断与失败签名 | task closeout、PR preflight、QA 报告口径、GitHub-backed task lifecycle 真值 |
...
| PRD-ENGINEERING-AWB-008 | `bounded-brainstorming-workflow-rollout` | `test_tier_required` | `AGENTS.md` 的 bounded brainstorming rule、handoff/planning option-framing 字段、本地 `bounded-brainstorming` skill，以及 borrowing design / skill-surface 文档改判 | `engineering` 的方向探索、scope decomposition 与 optional visual ideation 边界 |
| PRD-ENGINEERING-AWB-009 | `repo-owned-workflow-router` | `test_tier_required` | root `AGENTS.md` 的 phase order、`.agents/skills/repo-owned-workflow-router`、`.agents/skills/README.md` 入口，以及 borrowing design / skill-surface 文档对齐 | `engineering` 的端到端流程路由、阶段切换与本地 process-skill 组合 |
...
| PRD-ENGINEERING-AWB-006 | `default-role-subagent-orchestration`、`role-subagent-local-validation`、`subagent-driven-default-reconciliation`、`subagent-driven-default-workflow-rollout`、`workflow-behavior-eval-and-closeout-hardening` | `test_tier_required` + `test_tier_full` | `AGENTS.md` 默认 orchestrator/subagent 规则与 bounded subagent-driven execution、handoff/planning contract、borrowing design 文档改判、repo-owned multi-agent behavior eval | `engineering` 多角色协作、owner/task/worktree/PR 真值边界 |
| PRD-ENGINEERING-AWB-007 | `bounded-tdd-workflow-rollout` | `test_tier_required` | `AGENTS.md` 的 behavior-first testing rule、handoff/planning/test-skill 回写、borrowing design 文档改判与技能边界对齐 | `engineering` 行为变更类实现任务、自动化回归与 skip-reason 审计边界 |
| PRD-ENGINEERING-031 | `workflow-execution-surface-tightening`、`pm-step-evidence-lint-defaulting` | `test_tier_required` | repo-owned execution skill、`AGENTS.md` execution rule、execution-log step evidence lint、workflow-borrowing design 回写与文档治理校验 | `engineering` task 执行、逐步验证与 blocker handling 链路 |
- Decision Log:
| 决策ID | 选定方案 | 备选方案（否决） | 依据 |
| --- | --- | --- | --- |
| DEC-AWB-001 | 只借鉴结构化 workflow patterns，并把 adopted 项转成 repo-owned follow-up | 直接把 `superpowers` 当默认 bootstrap 或当前流程真值 | 当前真值已是 `AGENTS.md + workflow source-of-truth + GitHub-backed task evidence + GitHub PR review`，不能再并行引入第二条主链。 |
