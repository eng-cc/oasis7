...
- `completion-claim-verification-followup` (`PRD-ENGINEERING-AWB-002`, target `test_tier_required`): 建立“fresh verification before completion claims” 的 repo-owned checklist/helper/smoke，阻断未 fresh 跑验证就宣称完成、通过或可提 PR 的收口路径。启动时需创建独立 `.pm` task 与 worktree。
...
- 更新日期: 2026-05-19
- 当前阶段: planned
- 当前任务: `agent-workflow-borrowing-governance`
- 阻塞项:
  - `workflow-behavior-eval-harness-followup` 仍需决定 fixture 形态与“真实 agent 行为”采样面。
  - `completion-claim-verification-followup` 仍需决定 helper 是走 shell checklist、PM closeout hook 还是独立 smoke。
...
- 下一步:
  ...
  - 然后推进 `completion-claim-verification-followup`，把 evidence-first 收口从软规则升级为可验证契约。
