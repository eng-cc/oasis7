...
- Success Criteria:
  ...
  - SC-8: `workflow-report --phase close` 的 checklist 必须明确给出“commit 后通过 `./scripts/prepare-task-pr.sh` 执行 GitHub PR preflight / create，并以 required checks、requested changes、comment/thread closeout、mergeability 与 GitHub merge path 作为默认评审/合流边界；`REVIEW_REQUIRED` 仅回报不阻塞；review-approval-only `BLOCKED` 可按授权 admin merge”的固定动作，不得只在人工约定层存在。
