...
- Success Criteria:
  ...
  - SC-8A: `workflow-report --phase close` 与根 `AGENTS.md` 必须一致说明：commit 前 subagent review 属于仓库默认流程，不需要仅因执行该流程再单独向用户申请；但若当前运行环境要求显式授权派生 agent，或用户明确禁止派生 agent / subagent，则必须以上层约束为准。
  - SC-8B: 根 `AGENTS.md` 必须补充一条可直接复用的用户授权模板，供运行环境要求显式授权派生 agent 时直接使用；模板只能表达“允许按默认工作流派生 subagent review”，不得被误写成覆盖上层运行环境或平台策略的绝对授权。
...
- Critical User Flows:
  ...
  7. Flow-SE-007: `owner 进入新 worktree -> 执行 workflow-report --phase start --role <owner> --task-id <TASK-ID> -> task file 记录 last_started_at 并读取 backlog/memory/signal/stage 汇总 -> 开发完成后执行 workflow-report --phase close --task-id <TASK-ID> -> 回写 devlog + signal/memory/backlog + last_closed_at -> commit 前启动独立 subagent review 当前 diff 并处理 findings -> producer/owner 在评审时执行 workflow-report --phase review`
...
| Task registry | `task_id`、`owner_role`、`worktree_hint`、`status`、`source_refs[]`、`doc_refs[]`、`last_started_at`、`last_closed_at`、`updated_at` | 统一扫描 `.pm/tasks/*.yaml`，生成索引与报告；`workflow-report --task-id` 回写 start/close 时间戳；lint 要求 `blocked/done/deferred` 任务必须已有 `last_started_at`，而 `done/deferred` 还必须已有 `last_closed_at` | `missing -> registered -> active -> closed` | 任务 ID 单调递增；按 owner_role 分组 | 任务创建者负责建档；owner role 负责状态更新 |
...
- Acceptance Criteria:
  ...
  - AC-9A: 上述正式口径必须同时写清“仓库默认流程”“运行环境授权边界优先”“用户显式禁令优先”三层关系，不允许再把仓库规则误写成覆盖上层调度约束的绝对授权。
  - AC-9B: 根 `AGENTS.md` 必须附带一条可直接复用的用户授权模板，且模板正文与说明需要同时强调“这是显式授权模板，不是越过运行环境限制的授权替代品”。
...
| PRD-ENGINEERING-SE-007 | TASK-ENGINEERING-085/092/093 | `test_tier_required` + `test_tier_full` | `workflow-report --task-id` start/close/review 视图、task file 时间戳留痕、close checklist 中的 subagent review 要求、默认流程/授权边界/用户授权模板文案一致性、signal 汇总、`new-task-worktree` 提示和角色扩容场景验证 | 日常开发工作流、角色收口动作、阶段评审入口 |
- Decision Log:
