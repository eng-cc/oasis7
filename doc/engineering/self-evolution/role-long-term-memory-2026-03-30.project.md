...
- 最新完成:
  - `TASK-ENGINEERING-092`：已起草 `.pm/templates/role-memory-policy.yaml`，为 7 个标准角色冻结首版 `topic_prefix_allowlist` / `allowed_promotion_reasons` 草案；同时扩展 `pm_store.py` 的 `promotion_reason` 白名单、把 close-phase 记忆抽取三问接入 `workflow-report`，并将 `.pm` 回流要求补入 `producer/runtime/wasm/agent/viewer/qa/liveops` 职责卡。
