...
说明:
`doc/devlog` 历史压缩与 `world-simulator/viewer` 路径级治理都已完成首批入口收口。后续 `p2p / testing` 路径级治理，以及季度复核是否接入 baseline/门禁扩展，均需至少各自独立创建 `.pm` task；默认仍建议独立 worktree，除非用户明确要求复用当前 PR/工作树。
...
## 状态
- 当前阶段: M2 已完成
- 阶段说明: formalize + report + first two follow-ups closed
- 阻塞项: 无
- 最近更新: 2026-04-17
- 后续动作: 入口减重专题 `PRD-ENGINEERING-024` 在默认阅读面层面已完成收口，`doc/devlog` 与 `world-simulator/viewer` 也已完成首批 follow-up。后续应按 `p2p -> testing -> 季度复核` 的顺序继续切题执行，不再回到“只改 README/首屏”的处理方式。
