...
- Required-gate changed-path planning is config-driven through `scripts/ci-required-scope.v2.json`; the planner is the only mapping authority for GitHub, local preflight, and CI receipts. Rules union matching gates, `full` dominates, known documentation paths may select no Rust gates, and unmatched/unresolvable paths select full scope. Invalid configuration fails the planner before CI execution.
...
- While the draft candidate's same-head CI is running, TPM may run `./scripts/pm/review-plan.py` to deterministically precompute the minimal role set, canonical slice UUIDs, immutable expected batch epoch, and reference-only packet locations for the frozen task/head/evidence/comparison identity. Before any role selection or immutable artifact creation, the helper requires the resolved comparison OID to be an ancestor of the frozen head; a divergent range fails closed with a rebase/refresh instruction so no formal review epoch is spent on unrelated history. The plan records the canonical symbolic `comparison_ref` and the helper-resolved `comparison_oid`; both bind slice allocation and reuse, and a ref that no longer resolves to the recorded OID fails closed. `--preflight-dir` may additionally materialize only the incomplete batch artifacts and ledger. This planning/preflight work is preparation, not a formal review return, collection, or Pre-PR Ready evidence: it may not dispatch a completed review, write a collection receipt, or advance the CI-to-review gate order. Identical task/head/evidence/comparison/role inputs reuse the same plan and epoch; any such drift creates a distinct epoch and invalidates reuse. Invalid selector input fails closed.
- After same-head CI and fresh per-role packet creation, but immediately before each formal review specialist dispatch, TPM must run `./scripts/pm/subagent-task-packet.py review-admission --packet <packet> --review-plan <plan> --bootstrap-snapshot <snapshot>`. Admission revalidates live task/worktree/branch/HEAD truth, validates the snapshot's immutable task/bootstrap-epoch identity without requiring its initial HEAD/base to remain current, validates the plan against its canonical immutable review batch/epoch and complete role/slice/packet-ref set, then cross-binds the current packet path, task, role, slice, comparison ref/OID, base OID, and frozen head to exactly one planned slice. Project lifecycle status is a historical observation in the bootstrap snapshot, not epoch identity; a normal status transition does not require a replacement snapshot, while the fresh task packet still binds current status. The packet and plan, not the bootstrap snapshot, own the later review HEAD/comparison identity. Any snapshot digest or immutable epoch identity, batch/plan role-set, task, worktree, branch, live HEAD, comparison ref/OID, base, packet, role, or slice drift fails before dispatch and requires fresh valid inputs. The successful JSON result is ephemeral evidence for the immediately following human-operated dispatch; it is not persisted as an authority receipt and cannot be reused after drift. Repository code cannot intercept the Codex host spawn primitive, so TPM remains responsible for invoking this fail-closed gate at the skill boundary.
