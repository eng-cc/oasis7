...
The author, local preflight, and required-CI paths use one repository-owned product-document content checker for new or materially changed `doc/product/**/*.prd.md` and `doc/product/**/*.design.md` files. The checker must receive explicit trusted `base` and `head` commit OIDs for a committed range, or an explicit `--worktree` mode that evaluates the selected base/head range together with staged, unstaged, and untracked working-tree changes. A missing or empty range is reported as `checked 0` with its reason; it is not a successful substitute for a requested range. The caller may not bypass the gate with labels, comments, allowlists, or user-controlled opt-out variables.

`scripts/doc-governance-check.sh` is the local author entrypoint and invokes the checker in explicit worktree mode unless its caller supplies the trusted comparison OIDs. If either explicit local OID is supplied, the other is required; it may not silently derive a replacement. `scripts/prepare-task-pr.sh` derives its existing comparison OID and frozen source head, then invokes the same checker in worktree mode against those exact OIDs before reporting preflight success. Required CI passes the workflow's actual event or integration base/head OIDs into the same invocation before the required test tier; a missing, partial, malformed, or unsupported event range, including a CI invocation with no readable event payload or explicit OIDs, fails closed instead of falling back to a local comparison. The committed scope is the selected source head from `merge-base(base, head)`; in worktree mode, staged and unstaged product-document changes are relative to caller's checked-out `HEAD`, and untracked product docs are added separately. Target-only changes already present in a divergent checkout are not source changes unless locally edited. All three paths fail closed on malformed identity or checker failure and retain the product documentation standard's changed/new-only boundary; unchanged legacy product files remain exempt.
