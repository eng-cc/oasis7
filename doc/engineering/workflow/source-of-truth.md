...
- Multi-agent orchestration should request the repo-configured `multi_agent_v2` feature flag from `.codex/config.toml` when the installed Codex build exposes it. Because the feature can be under development or unavailable in a given connector/runtime, slice contracts must distinguish the intended multi-agent runtime (`multi_agent_v2`) from the actual exposed runtime/tool namespace. When the tool/runtime exposes a concrete namespace, runtime label, or tool surface, record that verbatim; when it exposes only a tool surface without a canonical runtime label, record that surface plus the limitation; use `inherited/unverified` only when the actual dispatch surface cannot be verified.
...
- Minimal read-only specialist slice contract:
  ...
  - intended multi-agent runtime, defaulting to repo-configured `multi_agent_v2` when the installed Codex build exposes it
  - actual multi-agent runtime/tool namespace or exposed tool surface, recorded verbatim when known; otherwise the limitation reason, or `inherited/unverified` only when the actual dispatch surface cannot be verified
...
- **v1.4.18 (2026-06-09)**
  - Enabled the repo-configured `multi_agent_v2` feature flag in `.codex/config.toml`.
  - Required subagent slice contracts to distinguish intended `multi_agent_v2` orchestration from the actual exposed multi-agent runtime/tool namespace or tool surface, recording the exposed label verbatim when known and using `inherited/unverified` only when the installed Codex build cannot verify the actual dispatch surface.
- **v1.4.10 (2026-06-03)**
