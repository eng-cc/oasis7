...
- After same-head CI and fresh per-role packet creation, but immediately before each formal review specialist dispatch, TPM must run `./scripts/pm/subagent-task-packet.py review-admission --packet <packet> --review-plan <plan> --bootstrap-snapshot <snapshot>`. Admission revalidates live task/worktree/branch/HEAD truth, validates the snapshot's immutable task/bootstrap-epoch identity without requiring its initial HEAD/base to remain current, validates the plan against its canonical immutable review batch/epoch and complete role/slice/packet-ref set, then cross-binds the current packet path, task, role, slice, comparison ref/OID, base OID, and frozen head to exactly one planned slice. The packet and plan, not the bootstrap snapshot, own the later review HEAD/comparison identity. Any snapshot digest or epoch identity, batch/plan role-set, task, worktree, branch, live HEAD, comparison ref/OID, base, packet, role, or slice drift fails before dispatch and requires fresh valid inputs. The successful JSON result is ephemeral evidence for the immediately following human-operated dispatch; it is not persisted as an authority receipt and cannot be reused after drift. Repository code cannot intercept the Codex host spawn primitive, so TPM remains responsible for invoking this fail-closed gate at the skill boundary.
