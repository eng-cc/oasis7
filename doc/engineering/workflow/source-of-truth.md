...
Construction is acyclic: build payload `P` without `manifest_digest`, server readback, or comment fields; hash `P` to obtain `manifest_digest`; then write `P` plus that digest once. `P` binds `task_uid`, frozen `head`, `epoch`, and one `status=completed` role record per finding-bearing return.
