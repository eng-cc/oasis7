...
For each `required=true` obligation, aggregate evidence MUST contain exactly one row whose `mapping_slot` equals the obligation's declared slot and whose slot owner loop/role equals the obligation owner. `mapping_slots.allowed_task_uids` omitted means no additional UID restriction, a non-empty array is an exact allowlist, and an explicit empty array allows no UID and therefore blocks any required obligation bound to that slot. A leaf evidence locator MUST be a structured canonical repository/Issue/comment identity with an exact body digest; its live Task Issue and comment MUST read back a versioned `oasis7.loop-leaf-result/v1` result produced by a repository-owned named verification profile. Generic PM, claim-verification or terminal comments, local JSON, mutable URLs and caller-declared digests or authors cannot substitute. The producer MUST read back the exact published body, and the consumer MUST verify the live server author and current comment permission before using the result.
...
- Before editing or execution, the `Plan-Gap Evidence` for each ordered execution step must record all of:
  `step_id`, `acceptance_refs`, `dependencies`, `verification_command`,
  `verification_evidence`, `write_scope`, `out_of_scope`, and `required_role_slices`.
  Each field is non-empty: refs map to acceptance/task truth, dependencies identify prerequisite evidence, verification fields name the command and expected evidence target before execution and capture the actual result afterward, scopes name exact paths and explicit non-goals, and role slices name required roles or an explicit reasoned exemption. A missing field, evidence, or mapping fails closed before editing or execution; record the entry in a GitHub task issue evidence comment, which remains the sole mutable plan truth and also records the current plan, work decomposition when needed, selected roles, and integration order before professional subagent dispatch.
