...
- Minimal read-only specialist slice contract:
  ...
  - actual multi-agent runtime/tool namespace, or fallback reason when the connector/runtime exposes only `multi_agent`/`multi_agent_v1` or cannot verify the actual runtime
...
- **v1.4.18 (2026-06-09)**
  ...
  - Required subagent slice contracts to distinguish intended `multi_agent_v2` orchestration from the actual exposed multi-agent runtime/tool namespace, including fallback to `multi_agent`/`multi_agent_v1` or `inherited/unverified` when the installed Codex build cannot verify v2 dispatch.
- **v1.4.10 (2026-06-03)**
