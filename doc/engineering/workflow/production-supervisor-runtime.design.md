...
Phase-specific aliases remain closed and must carry their category in evidence. Composite parents use only the closed `all_subcases_pass` outcome aggregate with exactly their declared child IDs/outcomes; a scalar parent code is invalid when subcases exist, while non-composites retain one scalar code. Durable wake cases W01-W03 and soak use `workflow_state=external_wait` separately from `input_phase=execute`, the phase where the wait was installed; `same_as_input` preserves that phase.
...
Each case emits one immutable `tpm-supervisor-staging-case/v1` JSON record with catalog identity, input state/phase, typed status/phase rules, and closed `outcome` (scalar non-composite or composite-only `all_subcases_pass` with exact child IDs/outcomes; scalar parent code is invalid when subcases exist); terminal cases require typed `terminal_outcome`, while positive cases require typed `workflow_phase_projection` and `implementation_operation_sequence`. Numeric assertions use
...
`injection`, `expected_outcome`, `status`, `phase`, `cardinality`, `recovery`, `readback`, and `digest`, using the same closed enums/predicates as its parent;
the parent uses `outcome={"kind":"all_subcases_pass","child_ids":[...],"child_outcomes":[...]}`, `cardinality={"aggregate_from":"subcases","operation":"sum"}`, and `recovery={"aggregate":"all_subcases"}`, never one tuple. Aggregate cardinality is the component-wise sum of subcase cardinalities; the parent
