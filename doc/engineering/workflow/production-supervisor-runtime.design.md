...
The following matrix is the minimum fault suite; status/phase are observed at the fault boundary, cardinality covers recovery, and each injection records its point and proves every predicate by readback. Composite A05/C03/W01 subcases remain within one case record per seed, so they add no catalog IDs or records.
...
is `[]` except for composite parent IDs `A05`, `C03`, and `W01`; each composite requires one nested object per declared variant with its own `id`, `fault`,
...
the parent uses `cardinality={"aggregate_from":"subcases","operation":"sum"}` and `recovery={"aggregate":"all_subcases"}`, never one tuple. Aggregate cardinality is the component-wise sum of subcase cardinalities; the parent
