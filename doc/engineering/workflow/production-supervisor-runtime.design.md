...
| `W01` | Wake | `external_wait` | `execute` | Same-digest duplicate, different-digest duplicate, stale, out-of-order, or fencing wake | `same_as_input` | `same_as_input` | `typed_subcases: same_digest_duplicate -> accepted(existing_result); different_digest_duplicate -> stale_receipt; stale -> stale_fencing; out_of_order -> stale_fencing; fencing -> stale_fencing` | `typed_subcases aggregate:A=0/P=0/S=0/R=4/B=0/C=0` | `typed_subcases per_subcase: same_digest_duplicate -> idempotent_existing(attempts<=1,elapsed_s<=120,new_epoch=false,same_key=true); different_digest_duplicate/stale/out_of_order/fencing -> reject_same_epoch(attempts<=1,elapsed_s<=120,new_epoch=false,same_key=false)` |
...
`input={workflow_state:external_wait,phase:execute}` and
...
Each case emits one immutable `tpm-supervisor-staging-case/v1` JSON record with catalog identity, input state/phase, typed status/phase rules, closed `expected_outcome` and `observed_outcome` semantic fields, and a separate closed `verdict` (`pass|fail|blocked`). A scalar semantic field is `{"kind":"code","code":"<catalog-code>"}`; a composite expected field is `{"kind":"subcases","child_ids":[...],"child_outcomes":[...]}`, and its observed aggregate is `{"kind":"all_subcases_pass","child_ids":[...],"child_outcomes":[...]}`. `verdict` is never an outcome code: `pass` requires all expected/observed semantic fields and predicates to match, `fail` records contradictory evidence, and `blocked` records unavailable required evidence; the run summary uses `verdict` only; scalar parent code is invalid when subcases exist. Terminal cases require typed `terminal_outcome`, while positive cases require typed `workflow_phase_projection` and `implementation_operation_sequence`. Numeric assertions use
