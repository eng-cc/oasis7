...
| --- | --- | --- | --- | --- |
| [split source review and integration contract](./source-of-truth.md#split-source-review-integration-contract) | 源审查适用性与集成执行身份分离；无关 target-only advance 可复用 source-bound CI，严格风险必须使用 current-target integration。 | [split source review and integration identity](#41-split-source-review-and-integration-identity) | `repository_health_engineer`; trusted PR CI and integration receipt producers | 不激活 v2、自动 dispatch、全局 loop authority 或 unattended supervisor。 |
...
The minimum safe reuse rule binds the source-review identity and independently verified review applicability; integration execution fields remain separate evidence. A target advance by itself does not require a new exact integration dispatch when trusted closure proves it is unrelated, so ordinary promotion may reuse successful source-bound PR CI with its recorded older target base. A high-risk escalation, verified related consumer/contract drift, merge conflict or unknown dependency/authority closure requires the latest exact integration dispatch and current-PR readback against the current target. A changed source head/scope, role/policy/input digest, new finding, wrong execution ref/base, current-PR drift, new failure/pending/uncertain state, or legacy v1 combined plan requires the applicable new review epoch. This task does not infer dependency closure from path names or activate v2 globally.
...
| 上游 requirement / product AC / professional acceptance（path#fragment） | 本设计条款（path#anchor） | 独立 obligation 与适用条件 | 准确验证方法、test/manual source、scenario/layer、candidate/environment | evidence target | 未证明范围 |
| --- | --- | --- | --- | --- | --- |
| [split source review and integration contract](./source-of-truth.md#split-source-review-integration-contract) | [split source review and integration identity](#41-split-source-review-and-integration-identity) | 普通模式在无关 target advance 后保留 source-bound PR CI；严格模式仍要求 current-target integration receipt；两者都拒绝错误 head/base/app 或失败 evidence。 | [`pr-lifecycle-loop.test.py`](../../../scripts/pm/pr-lifecycle-loop.test.py)；fixture 覆盖 ordinary source-bound reuse 与 strict stale-base rejection。 | Focused test output and the frozen PR review/CI packet. | Fixture 不证明 GitHub hosted activation、真实 dispatch 或 native client isolation。 |
...
| v2 review reuse | Ordinary source-bound PR CI may be reused when source identity and review applicability remain valid and trusted closure proves a target-only advance unrelated; strict/high-risk/related/unknown paths require fresh current-target integration success and current-PR readback. `tested_tree_oid` remains integration evidence, not an ordinary applicability boundary. | Changed source-review applicability, unknown closure, strict integration evidence failure, or a legacy v1 plan requires a new review epoch; tested-tree drift is relevant to strict integration evidence only. |
