...
- `engineering` 根目录默认只保留 `README.md / prd.md / design.md / project.md / prd.index.md` 五个模块入口；治理专题已分别下沉到 `doc-governance/`、`rust-governance/`、`governance/` 与 `self-evolution/`。其中 `doc-surface-area-governance` 负责默认阅读面，`doc-corpus-maintenance-governance` 负责入口减重后的存量维护成本，仍需当前读取的路径治理专题保留在本索引；已完成且规则已由目标子域 README 承接的一次性专题不再作为 live 三件套暴露。`agent-workflow-borrowing-governance` 负责将外部 agent workflow 方法论映射为 repo-owned adopted / rejected / deferred 治理结论，`skill-surface-replacement-governance` 负责冻结本地 skill inventory 的 keep / replace / retire / defer 边界。`doc/devlog` 的当前入口是 `doc/devlog/README.md` compact archive summary，不再通过 active 专题三件套暴露。
...
- 2026-03 全量 PRD 审读机制三件套已退役删除；历史审读证据保留在 Git history logs，当前新增/变更文档追踪以模块入口、`prd.index.md`、`project.md`、`prd.md` 与现行 doc governance 规则为准。
