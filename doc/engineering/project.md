...
- [x] pm-task-compaction-governance (PRD-ENGINEERING-021) [test_tier_required]: 新增 repo-owned `scripts/pm/compact-task-group.sh` 与 focused smoke，要求先把 project/topic Trace 收口到 survivor task，再安全并档同一 owner 的 closed micro-task group，并以当前 superpowers workflow PR 的 truth-refresh 微任务作为首批 compaction 样例。 Trace: .pm/tasks/task_96dd758fdc374cf68354ae3e53bde0e8.yaml
...
- [x] superpowers-conflict-reconciliation-doc (PRD-ENGINEERING-031) [test_tier_required]: 新增 `superpowers` workflow 冲突与互借参考文档，明确各冲突类的真值碰撞面、可 salvage 子模式与 reopen 条件，并吸收后续 conflict doc / table truth refresh 留痕，作为该子域的聚合 Trace。 Trace: .pm/tasks/task_b7b2e89a1bec4fd0a38615773ce91af3.yaml
