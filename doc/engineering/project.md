...
- [x] client-launcher-workspace-compile-recovery (PRD-ENGINEERING-001) [test_tier_required]: 修复 `oasis7_client_launcher` 中 `WebStateSnapshot` 的 `chain_replication_status` 契约漂移，恢复 native/web snapshot apply 路径与测试夹具一致，并将 `UX_STATE_PATH` 收紧为 test-only 导出，使 `./scripts/cargo-dev.sh check --workspace --all-targets` 再次通过。 Trace: .pm/tasks/task_806c53ca74aa4e96b416f5204a422ed2.yaml
