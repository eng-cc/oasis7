...
- [x] default-subagent-model (PRD-ENGINEERING-021) [test_tier_required]: 将专业角色 subagent slice 的默认模型配置写入 workflow 规则，当前固定为 `gpt-5.5` + `reasoning_effort=medium`，并要求非默认覆盖必须在 slice contract 记录原因。 Trace: .pm/tasks/task_b0cfea2835064691b1ac343501327378.yaml
