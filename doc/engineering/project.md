...
- [x] pm-store-lint-followup (PRD-ENGINEERING-001/003) [test_tier_required]: 将新拆出的 `pm_store_cli.py` / `pm_store_docio.py` 纳入 `scripts/pm/lint.sh` 的显式存在检查与 `py_compile` 语法检查，避免 `pm_store.py` 的 sibling modules 漏出 PM 自身结构门禁。 Trace: .pm/tasks/task_7b6929739bc3444f88376c382badf4cf.yaml
