...
- [x] pm-task-reuse-governance (PRD-ENGINEERING-021) [test_tier_required]: 对齐根 `AGENTS.md` 与 engineering 主 PRD 的 `.pm` task 复用口径，明确同一 PR / 同一 worktree 下的 review comment 若仍属于未关闭原 task 的同 owner、同 PRD、同验证面窄修复，默认继续复用原 task 与 execution log，只有原 task 已 close 或 scope/test boundary 扩张时才新开 follow-up task。 Trace: .pm/tasks/task_ed5d8e3d5669484a952f5ad2213c68be.yaml
