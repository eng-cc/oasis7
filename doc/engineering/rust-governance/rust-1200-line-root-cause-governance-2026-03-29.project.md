...
- 更新日期: 2026-04-21
- 当前阶段: active
- 当前任务: `module-release-support-hotspot-shrink` 已完成；`crates/oasis7/src/runtime/world/module_actions/release_support.rs` 已将 module release request/approval 分支抽到独立 `release_request_support.rs`，主文件从 1197 行降到 556 行，新 support 文件为 732 行，继续维持 `oversized code files=0, test files=0, structural slice files=0, include targets=0`。
- 阻塞项: 无；后续若再次出现任一超限 Rust 文件或结构切片命名，required gate 直接阻断。
- 最新完成:
  - `module-release-support-hotspot-shrink`：`crates/oasis7/src/runtime/world/module_actions/release_support.rs` 将 `submit/shadow/submit_attestation/approve_role/bind_roles/reject` 六个分支抽到 `release_request_support.rs`，主文件从 1197 行降到 556 行；验证已通过 `env -u RUSTC_WRAPPER cargo check -p oasis7 --lib`、`env -u RUSTC_WRAPPER cargo test -p oasis7 module_release_ -- --nocapture`、`./scripts/check-rust-file-size.sh`、`./scripts/doc-governance-check.sh` 与 `git diff --check`。
...
- 下一步:
  - 继续按 near-limit 清单推进预防性拆分；当前新的最高风险热点是 `crates/oasis7/src/bin/oasis7_chain_runtime.rs` 1195 行、`crates/oasis7_consensus/src/membership_recovery/replay.rs` 1192 行、`crates/oasis7/src/simulator/decision_provider.rs` 1187 行。
