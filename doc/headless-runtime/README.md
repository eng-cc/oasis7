...
- 想理解旧 `nonviewer` 命名为什么还保留在专题文件里：先读上面的“说明”，再进入 `doc/headless-runtime/nonviewer/` 中仍被主 PRD 引用的专题。
...
- 汇总 `nonviewer/` 主题下仍需当前读取的 hardening 专题。
- 承接与 testing / core 的 headless 证据链和发布门禁对接口径。

## 主题文档
- `nonviewer/`：仍需当前读取的鉴权与长稳归档专题。
