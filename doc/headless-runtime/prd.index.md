# headless-runtime PRD 文件级索引

审计轮次: 6

更新时间：2026-04-14

## 入口
- 模块 PRD：`doc/headless-runtime/prd.md`
- 模块设计总览：`doc/headless-runtime/design.md`
- 模块标准执行入口：`doc/headless-runtime/prd.md`

当前没有独立的活跃专题三件套。鉴权、防重放、长稳运行与冷归档合同已收敛到模块根 `prd.md` / `design.md` / GitHub task issue evidence comments；检查与事故记录继续使用 `checklists/`、`templates/`。

## 说明
- 本索引用于保证模块专题文档在根入口文档树中可达。
- 文档配对规则：`*.prd.md`、`*.design.md` 与同名 GitHub task issue evidence comments。
- `doc/headless-runtime/nonviewer/README.md` 只解释旧命名与退役专题的历史路由。
- 已退役的 hardening 与 `nonviewer-design-alignment-*` 三件套不再占据活跃索引；历史证据从 Git history 与 GitHub task issue evidence comments 追溯。
