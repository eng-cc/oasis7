...
## 目标
- 将玩家访问模式正式收口到 `viewer / pure_api` 双模式真值。
...
- User Personas:
  ...
  - `viewer_engineer`: 需要明确 Web 侧只维护 `viewer` canonical 入口。
  - `agent_engineer`: 需要继续把 provider/lane 与玩家访问模式分层。
- Critical User Flows:
  1. `判断结论属于 Web 入口还是纯接口入口 -> 绑定到 viewer / pure_api`
...
| --- | --- | --- | --- | --- | --- |
| 玩家访问模式注册表 | `mode_id`、`claim_scope`、`forbidden_claims`、`gameplay_prerequisites` | 任一结论先绑定模式 | `draft -> frozen -> audited` | 只允许 `viewer`、`pure_api`；`software_safe` 仅作兼容 alias | `producer_system_designer` 冻结 |
