...
- 模块设计总览：`doc/core/design.md`
  ...
  - ROUND-002 进展（2026-03-05）:
    ...
    - 已完成第二批执行 `C2-004`：CI/precommit 规则归属收口（CI 文档链定义规则，pre-commit 主专题保留入口与修复流程；后续已删除无独立权威的 remediation 兼容叶）。
...
- [ ] TASK-CORE-054 (PRD-CORE-009) [test_tier_required]: 在 `software_safe` 默认主入口实现与 QA 证据完成后，再统一更新 README/testing/manual/current-entry 文案，避免设计目标与已上线行为混写。
  - 当前命名注记：仓库现行 canonical Web/UI 名称已收口为 `viewer`，`software_safe` 仅作为 compat alias；该旧任务标题保留为历史追踪文本，不再代表当前 formal mode taxonomy。
