...
产品层只从 `doc/product/README.md` 进入，固定为四个产品模块；其中“大世界基础设施”只有一个产品 PRD。`doc/product/` 是这四个模块全部产品层文档的最终归并目录；现有散落在其他目录的产品层内容尚未完成迁移，不能据此视为已归并。

新建或改写四模块相关的产品承诺、跨域组合和端到端验收内容时，应归入对应的 `doc/product/<产品模块>/`；其他目录只保留其专业域规则、实现合同、测试/运维或任务证据，并回链产品入口，避免重复或混写产品正文。下方矩阵是工程与治理模块导航，两者不可混写。
