...
- Edge Cases & Error Handling:
  ...
  - task closeout helper：helper 只负责 `.pm` close-phase bookkeeping 与 `done` closeout 前的 fresh verification，不替代 GitHub task issue evidence 回写、commit 或 `prepare-task-pr`；若 task 尚未 start、已处于关闭态，或 `done` closeout 缺少 `--verify-command`，脚本必须在改动前失败，不允许留下半收口状态。
  ...
  - `.pm` rebase conflict helper：helper 只允许自动修 `.pm/inbox/signals.jsonl` 的 signal-id 冲突；若冲突来自 GitHub-backed task mapping / archive、memory、stage 或其他 canonical 对象，脚本只能分类并提示人工处理，不得擅自重写真值。
