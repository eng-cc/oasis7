...
- Success Criteria:
  ...
  - SC-12: 仓库必须提供标准化 task worktree GitHub PR 收口入口，让已完成需求能够在干净状态下统一执行 PR preflight / create；创建 PR 前必须已有 source-bound 的本地相关角色 subagent review evidence packet，并继续把 required checks + review/approval 作为 `main` 的服务端保护边界。
  - SC-12E: `scripts/prepare-task-pr.sh --create` 必须在 push / `gh pr create` 前检查 source worktree 绑定 task 的 execution log；缺少匹配 `Task UID`、`Source Worktree`、`Source Branch`、`Source Head`、`Comparison Ref` 与 `Pre-PR Local Role Review: passed` 的 evidence packet 时必须失败退出。PR helper 不再请求 Copilot review。
...
- Critical User Flows:
  ...
  6. Flow-SCR-006: `prepare-task-pr.sh [task/<module>-<task>] -> 检查 source task worktree 干净状态、base 分支对齐情况与 pre-PR local role review evidence -> 输出或执行 GitHub PR create 命令 -> 通过 required checks + review/approval 合入 `main` -> 同步本地 `main` 并删除已完成 task worktree/branch`
  6D. Flow-SCR-006D: `prepare-task-pr.sh --create -> 在 push / gh create 前校验 source-bound 的 `Pre-PR Local Role Review: passed` packet -> 若缺失或 source head stale 则失败 -> 若通过则执行标准 gh create 命令；不再追加 Copilot reviewer request`
...
| task worktree PR closure | `source_branch`、`source_worktree`、`base_branch`、`comparison_ref`、`ahead_count`、`behind_count`、`create_command`、`cleanup_commands`、`local_required_validation.scope`、`local_required_validation.reason_summary`、`local_required_validation.recommended_required_command` | 通过统一入口校验任务分支、输出或执行 GitHub PR create 命令，并给出 PR 合入前的本地 required 验证建议、planner 原因摘要与合入后的本地同步/cleanup 命令 | `ready_to_pr -> preflighted -> locally_recommended -> pr_opened -> merged -> cleaned_up` | 默认源分支取当前 branch，base 默认 `main`、remote 默认 `origin`；本地推荐只复用 changed-path planner 做只读建议，不自动执行 | `producer_system_designer` 定流程，scripts owner 维护入口 |
