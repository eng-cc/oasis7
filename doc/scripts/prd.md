...
- Success Criteria:
  ...
  - SC-12E: `./scripts/prepare-task-pr.sh --draft-candidate --create` 必须在 push / `gh pr create` 前检查 source worktree 绑定 task 的 GitHub task issue evidence comments；缺少匹配 `Task UID`、`Source Worktree`、`Source Branch`、`Source Head`、`Comparison Ref` 与 `Pre-PR Local Role Review: passed` 的 evidence packet 时必须失败退出。task-bound legacy `--create` 必须 fail closed，不得记录 `pr_watch`；只有在 exact-head CI、同头 role review 与 fresh live CI receipt / draft-state 检查通过后，才能使用 `./scripts/prepare-task-pr.sh --promote-draft <fresh ci_ready_receipt.json>`。PR helper 不再请求 Copilot review；只有明确记录 manual packaging/release CI purpose 的 PR 才允许在创建后停在人工打包 gate。
...
- Critical User Flows:
  ...
  6. Flow-SCR-006: `./scripts/prepare-task-pr.sh [task/<module>-<task>] --draft-candidate --create -> 检查 source task worktree 干净状态、base 分支对齐情况与 pre-PR local role review evidence -> 创建 frozen-head draft candidate -> exact-head CI 与 role review -> ./scripts/prepare-task-pr.sh --promote-draft <fresh ci_ready_receipt.json> -> 记录 PR purpose decision -> normal PR 盯 required checks + mergeability + comments/threads，REVIEW_REQUIRED 仅回报不阻塞，review-approval-only BLOCKED 可按授权 admin merge，失败修复并重推，comments 先处理/resolve，通过后合入 `main` -> 同步本地 `main` 并删除已完成 task worktree/branch`
  6D. Flow-SCR-006D: `./scripts/prepare-task-pr.sh --draft-candidate --create -> 在 push / gh create 前校验 source-bound 的 `Pre-PR Local Role Review: passed` packet -> 创建 draft candidate -> exact-head CI / role review -> ./scripts/prepare-task-pr.sh --promote-draft <fresh ci_ready_receipt.json> 前校验 live receipt 与 draft state；task-bound legacy --create 在 push / gh create / record-pr 前失败退出 -> 不再追加 Copilot reviewer request；除非明确记录 manual packaging/release CI hold，否则 PR 创建后继续 normal CI/comment/mergeability watch-fix-merge，且 REVIEW_REQUIRED 仅回报不阻塞；review-approval-only BLOCKED 可按授权 admin merge`
