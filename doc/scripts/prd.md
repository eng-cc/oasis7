...
- Success Criteria:
  ...
  - SC-12: 仓库必须提供标准化 task worktree GitHub PR 收口入口，让已完成需求能够在干净状态下统一执行 PR preflight / create；创建 PR 前必须已有 source-bound 的本地相关角色 subagent review evidence packet，并继续把 required checks + review/approval 作为 `main` 的服务端保护边界；普通 PR 创建后默认继续 watch required checks / review / mergeability，失败修复并重推，通过后合入与清理。
  - SC-12E: `scripts/prepare-task-pr.sh --create` 必须在 push / `gh pr create` 前检查 source worktree 绑定 task 的 execution log；缺少匹配 `Task UID`、`Source Worktree`、`Source Branch`、`Source Head`、`Comparison Ref` 与 `Pre-PR Local Role Review: passed` 的 evidence packet 时必须失败退出。PR helper 不再请求 Copilot review；只有明确记录 manual packaging/release CI purpose 的 PR 才允许在创建后停在人工打包 gate。
...
- Critical User Flows:
  ...
  6. Flow-SCR-006: `prepare-task-pr.sh [task/<module>-<task>] -> 检查 source task worktree 干净状态、base 分支对齐情况与 pre-PR local role review evidence -> 输出或执行 GitHub PR create 命令 -> 记录 PR purpose decision -> normal PR 盯 required checks + review/approval + mergeability，失败修复并重推，通过后合入 `main` -> 同步本地 `main` 并删除已完成 task worktree/branch`
  6D. Flow-SCR-006D: `prepare-task-pr.sh --create -> 在 push / gh create 前校验 source-bound 的 `Pre-PR Local Role Review: passed` packet -> 若缺失或 source head stale 则失败 -> 若通过则执行标准 gh create 命令；不再追加 Copilot reviewer request；除非明确记录 manual packaging/release CI hold，否则 PR 创建后继续 normal CI/review watch-fix-merge`
...
| task bootstrap followups | `doc_checks`、`today_devlog_path`、`harness_mode`、`harness_state_file`、`viewer_url` | 通过 `--init-docs` / `--with-harness` 补齐文档检查与 harness 预热 | `ready -> doc_checked -> harness_booted` | `--init-docs` 只读检查模块文档；`--with-harness` 默认调用 `worktree-harness.sh up` 并继承 formal gameplay 的 active LLM preflight | `qa_engineer` 与 scripts owner 协同维护 |
| task worktree PR closure | `source_branch`、`source_worktree`、`base_branch`、`comparison_ref`、`ahead_count`、`behind_count`、`create_command`、`cleanup_commands`、`local_required_validation.scope`、`local_required_validation.reason_summary`、`local_required_validation.recommended_required_command`、`pr_purpose_decision` | 通过统一入口校验任务分支、输出或执行 GitHub PR create 命令，并给出 PR 合入前的本地 required 验证建议、planner 原因摘要与合入后的本地同步/cleanup 命令；普通 PR 创建后继续盯 required checks/review/mergeability，失败修复并重推，通过后合入；明确 manual packaging/release CI purpose 的 PR 可停在人工打包 gate | `ready_to_pr -> preflighted -> locally_recommended -> pr_opened -> purpose_decided -> watching_ci_review -> fixing? -> merged -> cleaned_up` | 默认源分支取当前 branch，base 默认 `main`、remote 默认 `origin`；本地推荐只复用 changed-path planner 做只读建议，不自动执行；未明确记录 manual packaging/release CI purpose 时默认 `normal_pr_ci_watch` | `producer_system_designer` 定流程，scripts owner 维护入口 |
...
- Acceptance Criteria:
  ...
  - AC-4: `run-viewer-web.sh`、`viewer-software-safe-step-regression.sh` 与相关 freshness/browser automation 脚本被明确为当前 Viewer Web 主链路。
  ...
  - AC-15: `scripts/new-task-worktree.sh --help` 必须列出 `--init-docs` 与 `--with-harness`；前者输出 `doc/<module>/prd.md`、`doc/<module>/project.md` 和当日 `doc/devlog/YYYY-MM-DD.md` 的存在性摘要，后者在新 worktree 中后台预热 `./scripts/worktree-harness.sh up` 并继承 formal gameplay 的 active LLM preflight。
  ...
  - AC-18: `scripts/prepare-task-pr.sh --help` 必须明确列出 `--base`、`--remote`、`--create`、`--draft` 与 `--json`；`--json` 至少输出 `source_branch`、`source_worktree`、`base_branch`、`comparison_ref`、`ahead_count`、`behind_count`、`create_command`、`cleanup_commands`、`local_required_validation.scope` / `local_required_validation.reason_summary` / `local_required_validation.recommended_required_command` 与 `pre_pr_local_role_review.status` / `pre_pr_local_role_review.execution_log_path` / `pre_pr_local_role_review.missing_markers`。
  - AC-18J: `scripts/prepare-task-pr.sh` 不得请求 Copilot review，也不得公开 `--no-copilot-review`。`--create` 必须在 push / `gh pr create` 前强制要求 source-bound pre-PR local role review evidence；非 `--create` preflight 必须报告 evidence 状态，便于 owner 在创建 PR 前补齐。
