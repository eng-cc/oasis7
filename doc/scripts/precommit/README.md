...
- Need the current local commit-baseline contract, hook setup, or the boundary between `commit` and heavier `required` checks: read `pre-commit.prd.md`.
...
| --- | --- |
| What runs before a local commit? | `pre-commit.prd.md` plus `scripts/pre-commit.sh` |
