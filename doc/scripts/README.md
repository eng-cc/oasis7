...
- `scripts/run-local-letai-game-test.sh` 为本地真实 LetAI provider bridge + runtime/game 测试主入口；它负责 token config 规范化、默认 Rust direct provider bridge、Rust bridge chat probe/provider contract smoke 与 launcher stack 串联。Python `check-letai-chat-completions.sh` 只保留作 legacy adapter 兼容诊断。直接跑 `run-launcher-stack.sh` 只适合作为底层 bootstrap、wrapper 内部调用或定向回归，不应再作为本地真实 LLM 游戏测试的人工入口。
