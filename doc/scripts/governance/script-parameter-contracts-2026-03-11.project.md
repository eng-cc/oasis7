...
| `scripts/release-gate.sh` | `./scripts/release-gate.sh --out-dir .tmp/release_gate` | `--out-dir` 默认 `.tmp/release_gate`；`--quick` 缩短长跑；`--dry-run` 只记录不执行；`--skip-ci-full/--skip-sync/--skip-web-strict/--skip-s9/--skip-s10` 会缩减覆盖范围，且必须提供对应非空 `--skip-*-reason` | 非零可能来自某个 gate step 失败，也可能来自 skip 缺少 reason 的参数治理失败；使用 `skip-*` 不等同完整放行；summary 必须输出 skipped step 的 claim boundary；`--dry-run` 不产生真实通过结论 | Step 语义：`ci_full | sync_m1 | sync_m4 | sync_m5 | web_strict | s9 | s10`。 |
