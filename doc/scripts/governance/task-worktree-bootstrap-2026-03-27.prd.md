...
- Success Criteria:
  ...
  - SC-5: `--init-docs` 可立即报告 `doc/<module>/prd.md` 与 `doc/<module>/project.md` 的存在性；启用 PM bootstrap 时返回对应 GitHub task issue evidence comments 入口与 mapping 记录。
...
- Critical User Flows:
  ...
  4. `scripts/new-task-worktree.sh <module> <task> --init-docs -> 输出模块 PRD / project 的存在性与建议下一步；启用 PM bootstrap 时输出 GitHub task issue evidence comments 入口与 mapping 记录`
...
| 文档检查 | `doc_checks.prd`、`doc_checks.project`、`pm_task.issue_url`、`pm_task.mapping_path` | `--init-docs` 时报告模块文档存在性与路径；启用 PM bootstrap 时返回 GitHub task issue evidence comments 入口与 mapping 记录 | `ready -> docs_checked` | 只读检查，不自动创建文档；task evidence 由 GitHub task issue evidence comments 承载 | `producer_system_designer` / `qa_engineer` 可读 |
...
- Acceptance Criteria:
  ...
  - AC-6: `--init-docs` 至少输出 `doc/<module>/prd.md` 与 `doc/<module>/project.md` 的路径与存在性；启用 PM bootstrap 时输出对应 GitHub task issue evidence comments 路径。
