...
## 6. Rationale
- 选择把默认最终合流切到 GitHub PR，而不是继续强化本地 landing，是因为默认保护边界必须落在 required checks、requested changes/comment closeout、mergeability、GitHub merge path 与授权的 review-approval admin merge path，而不是单机历史整理；`REVIEW_REQUIRED` 仅作为状态回报，不单独阻塞。
