...
- [x] pm-rebase-conflict-helper (PRD-SCRIPTS-GHPR-005) [test_tier_required]: 新增 `scripts/pm/rebase-conflict-helper.sh`，统一分类 `.pm/**` rebase 冲突；`.pm/inbox/signals.jsonl` 已退休，命中时只提示删除退休文件或人工归档，git-ignored 本地视图只提示保留 `main` 删除并执行 `./scripts/pm/sync-views.sh`。原始落地追溯为 task_uid task_6e23e1a96ee34d059aa62e4280a367b7，本行当前语义覆盖由本次治理收口。 Trace: #1920 (task_25e251bae8eb4ccb8c375237eede524f)
...
### 3. `.pm` Rebase 冲突辅助
- `scripts/pm/rebase-conflict-helper.sh` 默认只读分类 `.pm/**` 未合并路径，输出 `retired_signal_inbox` / `generated_view` / `task_yaml` / `task_execution_log` / `memory_yaml` / `stage_yaml` 等类别。
- `.pm/inbox/signals.jsonl` 已退休；helper 命中该路径时只归类为 `retired_signal_inbox` 并提示删除退休文件或人工归档，不再提供 `--resolve-signals` 自动修复路径。
...
- 当前阶段：已完成
- 阻塞项：无
- 下一步：若后续要补 wasm gate 解释层、自动等待 required checks、merge queue 辅助或更激进的 `.pm` 冲突辅助，应新开下一轮交付专题；本轮只冻结“默认最终合流经由 GitHub PR + preflight 给出本地最小 required 推荐与 planner 原因摘要 + `.pm` rebase 冲突 helper 只分类并提示 retired signal inbox / generated view / manual conflict 处置”的契约。
