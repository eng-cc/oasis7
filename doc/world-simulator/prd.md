...
| launcher feedback / transfer / explorer / control plane | `PRD-WORLD_SIMULATOR-020-031`, `033/034/044` | native/Web launcher 共享控制面与前端语义；反馈当前 authority 为 `launcher/game-client-launcher-feedback.prd.md`：native 仅在 chain `Ready` 后尝试远端提交，远端失败才本地回落；Web 经控制面代理且不承诺浏览器本地回落。转账、explorer、链状态、GUI Agent 和 stale execution-world 恢复必须结构化、可诊断、可回归。 |
