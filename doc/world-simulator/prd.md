...
- 分册索引:
  ...
  - `doc/world-simulator/llm/llm-decision-provider-standard-loopback-provider-feasibility-2026-03-12.prd.md`（PRD-WORLD_SIMULATOR-036）
  - `doc/world-simulator/llm/llm-provider-loopback-http-integration-2026-03-12.prd.md`（PRD-WORLD_SIMULATOR-037）
  - `doc/world-simulator/llm/llm-provider-agent-experience-parity-2026-03-12.prd.md`（PRD-WORLD_SIMULATOR-038）
  ...
  - `doc/world-simulator/prd/acceptance/provider-agent-parity-scenario-matrix-2026-03-12.md`（PRD-WORLD_SIMULATOR-038）
  - `doc/world-simulator/prd/acceptance/provider-agent-parity-score-card-2026-03-12.md`（PRD-WORLD_SIMULATOR-038）
  - `doc/world-simulator/prd/acceptance/provider-agent-parity-benchmark-protocol-2026-03-12.md`（PRD-WORLD_SIMULATOR-038）
  - `doc/world-simulator/prd/acceptance/provider-agent-parity-aggregation-template-2026-03-12.md`（PRD-WORLD_SIMULATOR-038）
  - `doc/world-simulator/llm/provider-agent-profile-oasis7_p0_low_freq_npc-2026-03-13.md`（PRD-WORLD_SIMULATOR-037/038 supporting spec）
  ...
  - `doc/world-simulator/viewer/viewer-pixel-world-commercial-rendering-loop-2026-05-28.prd.md`（PRD-WORLD_SIMULATOR-046）
...
- M22 (2026-03-12): 完成 `Decision Provider` 标准层与 `Local Provider` 外部适配可行性建模，冻结“provider advisory / runtime authoritative”边界。
- M23 (2026-03-12): 完成 `Local Provider(Local HTTP)` 用户机接入方案建模，明确本地发现、握手、配置、决策与回退路径。
- M24 (2026-03-12): 完成 `Local Provider` 与内置 agent 体验等价（parity）验收方案建模，把 parity 升级为上线门禁。
...
- Success Criteria:
  ...
  - SC-41: world-simulator 的 Agent 决策层必须支持 provider-agnostic 标准接口，使外部 agent framework（如 `Local Provider`）可经 adapter 参与模拟，同时保持 runtime 权威、trace 连续性与可离线 required 测试。
  - SC-42: world-simulator 必须提供 `Local Provider(Local HTTP)` 首期接入路径，使安装在用户机器上的 `Local Provider` 可通过 `agent_decision_source=provider_backed + agent_provider_backend=provider_local_bridge + agent_provider_contract=worldsim_provider_v1 + agent_provider_transport=loopback_http` 驱动低频 agent，并具备发现、绑定、错误提示与安全回退能力；`agent_direct_connect/provider_loopback_http` 只保留为兼容 alias。
  - SC-43: `Local Provider` provider 在纳入范围的 agent 场景中必须达到与内置 agent 可感知等价的用户体验；若未通过 parity 验收，不得默认启用或扩大覆盖范围。
  ...
  - SC-53: repo-owned Local Provider lightweight runtime agent 的默认 agent id 与 setup 脚本 operator env key 必须迁移到 `oasis7_provider_agent` 与 `OASIS7_PROVIDER_RUNTIME_*`，并将旧 `OASIS7_PROVIDER_RUNTIME_*` 降为兼容 fallback；`oasis7-run.sh`、skill 文档、bridge 示例与 setup 脚本不得继续把旧品牌命名当作当前默认或有效 operator 入口。
  ...
  - SC-57: Local Provider 首期 `P0` 默认 gameplay profile id 必须迁移到 `oasis7_p0_low_freq_npc`，并移除旧 `oasis7_p0_low_freq_npc` 兼容别名；launcher、runtime live、parity bench、bridge 与 `oasis7` operator 文档不得继续把旧 profile id 当作当前默认或有效入口。
  ...
  - SC-60: `oasis7_provider_parity_bench` 的自定义 Local Provider profile 样例必须优先使用 `oasis7_*` 命名，避免 bench 参数解析测试继续把旧 `oasis7_*` profile id 当作非默认样例真值。
  ...
  - SC-71: `oasis7_game_launcher` 与 `viewer/runtime_live` 的 launcher path、viewer auth、Local Provider/runtime chat echo 兼容 env 代码路径中，源码级常量/辅助函数/定向测试命名必须优先使用 `compat` 语义而不是继续沿用 `LEGACY_*OASIS7_*` 口径，避免 launcher/runtime live 维护层的模块名/变量名继续把旧品牌当作当前源码真值。
  ...
  - SC-76: `setup-provider-oasis7-runtime` 等仍保留的活跃脚本中的旧品牌 Local Provider env helper 与局部变量命名必须优先使用 `compat` 语义，而不是继续沿用 `legacy_key` 一类旧口径，避免脚本维护层继续把旧品牌当作当前源码真值。
  ...
  - SC-99: `viewer/runtime_live` 运行时自身不再接受任何旧品牌 provider/chat echo alias；provider 配置与 QA chat echo 开关必须只认 `OASIS7_*` 当前入口。
  ...
  - SC-104: `oasis7_provider_local_bridge` 不再接受旧 profile alias `oasis7_p0_low_freq_npc`；bridge 默认与校验入口必须只认 `oasis7_p0_low_freq_npc` 当前 profile，避免本地 provider bridge 继续保留旧品牌 profile 入口。
  ...
  - SC-109: `slot-1` onboarding 不能只停留在文档；world-simulator 必须对齐 `PRD-GAME-011` / `PRD-WORLD_RUNTIME-040` 的直连认领口径：玩家读取 canonical quote，经显式确认后直接进入 `ClaimAgent`，runtime 在 dedicated pool 足够时自动补足 restricted starter amount；若资金仍不足，只返回资金不足 blocker，不生成 pending/operator-review 审批状态。`viewer` 正式玩法摘要（兼容旧 `software_safe` 入口名）必须展示 quote、auto-funding amount、资金缺口、claim result 与 provenance，使“点击 claim 后发生什么”落在真实产品链路与链上记录上。
...
- User Stories:
  ...
  - PRD-WORLD_SIMULATOR-036: As an `agent_engineer`, I want a provider-agnostic decision layer between world-simulator and external agent frameworks such as `Local Provider`, so that I can evaluate or swap external agent runtimes without weakening runtime authority, traceability, or QA contracts.
  - PRD-WORLD_SIMULATOR-037: As a 玩家 / 制作人, I want a provider-backed Local Provider path on my machine, with the current contract tuple `agent_decision_source=provider_backed + agent_provider_backend=provider_local_bridge + agent_provider_contract=worldsim_provider_v1 + agent_provider_transport=loopback_http`, so that I can let locally installed `Local Provider` drive low-frequency game agents through localhost without deploying remote services.
  ...
  - PRD-WORLD_SIMULATOR-039: As a 玩家 / QA / 制作人, I want `viewer` to be the low-fidelity but formally playable primary Web entry, with `software_safe` retained only as a compatibility alias, so that browser gameplay no longer depends on high-fidelity graphics prerequisites while still remaining bounded and auditable.
  ...
  - PRD-WORLD_SIMULATOR-041: As a 制作人 / viewer_engineer / qa_engineer, I want all 3D visualization work paused and the current user-interaction branch held as a staged reference, so that near-term capacity stays on non-3D interaction scope, especially `viewer` Web interaction, launcher/runtime interaction, and gameplay closure, while future resumption remains auditable.
...
- 模式分层说明：按 `PRD-CORE-009`，`PRD-WORLD_SIMULATOR-039` 对应玩家访问模式 `viewer`，且现在承接低保真但正式可玩的主要 Web 入口；`software_safe` 仅作为兼容 alias / 历史入口名保留。仓库已删除旧 3D / visual-QA Viewer surface，不再保留独立 `standard_3d` taxonomy。`PRD-WORLD_SIMULATOR-037` 定义 provider-backed Local Provider 路径（`agent_decision_source=provider_backed + agent_provider_backend=provider_local_bridge + agent_provider_contract=worldsim_provider_v1 + agent_provider_transport=loopback_http`；`agent_direct_connect/provider_loopback_http` 仅作兼容 alias），`PRD-WORLD_SIMULATOR-040` 定义 `player_parity / headless_agent` execution lane；`PRD-WORLD_SIMULATOR-041` 只定义当前研发优先级与冻结策略，不新增玩家访问模式。`non-3D interaction` / `2D 优先` 在这里是 Viewer 当前 delivery priority 或 interaction scope，不是 `viewer` 的别名，也不是新的 mode_id。
- Critical User Flows:
  ...
  33. Flow-WS-033（Viewer 3D 工作流冻结）:
     `收到新的 Viewer 需求 -> 判断是否属于 3D scene/camera/render/material/light/post-process 可视化专题 -> 若是则标记 paused 并归档到当前用户交互分支暂存态 -> 若否则继续落到 viewer / launcher / runtime interaction 主链路 -> 仅在恢复门禁通过后才允许重新激活 3D workstream`
  34. Flow-WS-034（Launcher explorer 主链级信息架构重构）:
     `打开 explorer -> 先读链健康概览与状态 mix -> 切 tab 进入高密度主列表 -> 单击目标后在详情检查板确认字段 -> 遇到空态/错误/未就绪时直接在当前面板恢复`
...
| Launcher native/web 同控制面 | `/api/state`、`/api/start`、`/api/stop`、`/api/chain/start`、`/api/chain/stop`、`chain_p2p_status/chain_observability_status/chain_replication_status` | native 与 web 端统一走 API 触发游戏/区块链独立启停，并消费同一份链状态/peer 观测快照 | `service_ready -> game_running/stopped + chain_starting/ready/stopped` | 状态以服务端快照为准，客户端仅做展示与轮询 | 控制面部署在受信网络，客户端仅消费授权 API |
...
| Viewer 3D 暂停治理 | `workstream_id`、`pause_status`、`branch_state`、`allowed_change_scope`、`resume_gate` | 新的 3D 需求进入暂停池；当前用户交互分支只保留暂存参考；仅允许保证主链路不腐烂的低风险维护 | `active -> paused -> hold -> resume_review -> resumed` | 先按是否属于 3D 可视化专题分类，再按是否影响 `viewer` / launcher / runtime 主链路决定是否允许修改 | `producer_system_designer` 冻结；`viewer_engineer` 仅可在允许范围内维护 |
- Acceptance Criteria:
  - AC-1: world-simulator PRD 覆盖场景、Viewer、LLM、启动器四条主线。
  - AC-2: world-simulator project 文档维护任务拆解与状态。
  - AC-3: 与 `doc/world-simulator/scenario/scenario-files.prd.md`、`doc/world-simulator/viewer/viewer-web-software-safe-mode-2026-03-16.prd.md` 等活跃专题文档一致。
  ...
  - AC-51: `viewer` Web 入口、旧 `software_safe` alias、auth/bootstrap 与当前仍保留的 panel/headless 控制必须优先读取 `OASIS7_VIEWER_*`；现存 Web 主链路不得因前缀切换而失效。
  ...
  - AC-58: `scripts/setup-provider-oasis7-runtime.sh`、`site/skills/oasis7.md` 与 `oasis7_provider_local_bridge` 相关测试样例必须只使用 `oasis7_provider_agent` 与 `OASIS7_PROVIDER_RUNTIME_{AGENT_ID,WORKSPACE,MODEL}`；repo-owned runtime workspace 默认路径必须切到 `tools/provider/oasis7_provider_workspace`，旧 `OASIS7_PROVIDER_RUNTIME_*` / `oasis7_provider_*` 仅可作为兼容或历史上下文保留，不得继续作为 operator-facing 真值。
  ...
  - AC-62: `oasis7_game_launcher`、`oasis7_provider_parity_bench`、`viewer/runtime_live/llm_sidecar`、`oasis7_client_launcher`、`scripts/provider-parity-p0.sh` 与 `site/skills/oasis7.md` 必须只使用 `oasis7_p0_low_freq_npc`；`oasis7_provider_local_bridge` 不得继续接受旧 `oasis7_p0_low_freq_npc` 别名，定向回归必须明确断言旧 profile id 已失效。
  ...
  - AC-65: `crates/oasis7/src/bin/oasis7_provider_parity_bench.rs` 的 `parse_options_accepts_custom_agent_provider_profile` 样例值必须改为 `oasis7_p1_memory_loop`，避免 bench 的自定义 profile 参数解析回归继续把旧 `oasis7_p1_memory_loop` 当作当前样例真值。
  ...
  - AC-73: `crates/oasis7_client_launcher/src/{self_guided.rs,llm_settings_web.rs}`、`crates/oasis7_viewer/src/right_panel_module_visibility.rs` 与 `crates/oasis7/src/bin/oasis7_provider_local_bridge.rs` 中保留旧品牌 fallback literal 的源码常量/测试名必须改为 `compat` 语义命名；兼容读取行为保持不变，但源码中不得继续把 `LEGACY_*oasis7*` 当作当前维护口径。
  ...
  - AC-81: `scripts/{setup-provider-oasis7-runtime.sh,viewer-texture-inspector-lib.sh,viewer-texture-inspector.sh,capture-viewer-frame.sh}` 中原旧品牌 fallback literal 对应的 helper/局部变量名与调用点必须改为 `compat` 语义命名；脚本行为保持不变，且源码中不得继续把旧品牌 helper 命名当作当前维护口径。
  ...
  - AC-93a: 启动器 `节点观测` 卡必须在保持 native/web 同源 UI 的前提下，直接展示本地 peer id 与已连接 p2p peer 明细；数据来源统一复用 `/api/state` 透传的 `chain_replication_status`，不得新增第二套客户端探针或额外轮询节奏。
  ...
  - AC-98: `testing-manual.md`、`scripts/ci-tests.sh` 与 `crates/oasis7/src/viewer/{server.rs,runtime_live.rs,live_split_part2.rs}` 中作为当前默认说明/输出使用的 `oasis7` 字符串必须全部改为 `oasis7`；`crates/oasis7_proto/src/viewer.rs` 中用于兼容测试的旧 payload 样例可保留，但其测试语义必须继续明确这是 compat payload，而非默认值。
  - AC-99: `README.md`、`site/{index.html,en/index.html}`、`doc/world-simulator/viewer/viewer-manual.manual.md`、`site/doc/{cn,en}/viewer-manual.html`、`oasis7_viewer_live.release.example.toml` 与 `tools/scenario_test_runner/**` 中作为当前入口使用的 `oasis7*` crate/path/command/env/path 说明必须全部改为 `oasis7*` / `OASIS7_VIEWER_*` / `.oasis7_viewer`；其中 `tools/scenario_test_runner` 必须恢复到可解析 `../../crates/oasis7` 的依赖路径并能通过 cargo 检查。
  ...
  - AC-104: `crates/oasis7/src/viewer/runtime_live/{llm_sidecar.rs,control_plane.rs,tests.rs}` 必须移除旧品牌 provider/chat echo fallback 读取；provider 配置与 chat echo 定向测试必须改为断言旧 alias 已失效，且 `cargo test -p oasis7 runtime_live::tests -- --nocapture`、`./scripts/doc-governance-check.sh` 与 `git diff --check` 必须通过。
  ...
  - AC-107: `scripts/build-game-launcher-bundle.sh` 与当前仍保留的 Viewer Web 相关入口必须移除旧品牌 viewer/profile fallback 读取及相关旧品牌 operator 文案；已删除的 `capture-viewer-frame` / texture-inspector 脚本不得再作为验证矩阵的一部分出现。
...
| PRD-WORLD_SIMULATOR-034 | TASK-WORLD_SIMULATOR-103/104/107/108/284 | `test_tier_required` | `./scripts/doc-governance-check.sh` + `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_web_launcher -- --nocapture` + `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_game_launcher -- --nocapture` + GUI Agent 闭环（默认 node id stale 失败 + fresh node id 恢复 + explorer overview 查询成功） + `./scripts/worktree-harness.sh up` 或 `./scripts/run-launcher-stack.sh --with-llm` / agent-browser Web 闭环（fresh `chain_node_id` 默认值） | launcher 链启动恢复体验、GUI Agent 契约、chain-enabled 试玩可达性、一键试玩栈稳定性与 formal gameplay 的 LLM-required 入口约束 |
...
| PRD-WORLD_SIMULATOR-041 | TASK-WORLD_SIMULATOR-285/286 | `test_tier_required` | `./scripts/doc-governance-check.sh` + `rg -n "standard_3d|software_safe|旧 3D / visual-QA Viewer surface|不再保留独立" doc/world-simulator/prd.md doc/world-simulator/project.md doc/world-simulator/viewer/viewer-web-software-safe-mode-2026-03-16.prd.md` + `git diff --check` | 旧 3D workstream 退场治理、现行单入口口径与 active docs 的可审计性 |
| PRD-WORLD_SIMULATOR-046 | `task_b399bf37eff94c44a300c55f5db739d3` | `test_tier_required` | `npm --prefix crates/oasis7_viewer run test:ui -- pixel_world_host` + `npm --prefix crates/oasis7_viewer run build:software-safe` + `./scripts/doc-governance-check.sh` + `./scripts/pm/lint.sh` + `git diff --check` | Pixel-world 商业化首屏、player leverage 语义、fallback route、renderer diagnostics 降噪与 checked-in Viewer Web bundle |
| PRD-WORLD_SIMULATOR-042 | TASK-WORLD_SIMULATOR-308/309 | `test_tier_required` | `bash -n scripts/build-game-launcher-bundle.sh` + `bash -n scripts/package-native-installer.sh` + `bash -n scripts/validate-release-platform-entrypoints.sh` + Linux 实 bundle/`.deb` 验证（`./scripts/build-game-launcher-bundle.sh --out-dir <tmp>/linux-bundle --profile release`、`./scripts/validate-release-platform-entrypoints.sh --platform linux-x64 --bundle-dir <tmp>/linux-bundle`、`./scripts/package-native-installer.sh --platform linux-x64 --bundle-dir <tmp>/linux-bundle --out-dir <tmp>/out --asset-name oasis7-linux-x64.deb --version 0.0.0`、`dpkg-deb --contents <tmp>/out/oasis7-linux-x64.deb`） + macOS/Windows dry-run 入口检查（`./scripts/build-game-launcher-bundle.sh --dry-run --target-triple x86_64-apple-darwin ...`、`./scripts/build-game-launcher-bundle.sh --dry-run --target-triple x86_64-pc-windows-msvc ...`） | Release 下载资产的可运行性、平台原生入口真值、bundle 和安装包之间的入口一致性 |
| PRD-WORLD_SIMULATOR-043 | `task_5d40365b4e714e5799a3baa834e84515` / `task_22a0d58d0d9445b1ad74127c25768d8d` / `task_85eb196085584f8e9ebb9b3f988cd169` / `task_3ec0a2b3318344209a2569fa294d05d1` / `task_30d0e01f74a3ced99f104893beb9d53f` | `test_tier_required` | `./scripts/doc-governance-check.sh` + `./scripts/pm/lint.sh` + `bash -n scripts/build-game-launcher-bundle.sh` + `git diff --check` + `rg -n "AppImage|notariz|codesign|standard installer|主 CTA|覆盖安装|config.toml|reward-runtime-execution-world" doc/world-simulator doc/site/github-pages site scripts/build-game-launcher-bundle.sh` + Windows bundle/installer dry-run + Linux AppImage dry-run | 普通用户向安装分发口径、公开下载面的一包一平台主路径、平台信任链、升级/覆盖边界与支持边界 |
| PRD-WORLD_SIMULATOR-044 | `task_552222a529fa48489eab10deb789ed54` | `test_tier_required` | `./scripts/doc-governance-check.sh` + `env -u RUSTC_WRAPPER cargo test -p oasis7_client_launcher -- --nocapture` + `env -u RUSTC_WRAPPER cargo check -p oasis7_client_launcher --target wasm32-unknown-unknown` + `env -u RUSTC_WRAPPER cargo fmt --all` + Web-first explorer smoke（agent-browser） | 启动器区块链浏览器的主链级信息架构、跨 tab 可扫描性、详情核查效率与跨端一致性 |
- Decision Log:
...
| DEC-WS-024 | 暂停所有新的 3D 可视化推进，并把当前用户交互分支冻结为暂存参考 | 继续并行推进 3D 视觉专题，或直接删除当前用户交互分支实验资产 | 当前阶段最稀缺的是把 formal gameplay、`viewer` Web mode 与非 3D 交互主链路压实；直接删除会丢失未来恢复上下文，继续并行推进则会稀释交付资源。 |
