...
- User Stories:
  ...
  - PRD-WORLD_SIMULATOR-037: As a 玩家 / 制作人, I want an `agent_direct_connect` 接入方式 on my machine, with current provider implementation `OpenClaw(Local HTTP)`, so that I can let locally installed `OpenClaw` drive low-frequency game agents through localhost without deploying remote services.
...
- 模式分层说明：按 `PRD-CORE-009`，`PRD-WORLD_SIMULATOR-039` 对应玩家访问模式 `software_safe`，`PRD-WORLD_SIMULATOR-037` 定义 `agent_direct_connect -> openclaw_local_http` 的接入方式与 provider implementation，`PRD-WORLD_SIMULATOR-040` 定义 `player_parity / headless_agent / debug_viewer` execution lane；`PRD-WORLD_SIMULATOR-041` 只定义当前研发优先级与冻结策略，不新增玩家访问模式。当前 `standard_3d` taxonomy 继续保留，但进入暂停研发态，不再接受新的 3D feature 承诺。
- Critical User Flows:
