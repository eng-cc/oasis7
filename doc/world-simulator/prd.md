...
- Success Criteria:
  ...
  - SC-108: GitHub Release 的三平台公开资产必须继续保持“每个平台一个安装包或可直接运行入口”边界：Linux `.deb` 安装后有 `/usr/bin/oasis7-client`，macOS `.dmg` 内有可双击的 `oasis7 Client Launcher.app`，Windows `.exe` 解包后默认运行 `run-client.cmd`，且 bundle 校验脚本会在平台原生入口缺失时直接失败。
