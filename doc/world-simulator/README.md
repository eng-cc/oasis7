...
- 想先进入 Viewer 热点子域，而不是在 216 份 Viewer Markdown 里盲扫：`doc/world-simulator/viewer/README.md`
...
- `viewer/README.md` 是 `viewer/` 热点子域的 landing page，负责把 216 份 Viewer Markdown 按问题分流。
...
- `viewer/`（216）：Viewer、Web 闭环、`software_safe`、2D/3D 与操作手册；先看 `viewer/README.md`，再按簇进入 `viewer-manual.manual.md` 或具体专题。
...
- `doc/world-simulator/viewer/` 当前共有 216 份 Markdown；模块级入口只负责把你送到 `viewer/README.md`，不再在根入口直接承担 Viewer 子域分流。
