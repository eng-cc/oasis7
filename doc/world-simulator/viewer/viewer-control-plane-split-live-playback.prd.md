...
- `accepted` 可表示无 mutation 的 Preview 或 authenticated verified no-op；AC-PROMPT-004 的 Preview 必须明确 `preview=true`，scope 全为 `none`，并以 profile/event count 证明未 mutation。`applied` 只表示 authority 已 commit，并携带 `mutation_count=1` 与 runtime-instance scope。`blocked` 表示 authority 未应用；`control_lost`、`result_unknown` 是稳定 reason code。`stale` 表示控制仍有效但 commit 时 version/digest 不匹配；旧 patch 不应用。`rejected` 是 deterministic malformed/unsupported/unknown-target/request-ID conflict，不能 silent retry。
