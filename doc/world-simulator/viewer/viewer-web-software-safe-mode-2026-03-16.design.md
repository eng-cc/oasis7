...
## 2. 核心设计决策
- 保留 `viewer.html` + `software_safe_src/**` 作为源码入口，generated bundle 以 `viewer.js` 为 canonical，并保留 `software_safe.html` / `software_safe.js` 作为 compat alias；dist / bundle / release 同步产出 `viewer.html` + `viewer.js` 与 compat 副本。
