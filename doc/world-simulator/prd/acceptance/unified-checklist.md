...
| G2 | Viewer Web 闭环 | 页面可加载、至少 1 张截图、`viewer` canonical 主入口与 `software_safe` compat 状态可追溯 | 按 `doc/testing/manual/web-ui-agent-browser-closure-manual.manual.md` 执行 S6（如 `./scripts/viewer-primary-web-entry-regression.sh`、`./scripts/viewer-software-safe-step-regression.sh`） | `output/playwright/*.png` + `output/playwright/viewer/console.log` |
