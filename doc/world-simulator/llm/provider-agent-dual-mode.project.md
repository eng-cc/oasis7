...
- [x] provider-agent-dual-mode-contract (PRD-WORLD_SIMULATOR-040) [test_tier_required]: 由 `agent_engineer` 牵头冻结 `player_parity` / `headless_agent` 的 observation/action contract、schema version 与禁止泄露的真值边界，并形成 supporting spec `provider-agent-dual-mode-contract.md`。 Trace: #2546 (task_cfcdf9b4a87e4c7992b6e59db7df657e)
...
- [x] provider-agent-dual-mode-recertification (PRD-WORLD_SIMULATOR-040) [test_tier_full]: QA/producer reran real dual-mode evidence after T4.2~T4.4 and restored the completed remediation scope. Durable reviewer evidence and both lane commands/run IDs are retained in `doc/testing/evidence/provider-agent-dual-mode-recertification-evidence-2026-04-07.md`. Trace: #2546 (task_cfcdf9b4a87e4c7992b6e59db7df657e)
...
- `doc/world-simulator/llm/provider-agent-dual-mode-contract.md`
- `testing-manual.md`
...
- 备注: 本专题的目标态没有变化，仍然是为当前 provider-backed Local Provider 组合（兼容 alias=`agent_direct_connect/provider_loopback_http`）收口“双轨 lane”的产品目标与执行边界。2026-04-06 formal review 提出的 launcher 可达性、dual-mode observation、provider handshake 与 fallback 审计链缺口，已在 `TASK-WORLD_SIMULATOR-295~298` 完成 remediation 并由 retained recertification evidence 复核。
...
## 2026-04 review / recertification durable disposition

| Historical record | Durable disposition | Evidence and limit |
| --- | --- | --- |
| 2026-04-06 direct-connect review | Gaps in launcher lane reachability, observation separation/no hidden truth, timeout policy, capability/profile handshake, and `fallback_reason` audit were remediated by TASK-WORLD_SIMULATOR-295~298. | This migration records the disposition; it does not perform a new independent audit. |
| 2026-04-07 dual-mode recertification | `player_parity` and `headless_agent` each had a one-sample real lane run with mode/schema/environment, capabilities, supported action sets, compatibility status and fallback reason retained. | `doc/testing/evidence/provider-agent-dual-mode-recertification-evidence-2026-04-07.md` preserves commands/run IDs/raw-artifact locations, samples=1, 8.7-9.4s waits, and one recoverable `provider_unreachable` observation. |

`PRD-WORLD_SIMULATOR-040 completed` means the cited dual-mode contract/remediation scope closed. It does not certify broad builtin/provider parity, provider availability, cost/stability, release readiness, or default enablement; `PRD-WORLD_SIMULATOR-038` remains `behavior_parity_pass / latency_class B / keep experimental` until its own gates change.
