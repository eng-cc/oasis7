...
- T5 主链路补充: `oasis7_client_launcher` 已把 `agent_provider_mode/openclaw_base_url/openclaw_auth_token/openclaw_connect_timeout_ms/openclaw_agent_profile` 正式透传到 `oasis7_game_launcher`；后者再通过环境变量把 OpenClaw 设置注入 `oasis7_viewer_live` 的 runtime live sidecar，OpenClaw 现在可以走产品默认启动链路进入真实运行时。
