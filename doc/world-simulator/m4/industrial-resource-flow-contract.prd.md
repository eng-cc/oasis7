...
- 动态报价配置保留 `dynamic_price_enabled`、base/min/max price、scarcity adjustment、distance adjustment 与 allowed price band 六类输入；具体数值由当前配置和 runtime 测试拥有，不在本 PRD 冻结。
- 启用动态报价且提交价为零或未显式指定时，交易使用权威 current quote；显式价格超出 current quote 允许带宽时拒绝，并返回可解释的 quote/band 原因。
- current quote 由 base price 加 scarcity 与 distance 调整后钳制在 min/max 范围；相同权威状态必须产生相同报价，event/receipt 记录实际 quoted price 与 settlement amount，replay 不重新报价。
