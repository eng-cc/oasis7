...
- [ ] TASK-WORLD_SIMULATOR-297 (PRD-WORLD_SIMULATOR-037/040) [test_tier_required]: 由 `viewer_engineer` / `runtime_engineer` 为 launcher probe、runtime live snapshot、viewer debug context 与 parity summary 补 `capabilities/supported_action_sets` 兼容校验与 `fallback_reason` 透传。
...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-07 / active / `TASK-WORLD_SIMULATOR-297`（补 compatibility/fallback 审计链） / `TASK-WORLD_SIMULATOR-296`（已为 dual-mode request 落地 provider-facing observation adapter，`player_parity` 与 `headless_agent` 不再只靠 metadata 区分；bridge 现会对 `unsupported_schema_version` 与 `mode_observation_mismatch` 返回结构化失败，并补齐 fixture diff/negative tests） / `TASK-WORLD_SIMULATOR-297`、`TASK-WORLD_SIMULATOR-298`（继续修正 compatibility/fallback 审计链并重跑 dual-mode 采证） / `PRD-WORLD_SIMULATOR-041` 仍要求 3D 仅做最小维护；同时 `PRD-WORLD_SIMULATOR-040` 当前剩余阻断已收敛为 compatibility/fallback 审计链与 remediation 后的正式重采证，专题仍不能恢复 completed。 / 真实 `OpenClaw(Local HTTP)` 单 NPC 闭环试点（T5 / `experimental`）、继续优化 OpenClaw absolute wait latency，以及完成 `TASK-WORLD_SIMULATOR-298` 所需的 dual-mode 重采证；3D 可视化方向保持暂停，直至显式恢复 gate 通过。
