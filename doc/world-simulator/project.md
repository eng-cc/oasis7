...
- `crates/oasis7/src/bin/oasis7_chain_runtime.rs`、`crates/oasis7/src/bin/oasis7_game_launcher.rs`、`crates/oasis7/src/bin/oasis7_web_launcher.rs`
...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-08 / active / 优先同步 `PRD-WORLD_SIMULATOR-039` formal gameplay PASS 到 README/current-entry/release claim，并继续推进 `PRD-WORLD_SIMULATOR-037/038` 的 experimental follow-up / `TASK-WORLD_SIMULATOR-306`（已补齐 repo-owned dual-mode 复签摘要、`PRD-WORLD_SIMULATOR-040 completed` vs `PRD-WORLD_SIMULATOR-038 keep experimental` 的正式边界，以及 runtime live / software-safe 的 actual provider readiness truth 展示）；本轮追加完成 `https://api.letai.run/v1` 直连兼容修复与 `output/playwright/viewer-software-safe-step/20260408-133532/` formal gameplay PASS 复采 / 当前优先剩余项一是把该 PASS 与 dual-mode boundary 一起回写到 release/current-entry 对外口径；二是继续推进 `PRD-WORLD_SIMULATOR-037/038` 的 experimental follow-up（绝对等待时延、更多 parity 样本、单次可恢复 `provider_unreachable` 观察）。`PRD-WORLD_SIMULATOR-040` 维持 `completed`，但这不构成默认启用资格 / 当前阻断已不再是 formal gameplay 缺 provider 配置；formal gameplay PASS 已复采，当前剩余约束集中在 release claim 同步与 `PRD-WORLD_SIMULATOR-038` 的 parity/default-enable gate。`PRD-WORLD_SIMULATOR-038` 仍是 `behavior_parity_pass / latency_class B / keep experimental`，因此 Local Provider 仍不得默认启用。 / OpenClaw provider-backed 单 NPC 闭环试点与 latency 扩面继续并行；3D 可视化方向保持暂停，并继续仅作为 visual QA/screenshot surface 维护。
