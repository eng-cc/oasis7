...
- [x] TASK-WORLD_SIMULATOR-124 (PRD-WORLD_SIMULATOR-037) [test_tier_required]: 打通 `oasis7_client_launcher -> oasis7_game_launcher -> oasis7_viewer_live` 的 OpenClaw 参数透传，让产品主链路可把 `openclaw_agent_profile` 与本地 HTTP 配置送到 runtime live sidecar。
- [x] TASK-WORLD_SIMULATOR-125 (PRD-WORLD_SIMULATOR-037/038) [test_tier_required]: 新增 `oasis7_openclaw_local_bridge`，把本机已安装的 `OpenClaw Gateway/CLI` 转成 loopback-only world-simulator provider（`127.0.0.1:5841`），并完成真实 OpenClaw 主链路 / parity 冒烟。
...
- [x] TASK-WORLD_SIMULATOR-137 (PRD-WORLD_SIMULATOR-037) [test_tier_required]: 为 `oasis7` 补齐 chain node 公私钥资产说明，明确 OpenClaw real-play 默认仍可能启动 `oasis7_chain_runtime`，并把 node private key 的存储/共享/录屏/日志禁令写成 operator contract。
...
- [x] TASK-WORLD_SIMULATOR-175 (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 将 simulator LLM 配置与 longrun stress 脚本的默认 env key 优先切到 `OASIS7_LLM_*`，并保留旧品牌 LLM fallback，继续收口 agent/LLM 配置链路的低风险内部标识迁移。
  ...
  - 验收命令 (`test_tier_required`):
    ...
    - `./scripts/doc-governance-check.sh`
    - `git diff --check`
- [x] TASK-WORLD_SIMULATOR-176 (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 将 runtime live 的 OpenClaw provider / QA chat echo 默认 env key 优先切到 `OASIS7_AGENT_PROVIDER_MODE`、`OASIS7_OPENCLAW_*` 与 `OASIS7_RUNTIME_AGENT_CHAT_ECHO`，并保留旧品牌 fallback，继续收口 runtime live / launcher / QA 脚本链路的低风险内部标识迁移。
  - 产物文件:
    - `doc/world-simulator/prd.md`
    - `doc/world-simulator/project.md`
...
- [x] TASK-WORLD_SIMULATOR-183 (PRD-WORLD_SIMULATOR-037/038) [test_tier_required]: 将 OpenClaw 首期 `P0` 默认 gameplay profile id 优先切到 `oasis7_p0_low_freq_npc`，并保留旧 `oasis7_p0_low_freq_npc` 兼容别名，收口 launcher/runtime live/parity/skill 文档里的旧 profile 默认口径。
  - 产物文件:
    ...
    - `crates/oasis7/src/simulator/tests/openclaw_adapter.rs`
  - 验收命令 (`test_tier_required`):
...
- [x] TASK-WORLD_SIMULATOR-186 (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 将 `oasis7` 主包里 runtime / chain-runtime / simulator 相关回归与临时构建目录默认前缀切到 `oasis7_*`，收口执行桥、存储指标、节点积分、builtin wasm 构建与 kernel 持久化产物里残留的旧 `oasis7` / `oasis7` 内部命名。
  ...
  - 验收命令 (`test_tier_required`):
    ...
    - `./scripts/doc-governance-check.sh`
    - `git diff --check`
- [x] TASK-WORLD_SIMULATOR-187 (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 将 `oasis7_openclaw_parity_bench` 自定义 OpenClaw profile 参数解析样例切到 `oasis7_p1_memory_loop`，收口 bench 测试里残留的旧 `oasis7_*` profile 命名。
  - 产物文件:
    - `doc/world-simulator/prd.md`
    - `doc/world-simulator/project.md`
    - `crates/oasis7/src/bin/oasis7_openclaw_parity_bench.rs`
...
- [x] TASK-WORLD_SIMULATOR-195 (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 将保留旧品牌 fallback literal 的 client launcher / viewer / OpenClaw bridge 源码常量命名切到 `compat` 语义，收口源码级模块名/变量名里残留的 `LEGACY_*oasis7*` 口径。
  ...
  - 验收命令 (`test_tier_required`):
    ...
    - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_openclaw_local_bridge validate_profile_accepts_oasis7_and_rejects_removed_old_brand_profile -- --nocapture`
    - `./scripts/doc-governance-check.sh`
    - `git diff --check`
...
- [x] TASK-WORLD_SIMULATOR-197 (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 将 `oasis7` runtime 的 builtin wasm、module source compile 与 simulator LLM 兼容 env 常量/辅助函数/定向测试命名切到 `compat` 语义，收口源码级模块名/变量名里残留的旧品牌维护口径。
  ...
  - 验收命令 (`test_tier_required`):
    ...
    - `./scripts/doc-governance-check.sh`
    - `git diff --check`
- [x] TASK-WORLD_SIMULATOR-198 (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 将 `oasis7_game_launcher` 与 `viewer/runtime_live` 的 launcher path、viewer auth、OpenClaw/runtime chat echo 兼容 env 常量/辅助函数/定向测试命名切到 `compat` 语义，收口源码级模块名/变量名里残留的旧品牌维护口径。
  - 产物文件:
    - `doc/world-simulator/prd.md`
    - `doc/world-simulator/project.md`
    - `crates/oasis7/src/bin/oasis7_game_launcher.rs`
...
- [x] TASK-WORLD_SIMULATOR-233 (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 移除 `crates/oasis7/src/bin/oasis7_openclaw_local_bridge.rs` 与相关测试中的旧 profile alias `oasis7_p0_low_freq_npc`，收口 OpenClaw local bridge 当前入口到 `oasis7_p0_low_freq_npc`，并把 compat 测试改为断言旧 alias 已失效。
...
- [x] TASK-WORLD_SIMULATOR-152 (PRD-WORLD_SIMULATOR-040) [test_tier_required]: 由 `agent_engineer` 接通真实 `player_parity` 执行 lane 到 runtime live / `oasis7_game_launcher` / `oasis7_openclaw_parity_bench` / `oasis7`，并完成 `headless_agent` / `player_parity` 双 smoke 采证。
...
- [x] TASK-WORLD_SIMULATOR-156 (PRD-WORLD_SIMULATOR-038) [test_tier_required]: 由 `agent_engineer` 为 `oasis7_openclaw_local_bridge` 增加 gateway timeout -> `openclaw agent --local` fallback 与稳定 `session_id`，修复真实 `OpenClaw(Local HTTP)` 连续 timeout，并输出 `fix3` 复验证据。
...
- [x] TASK-WORLD_SIMULATOR-295 (PRD-WORLD_SIMULATOR-037/040) [test_tier_required]: 由 `viewer_engineer` 收口 client launcher 的 OpenClaw execution mode 配置与参数透传，并统一 launcher / operator timeout policy，确保 GUI 主链路可真实进入 `player_parity` 且不再默认命中 `200ms` 假超时。产物文件: `crates/oasis7_client_launcher/src/{main.rs,launcher_core.rs,config_ui.rs,main_tests.rs}`、`crates/oasis7_launcher_ui/src/lib.rs`、`crates/oasis7/src/bin/oasis7_game_launcher.rs`、`crates/oasis7/src/bin/oasis7_game_launcher/oasis7_game_launcher_tests.rs`、`doc/world-simulator/llm/llm-openclaw-agent-dual-mode-2026-03-16.project.md`、`doc/world-simulator/project.md`。验收命令 (`test_tier_required`): `env -u RUSTC_WRAPPER cargo test -p oasis7_client_launcher build_launcher_args_accepts_agent_direct_connect_alias -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7_client_launcher launch_config_defaults_enable_llm -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7_client_launcher collect_required_config_issues_requires_valid_openclaw_execution_mode -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_game_launcher parse_options_defaults -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_game_launcher parse_options_accepts_overrides -- --nocapture`
- [x] TASK-WORLD_SIMULATOR-296 (PRD-WORLD_SIMULATOR-040) [test_tier_required]: 由 `agent_engineer` / `runtime_engineer` 落地真实 `player_parity` / `headless_agent` observation adapter 与 fixture diff 验证，补 schema mismatch 结构化失败，证明 dual-mode 不再只是 metadata 标签。产物文件: `crates/oasis7/src/simulator/{decision_provider.rs,mod.rs}`、`crates/oasis7/src/simulator/tests/decision_provider.rs`、`crates/oasis7/src/bin/oasis7_openclaw_local_bridge{.rs,/support.rs,/tests.rs}`、`doc/world-simulator/llm/llm-openclaw-agent-dual-mode-2026-03-16.project.md`、`doc/world-simulator/project.md`。验收命令 (`test_tier_required`): `env -u RUSTC_WRAPPER cargo test -p oasis7 --lib provider_backed_agent_behavior_builds_mode_differentiated_observation_payloads -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7 --lib decision_request_validate_contract_rejects_player_parity_headless_fields -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_openclaw_local_bridge handle_decision_rejects_unsupported_schema_version -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_openclaw_local_bridge handle_decision_rejects_unsupported_action_schema_version -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_openclaw_local_bridge handle_decision_rejects_player_parity_request_with_headless_fields -- --nocapture`
- [x] TASK-WORLD_SIMULATOR-297 / TASK-WORLD_SIMULATOR-300 (PRD-WORLD_SIMULATOR-037/040) [test_tier_required]: 由 `viewer_engineer` / `runtime_engineer` 为 launcher provider compatibility check、runtime live snapshot、viewer debug context 与 parity summary 补 `capabilities/supported_action_sets` 兼容校验与 `fallback_reason` 透传，统一 `ready/degraded/incompatible` 判定并把 `agent_direct_connect` alias 写入 fallback 审计链；后续由 `viewer_engineer` 收口 OpenClaw compatibility-aware check 的源码与 UI 命名，避免继续把实际会读取 `/v1/provider/info` + `/v1/provider/health` 并产出 `compatibility_status` / `fallback_reason` 的 provider check 表述成普通 `probe`。产物文件: `crates/oasis7_client_launcher/src/{main.rs,launcher_core.rs,config_ui.rs,main_tests.rs,main_app_shell.rs}`、`crates/oasis7/src/simulator/{mod.rs,provider_loopback_http.rs,world_model.rs}`、`crates/oasis7/src/viewer/runtime_live/{llm_sidecar.rs,mapping.rs}`、`crates/oasis7/src/viewer/runtime_live/tests/{snapshot_progress.rs}`、`crates/oasis7/src/viewer/runtime_live/tests.rs`、`crates/oasis7/src/bin/oasis7_openclaw_parity_bench{.rs,/tests.rs}`、`doc/world-simulator/{project.md,llm/llm-openclaw-agent-dual-mode-2026-03-16.project.md}`、`.pm/tasks/task_dae20e4880a04a4388aac47cf888c85b.execution.md`。验收命令 (`test_tier_required`): `env -u RUSTC_WRAPPER CARGO_TARGET_DIR=/tmp/oasis7-task297-target cargo test -p oasis7_client_launcher check_provider_loopback_http_provider -- --nocapture`；`env -u RUSTC_WRAPPER CARGO_TARGET_DIR=/tmp/oasis7-task297-target cargo test -p oasis7 --lib runtime_openclaw_compat_snapshot -- --nocapture`；`env -u RUSTC_WRAPPER CARGO_TARGET_DIR=/tmp/oasis7-task297-target cargo test -p oasis7 --bin oasis7_openclaw_parity_bench prepare_provider_info -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7_client_launcher build_launcher_args_accepts_agent_direct_connect_alias -- --nocapture`；`git diff --check`
- [x] TASK-WORLD_SIMULATOR-301 (PRD-WORLD_SIMULATOR-037/040) [test_tier_required]: 将 OpenClaw provider 当前正式 operator-facing taxonomy 收口为 `agent_decision_source + agent_provider_backend/contract/transport/url/auth/connect_timeout_ms/profile + agent_execution_lane`，把 `agent_direct_connect/provider_loopback_http` 降为兼容 alias；同步更新 launcher/runtime_live 测试、主 PRD、OpenClaw integration/dual-mode 专题与 testing 口径。产物文件: `crates/oasis7_client_launcher/src/{main.rs,launcher_core.rs,config_ui.rs,feedback_window.rs,main_tests.rs}`、`crates/oasis7_launcher_ui/src/lib.rs`、`crates/oasis7/src/bin/oasis7_game_launcher{.rs,/oasis7_game_launcher_tests.rs}`、`crates/oasis7/src/viewer/runtime_live/{llm_sidecar.rs,tests.rs}`、`doc/world-simulator/{prd.md,project.md}`、`doc/world-simulator/llm/llm-openclaw-{local-http-provider-integration-2026-03-12,agent-dual-mode-2026-03-16}.{prd,project}.md`、`doc/core/player-access-mode-contract-2026-03-19.{prd,design,project}.md`、`testing-manual.md`、`.pm/tasks/task_bf477b3a0333411d8fa8bc2a130d1f16.execution.md`。验收命令 (`test_tier_required`): `env -u RUSTC_WRAPPER cargo test -p oasis7_client_launcher build_launcher_args_accepts_agent_direct_connect_alias -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_game_launcher openclaw_viewer_live_env_sets_provider_specific_overrides_without_builtin_llm_timeout -- --nocapture`；`env -u RUSTC_WRAPPER cargo test -p oasis7 provider_settings_from_env_parses_profile_and_timeout -- --nocapture`；`./scripts/doc-governance-check.sh`；`git diff --check`
- [x] TASK-WORLD_SIMULATOR-305 (PRD-WORLD_SIMULATOR-037/040) [test_tier_required]: 把活跃 provider 结构层的 OpenClaw naming 继续收口为 generic provider 命名，更新 canonical implementation `provider_loopback_http`、bridge/parity 二进制、runtime-agent setup/workspace 默认路径与 launcher/runtime compat 解析，让当前 operator 入口不再把 `openclaw_local_http`、`oasis7_openclaw_*` 或 `tools/openclaw/*` 当主真值。产物文件: `crates/oasis7/src/{simulator/{mod.rs,provider_loopback_adapter.rs,provider_loopback_http.rs},bin/{oasis7_provider_local_bridge.rs,oasis7_provider_parity_bench.rs,oasis7_web_launcher/runtime_paths.rs},viewer/runtime_live/{llm_sidecar.rs,control_plane.rs,mapping.rs}}`、`crates/oasis7_client_launcher/src/{launcher_core.rs,main.rs,main_tests.rs,config_ui.rs,main_app_shell.rs}`、`scripts/{provider-parity-p0.sh,setup-provider-oasis7-runtime.sh}`、`tools/provider/oasis7_provider_workspace/IDENTITY.md`、`.agents/skills/oasis7/SKILL.md`、`doc/world-simulator/{prd.md,project.md}`。验收命令 (`test_tier_required`): `bash -n scripts/provider-parity-p0.sh`；`bash -n scripts/setup-provider-oasis7-runtime.sh`；`env -u RUSTC_WRAPPER cargo check -p oasis7 --bin oasis7_provider_local_bridge --bin oasis7_provider_parity_bench --bin oasis7_game_launcher -p oasis7_client_launcher`；`./scripts/doc-governance-check.sh`；`git diff --check`
- [x] TASK-WORLD_SIMULATOR-298 (PRD-WORLD_SIMULATOR-040) [test_tier_full]: 由 `qa_engineer` / `producer_system_designer` 在 295~297 完成后重跑 dual-mode 真实采证，并重新判断 `PRD-WORLD_SIMULATOR-040` 是否可恢复 completed，同时确认与 `PRD-WORLD_SIMULATOR-038` 的 parity 口径是否仍成立。产物文件: `doc/world-simulator/llm/openclaw-agent-dual-mode-recertification-2026-04-07.md`、`doc/world-simulator/llm/llm-openclaw-agent-dual-mode-2026-03-16.project.md`、`doc/world-simulator/project.md`、`.pm/tasks/task_1851a5bec0014b97b8bc994520bfede0.execution.md`。验收命令 (`test_tier_full`): `openclaw --version`；`curl -sS http://127.0.0.1:18789/health`；`curl -sS http://127.0.0.1:5841/v1/provider/health | jq .`；`curl -sS http://127.0.0.1:5841/v1/provider/info | jq .`；`env -u RUSTC_WRAPPER CARGO_TARGET_DIR=/tmp/oasis7-task298-target bash scripts/openclaw-parity-p0.sh --openclaw-only --samples 1 --ticks 4 --timeout-ms 15000 --openclaw-base-url http://127.0.0.1:5841 --openclaw-connect-timeout-ms 15000 --openclaw-agent-profile oasis7_p0_low_freq_npc --execution-mode headless_agent`；`env -u RUSTC_WRAPPER CARGO_TARGET_DIR=/tmp/oasis7-task298-target bash scripts/openclaw-parity-p0.sh --openclaw-only --samples 1 --ticks 4 --timeout-ms 15000 --openclaw-base-url http://127.0.0.1:5841 --openclaw-connect-timeout-ms 15000 --openclaw-agent-profile oasis7_p0_low_freq_npc --execution-mode player_parity`；`git diff --check`
