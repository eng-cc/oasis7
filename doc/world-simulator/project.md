...
- [x] TASK-WORLD_SIMULATOR-297 (PRD-WORLD_SIMULATOR-037/040) [test_tier_required]: 由 `viewer_engineer` / `runtime_engineer` 为 launcher probe、runtime live snapshot、viewer debug context 与 parity summary 补 `capabilities/supported_action_sets` 兼容校验与 `fallback_reason` 透传，统一 `ready/degraded/incompatible` 判定并把 `agent_direct_connect` alias 写入 fallback 审计链。产物文件: `crates/oasis7_client_launcher/src/{main.rs,launcher_core.rs,config_ui.rs,main_tests.rs}`、`crates/oasis7/src/simulator/{mod.rs,openclaw_local_http.rs,world_model.rs}`、`crates/oasis7/src/viewer/runtime_live/{llm_sidecar.rs,mapping.rs}`、`crates/oasis7/src/viewer/runtime_live/tests/{snapshot_progress.rs}`、`crates/oasis7/src/viewer/runtime_live/tests.rs`、`crates/oasis7/src/bin/oasis7_openclaw_parity_bench{.rs,/tests.rs}`、`doc/world-simulator/llm/llm-openclaw-agent-dual-mode-2026-03-16.project.md`、`doc/world-simulator/project.md`。验收命令 (`test_tier_required`): `env -u RUSTC_WRAPPER CARGO_TARGET_DIR=/tmp/oasis7-task297-target cargo test -p oasis7_client_launcher probe_openclaw_local_http -- --nocapture`；`env -u RUSTC_WRAPPER CARGO_TARGET_DIR=/tmp/oasis7-task297-target cargo test -p oasis7 --lib runtime_openclaw_compat_snapshot -- --nocapture`；`env -u RUSTC_WRAPPER CARGO_TARGET_DIR=/tmp/oasis7-task297-target cargo test -p oasis7 --bin oasis7_openclaw_parity_bench prepare_provider_info -- --nocapture`；`git diff --check`
