...
- [x] oasis7-governance-call-surface (PRD-WORLD_SIMULATOR-037/040) [test_tier_required]: 为 `oasis7` 增加治理调用分册，明确 `snapshot` / `chat` / `prompt_control` / `gameplay_action` / `POST /v1/chain/gameplay/submit` 的产品链路、签名/auth 边界，以及 `agent_claim` 当前“可观测但无通用直调 endpoint” 的治理事实；同步回写 skill、模块 README、主 PRD 与索引。产物文件: `.agents/skills/oasis7/{SKILL.md,references/real-play-config.md,references/governance-call-surfaces.md}`、`doc/world-simulator/{README.md,prd.md,prd.index.md,project.md,llm/oasis7-governance-call-surface-2026-04-26.md}`、`.pm/tasks/task_ad328b3787db4d8699575e70b16be58f.execution.md`。验收命令 (`test_tier_required`): `./scripts/doc-governance-check.sh`；`git diff --check` Trace: .pm/tasks/task_ad328b3787db4d8699575e70b16be58f.yaml
## 依赖
...
- `doc/world-simulator/kernel/power-storage-complete-removal-2026-03-06.prd.md`、`doc/world-simulator/launcher/game-client-launcher-i18n-required-config-2026-03-02.prd.md`
...
- `crates/oasis7_client_launcher/src/{main.rs,main_app_shell.rs,app_process.rs,app_process_web.rs}`
- `crates/oasis7_client_launcher/src/{explorer_window.rs,explorer_window_view.rs}`
