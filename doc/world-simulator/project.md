...
- [x] viewer-visual-cleanup-audit (PRD-WORLD_SIMULATOR-046) [test_tier_required]: 清理 Viewer 可视化治理文档中的 generated runtime 路径与 canonical manual 引用漂移，删除未被当前 dist/freshness/manual contract 使用的 stale evidence fixture，并把 `legacy_core.js` facade split 剩余债务转成后续 viewer owner 候选任务。 Trace: .pm/tasks/task_b7bf95fd18054ef6a1fbe3401924755b.yaml
- [x] viewer-legacy-core-facade-split-cleanup (PRD-WORLD_SIMULATOR-046) [test_tier_required]: 删除未被当前文档/脚本引用的旧 Viewer 可视化证据图，拆出 `legacy_core.js` 的 constants/state/auth crypto helper，并把剩余 control/render/bootstrap debt 重定标为后续 viewer 工程债。 Trace: .pm/tasks/task_f77968a2f95a4b8fa34a542a62a0aec0.yaml
