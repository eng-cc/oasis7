...
- [x] TASK-WORLD_SIMULATOR-183 (PRD-WORLD_SIMULATOR-037/038) [test_tier_required]: 将 OpenClaw 首期 `P0` 默认 gameplay profile id 优先切到 `oasis7_p0_low_freq_npc`，并保留旧 `oasis7_p0_low_freq_npc` 兼容别名，收口 launcher/runtime live/parity/skill 文档里的旧 profile 默认口径。
  - 产物文件:
    ...
    - `crates/oasis7/src/simulator/tests/openclaw_local_http.rs`
...
- [x] TASK-WORLD_SIMULATOR-198 (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 将 `oasis7_game_launcher` 与 `viewer/runtime_live` 的 launcher path、viewer auth、OpenClaw/runtime chat echo 兼容 env 常量/辅助函数/定向测试命名切到 `compat` 语义，收口源码级模块名/变量名里残留的旧品牌维护口径。
  - 产物文件:
    ...
    - `crates/oasis7/src/viewer/runtime_live/tests.rs`
  - 验收命令 (`test_tier_required`):
    - `env -u RUSTC_WRAPPER cargo test -p oasis7 openclaw_settings_from_env -- --nocapture`
...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-07 / active / 优先推进 `PRD-WORLD_SIMULATOR-039` 的 `software_safe` 主入口实现（`TASK-WORLD_SIMULATOR-303/304`） / `TASK-WORLD_SIMULATOR-302`（已完成 `software_safe` 从 fallback 到主要正式 Web 入口的设计重写）；`TASK-WORLD_SIMULATOR-301`（OpenClaw provider operator-facing taxonomy 收口）也已完成 / `PRD-WORLD_SIMULATOR-039` 的主入口实现与 release claim 收口优先，其次继续推进 `PRD-WORLD_SIMULATOR-037/038` 的 experimental follow-up（绝对等待时延、更多 parity 样本、单次可恢复 `provider_unreachable` 观察） / 当前阻断是仓内“设计真值已切到 software_safe 主入口，但 launcher/default/manual/current-entry 仍未实现对齐”；在 `TASK-WORLD_SIMULATOR-303/304` 完成前，不得把该设计改向表述成已上线行为。 / `OpenClaw(Local HTTP)` 单 NPC 闭环试点与 latency 扩面继续并行；3D 可视化方向保持暂停，并继续仅作为 visual QA/screenshot surface 维护。
