...
- [x] viewer-pixel-world-commercial-rendering-loop (PRD-WORLD_SIMULATOR-046) [test_tier_required]: 将 pixel-world 主舞台从 renderer/debug 状态面板收口为商业化游戏棋盘，首屏优先展示目标、下一步、玩家杠杆与 agent-route 关系，并默认折叠 renderer diagnostics。 Trace: .pm/tasks/task_b399bf37eff94c44a300c55f5db739d3.yaml
- [x] simulator-kernel-persistence-state-hardening (PRD-WORLD_SIMULATOR-002/003) [test_tier_required]: 收口 `WorldKernel` snapshot/replay 的可恢复状态边界，补齐 `intel_ttl_ticks` 持久化与 legacy fallback 回归，并把进程内 cache/hook 重置语义显式固定在 persistence contract 内，避免恢复后行为配置静默漂移。 Trace: .pm/tasks/task_6780f8bf31a042dea2c929673ef8db40.yaml
...
- [x] viewer-web-entry-visual-redesign (PRD-WORLD_SIMULATOR-046) [test_tier_required]: 将 Viewer Web 主入口从等权重诊断控制台重构为世界优先的正式入口，分阶段完成结构重排、视觉语言刷新与 Web 回归收口；若后续重启下一轮结构/视觉大改，先按 `viewer-visual-companion-pilot-followup` contract 产出 IA / wireframe / layout compare artifact、推荐方向与 formal writeback，再创建新的实现 task。 Trace: .pm/tasks/task_367fa36f5a514e9ea3bc11da95cd8d5d.yaml
- [x] viewer-web-pixel-world-content-density (PRD-WORLD_SIMULATOR-046) [test_tier_required]: 为 pixel-world 主舞台补地点尺度提示、Agent-地点关系线与 gameplay/event 热点层，确保稀疏快照下首屏仍能读出世界关系。 Trace: .pm/tasks/task_f5838a7590534c93a50d3433d1103851.yaml
...
- [x] software-safe-playability-unblock (PRD-WORLD_SIMULATOR-039) [test_tier_required]: 让 `software_safe` formal summary 将 canonical `available_actions` 重新暴露为可执行入口，并在 gameplay summary 与空实体快照并存时显式标记 `runtime_snapshot_empty_entities` blocker。 Trace: .pm/tasks/task_1c5ac527bed54e969b737137fc998ab8.yaml
...
- 边界说明: direct `config.toml` / legacy direct model env LLM 决策路径不再是正式试玩入口；source/bundle formal stack 统一走 `provider_backed + remote_https`，默认通过 `--newapi-user-ref <ref>` 或 `OASIS7_NEWAPI_USER_REF` 交给云端 NewAPI bridge/provider bridge 自动映射 token。`standard_3d` 相关代码、脚本与活跃文档已移除，当前仅保留 `viewer` canonical Web 主入口与 `software_safe` compat alias。
