...
- [ ] TASK-WORLD_SIMULATOR-298 (PRD-WORLD_SIMULATOR-040) [test_tier_full]: 由 `qa_engineer` / `producer_system_designer` 在 295~297 完成后重跑 dual-mode 真实采证，并重新判断 `PRD-WORLD_SIMULATOR-040` 是否可恢复 completed，同时确认与 `PRD-WORLD_SIMULATOR-038` 的 parity 口径是否仍成立。
## 依赖
...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-07 / active / `TASK-WORLD_SIMULATOR-298`（重跑 dual-mode 真实采证并复签专题状态） / `TASK-WORLD_SIMULATOR-297`（已为 launcher probe、runtime live debug context 与 parity summary/raw 补齐 `capabilities/supported_action_sets` compatibility 判定和 `fallback_reason` 审计链，`agent_direct_connect` alias 现可见于 fallback metadata） / `TASK-WORLD_SIMULATOR-298`（完成 remediation 后的正式重采证） / `PRD-WORLD_SIMULATOR-041` 仍要求 3D 仅做最小维护；`PRD-WORLD_SIMULATOR-040` 的代码级 remediation 已完成，当前剩余阻断仅是 QA/producer 的真实 dual-mode 复采证与 completed 复签。 / 真实 `OpenClaw(Local HTTP)` 单 NPC 闭环试点（T5 / `experimental`）、继续优化 OpenClaw absolute wait latency，以及完成 `TASK-WORLD_SIMULATOR-298` 所需的 dual-mode 重采证；3D 可视化方向保持暂停，直至显式恢复 gate 通过。
