...
- [ ] TASK-WORLD_SIMULATOR-299 (PRD-WORLD_SIMULATOR-039) [test_tier_required]: 由 `viewer_engineer` 在当前 `main` 基线上重做 `software_safe` feedback UX 收口，补 prompt/chat/rollback/control 的结构化 summary/detail + diagnostics 折叠，明确 rollback 当前版本/恢复来源/下一目标语义，并为 `llm_init_failed` 等配置失败补产品级摘要与 contract test。产物文件: `doc/world-simulator/{project.md,viewer/viewer-web-software-safe-mode-2026-03-16.prd.md,viewer/viewer-web-software-safe-mode-2026-03-16.project.md}`、`crates/oasis7_viewer/{software_safe.html,software_safe.js,software_safe_src/main.jsx,software_safe_src/legacy_core.js,scripts/software-safe-feedback-contract.test.mjs}`。验收命令 (`test_tier_required`): `node crates/oasis7_viewer/scripts/software-safe-feedback-contract.test.mjs`；`node --check crates/oasis7_viewer/software_safe.js`；`./scripts/doc-governance-check.sh`；`./scripts/pm/lint.sh`；`git diff --check`
## 依赖
