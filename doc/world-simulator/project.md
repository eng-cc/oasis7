...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-07 / active / 继续推进 `PRD-WORLD_SIMULATOR-037/038` 的真实 `OpenClaw(Local HTTP)` 单 NPC 闭环试点与 latency 收口 / `TASK-WORLD_SIMULATOR-301`（已将 OpenClaw provider 正式 operator-facing taxonomy 收口为 `agent_decision_source + agent_provider_* + agent_execution_lane`，并把 `agent_direct_connect/openclaw_local_http` 降为兼容 alias） / `PRD-WORLD_SIMULATOR-037/038` 的 experimental follow-up（绝对等待时延、更多 parity 样本、单次可恢复 `provider_unreachable` 观察） / `PRD-WORLD_SIMULATOR-041` 仍要求 3D 仅做最小维护；`PRD-WORLD_SIMULATOR-040` 已恢复 `completed`，当前 world-simulator 侧 OpenClaw 剩余限制集中在 `PRD-WORLD_SIMULATOR-038` 的 latency_class 与扩面验证。 / 真实 `OpenClaw(Local HTTP)` 单 NPC 闭环试点（T5 / `experimental`）、继续优化 OpenClaw absolute wait latency、扩面 builtin/OpenClaw parity 样本；3D 可视化方向保持暂停，直至显式恢复 gate 通过。
