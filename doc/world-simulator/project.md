...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-08 / active / 优先推进 `PRD-WORLD_SIMULATOR-039` 的 LLM-enabled formal gameplay 复采与 release/current-entry 口径更新 / `TASK-WORLD_SIMULATOR-306`（已补齐 repo-owned dual-mode 复签摘要、`PRD-WORLD_SIMULATOR-040 completed` vs `PRD-WORLD_SIMULATOR-038 keep experimental` 的正式边界，以及 runtime live / software-safe 的 actual provider readiness truth 展示）；`TASK-WORLD_SIMULATOR-304`、`TASK-WORLD_SIMULATOR-305` 已完成 / 当前优先剩余项一是为 `PRD-WORLD_SIMULATOR-039` 在具备可用 LLM provider 的环境里重跑 formal gameplay PASS 并据此更新 README/current-entry/release claim；二是继续推进 `PRD-WORLD_SIMULATOR-037/038` 的 experimental follow-up（绝对等待时延、更多 parity 样本、单次可恢复 `provider_unreachable` 观察）。`PRD-WORLD_SIMULATOR-040` 维持 `completed`，但这不构成默认启用资格 / 当前阻断已收敛为“formal gameplay QA 环境缺少可用 LLM provider 配置，release-grade PASS 尚未复采”；同时 `PRD-WORLD_SIMULATOR-038` 仍是 `behavior_parity_pass / latency_class B / keep experimental`，因此 Local Provider 仍不得默认启用。 / OpenClaw provider-backed 单 NPC 闭环试点与 latency 扩面继续并行；3D 可视化方向保持暂停，并继续仅作为 visual QA/screenshot surface 维护。
