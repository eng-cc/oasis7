...
- [ ] TASK-WORLD_SIMULATOR-295 (PRD-WORLD_SIMULATOR-037/040) [test_tier_required]: 由 `viewer_engineer` 收口 client launcher 的 OpenClaw execution mode 配置与参数透传，并统一 launcher / operator timeout policy，确保 GUI 主链路可真实进入 `player_parity` 且不再默认命中 `200ms` 假超时。
...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-06 / active / `TASK-WORLD_SIMULATOR-295`（先修 client launcher execution mode 可达性与 timeout policy） / `TASK-WORLD_SIMULATOR-294`（已固化 `agent_direct_connect` 设计 / 实现 formal review，并把 dual-mode 专题从 completed 重开为 remediation） / `TASK-WORLD_SIMULATOR-295`、`TASK-WORLD_SIMULATOR-296`、`TASK-WORLD_SIMULATOR-297`（依序修正 launcher 可达性、observation 分层、compatibility/fallback 审计链） / `PRD-WORLD_SIMULATOR-041` 仍要求 3D 仅做最小维护；同时 2026-04-06 formal review 已确认 `PRD-WORLD_SIMULATOR-040` 当前仍缺 launcher `player_parity` 可达性、真实 dual-mode observation 与完整 compatibility/fallback 审计链，专题暂不能继续按 completed 口径对外表述。 / 真实 `OpenClaw(Local HTTP)` 单 NPC 闭环试点（T5 / `experimental`）、继续优化 OpenClaw absolute wait latency，以及完成 `TASK-WORLD_SIMULATOR-298` 所需的 dual-mode 重采证；3D 可视化方向保持暂停，直至显式恢复 gate 通过。
