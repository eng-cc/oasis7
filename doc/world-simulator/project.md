...
- [ ] TASK-WORLD_SIMULATOR-296 (PRD-WORLD_SIMULATOR-040) [test_tier_required]: 由 `agent_engineer` / `runtime_engineer` 落地真实 `player_parity` / `headless_agent` observation adapter 与 fixture diff 验证，补 schema mismatch 结构化失败，证明 dual-mode 不再只是 metadata 标签。
...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-06 / active / `TASK-WORLD_SIMULATOR-296`（补真实 dual-mode observation adapter 与 fixture diff） / `TASK-WORLD_SIMULATOR-295`（已为 client launcher 增加 OpenClaw execution mode 配置/透传，并把 client launcher / game launcher / operator 默认 connect timeout 对齐到 `15000ms`，GUI 主链路不再默认静默落回 `headless_agent` 或命中 `200ms` 假超时） / `TASK-WORLD_SIMULATOR-296`、`TASK-WORLD_SIMULATOR-297`（继续修正 observation 分层、compatibility/fallback 审计链） / `PRD-WORLD_SIMULATOR-041` 仍要求 3D 仅做最小维护；同时 `PRD-WORLD_SIMULATOR-040` 当前剩余阻断已收敛为真实 dual-mode observation 与完整 compatibility/fallback 审计链，专题仍不能恢复 completed。 / 真实 `OpenClaw(Local HTTP)` 单 NPC 闭环试点（T5 / `experimental`）、继续优化 OpenClaw absolute wait latency，以及完成 `TASK-WORLD_SIMULATOR-298` 所需的 dual-mode 重采证；3D 可视化方向保持暂停，直至显式恢复 gate 通过。
