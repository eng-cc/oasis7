...
- [ ] TASK-WORLD_SIMULATOR-303 / TASK-WORLD_SIMULATOR-304 (PRD-WORLD_SIMULATOR-039) [test_tier_required]: 将默认 Web 产品入口与 `render_mode` / launcher / bootstrap 语义切到 `software_safe` 主路径，并补齐 canonical 玩家语义摘要、blocked/handoff surface 与 release claim 采证；`standard` 收口为显式 visual intent 入口，`main_token_transfer` 等专门动作继续不在该 surface 暴露。
## 依赖
...
- `crates/oasis7/src/runtime/world/event_processing/action_to_event_core.rs`、`crates/oasis7/src/runtime/tests/{agent_default_modules.rs,power_bootstrap.rs}`
...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-07 / active / 优先推进 `PRD-WORLD_SIMULATOR-039` 的 `software_safe` 主入口实现（`TASK-WORLD_SIMULATOR-303/304`） / `TASK-WORLD_SIMULATOR-302`（已完成 `software_safe` 从 fallback 到主要正式 Web 入口的设计重写）；`TASK-WORLD_SIMULATOR-305`（已将活跃结构层 naming 收口到 `provider_loopback_http` / `oasis7_provider_*` / `oasis7_provider_agent`，旧 `openclaw_local_http` 仅保留兼容解析）与 `TASK-WORLD_SIMULATOR-301`（OpenClaw provider operator-facing taxonomy 收口）也已完成 / `PRD-WORLD_SIMULATOR-039` 的主入口实现与 release claim 收口优先，其次继续推进 `PRD-WORLD_SIMULATOR-037/038` 的 experimental follow-up（绝对等待时延、更多 parity 样本、单次可恢复 `provider_unreachable` 观察） / 当前阻断是仓内“设计真值已切到 software_safe 主入口，但 launcher/default/manual/current-entry 仍未实现对齐”；在 `TASK-WORLD_SIMULATOR-303/304` 完成前，不得把该设计改向表述成已上线行为。 / OpenClaw provider-backed 单 NPC 闭环试点与 latency 扩面继续并行；3D 可视化方向保持暂停，并继续仅作为 visual QA/screenshot surface 维护。
