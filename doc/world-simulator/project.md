...
## 状态: 2026-04-07 / active / 优先推进 `PRD-WORLD_SIMULATOR-039` 后续 release/current-entry 口径与 LLM-enabled formal gameplay 复采 / `TASK-WORLD_SIMULATOR-304`（已补齐新主入口 contract QA 证据：`/` 与 `render_mode=auto` -> `software_safe` PASS、显式 `render_mode=standard` visual QA PASS、formal gameplay 当前因缺少 `OASIS7_LLM_MODEL` 而 blocked）；`TASK-WORLD_SIMULATOR-303`、`TASK-WORLD_SIMULATOR-302` 与 `TASK-WORLD_SIMULATOR-305` 已完成 / `PRD-WORLD_SIMULATOR-039` 当前优先剩余项从“主入口 contract 证据缺失”转为“在具备可用 LLM provider 的环境里重跑 formal gameplay PASS，并据此更新 README/current-entry/release claim”；其次继续推进 `PRD-WORLD_SIMULATOR-037/038` 的 experimental follow-up（绝对等待时延、更多 parity 样本、单次可恢复 `provider_unreachable` 观察） / 当前阻断已从“默认入口未实现、证据未重跑”收敛为“formal gameplay QA 环境缺少可用 LLM provider 配置，release-grade PASS 尚未复采”；因此仍不得把该设计改向表述成已完成 formal Web gameplay release claim。 / OpenClaw provider-backed 单 NPC 闭环试点与 latency 扩面继续并行；3D 可视化方向保持暂停，并继续仅作为 visual QA/screenshot surface 维护。
