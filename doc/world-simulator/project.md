...
- [ ] viewer-web-entry-visual-redesign (PRD-WORLD_SIMULATOR-046) [test_tier_required]: 将 Viewer Web 主入口从等权重诊断控制台重构为世界优先的正式入口，分阶段完成结构重排、视觉语言刷新与 Web 回归收口；若后续重启下一轮结构/视觉大改，先参考 `PRD-ENGINEERING-031` 的 optional visual companion 预演边界，再创建新的实现 task。 Trace: .pm/tasks/task_367fa36f5a514e9ea3bc11da95cd8d5d.yaml
