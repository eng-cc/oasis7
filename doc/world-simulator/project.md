...
- [x] TASK-WORLD_SIMULATOR-283 (PRD-WORLD_SIMULATOR-037/038/040) [test_tier_required]: 对齐 `oasis7` skill / references / helper 与当前仓库 reality，修复仍引用 `world_*` 旧 bridge/launcher/viewer 名称的 operator 文档和 repo-backed helper，补充 `T4` 双轨默认模式与 parity `latency_class B / keep experimental` 的正式锚点。
...
- [ ] TASK-WORLD_SIMULATOR-304 (PRD-WORLD_SIMULATOR-039) [test_tier_required]: 按新的主入口 contract 重跑 formal Web gameplay vs `standard` visual QA 证据，并在证据齐备后再更新 README / current-entry / release claim 口径。
## 依赖
...
## 状态
- 更新日期 / 当前状态 / 下一任务 / 最新完成 / 当前优先任务 / 当前阻断说明 / 并行待办: 2026-04-07 / active / 优先推进 `PRD-WORLD_SIMULATOR-039` 的 formal Web vs visual QA 新证据（`TASK-WORLD_SIMULATOR-304`） / `TASK-WORLD_SIMULATOR-303`（已将默认 Web / launcher / bootstrap 语义切到 `software_safe` 主路径，并在主入口页面补齐 canonical gameplay summary、blocked/handoff surface 与 `main_token_transfer` 显式未开放口径）；`TASK-WORLD_SIMULATOR-302` 设计重写与 `TASK-WORLD_SIMULATOR-305` provider naming 收口也已完成 / `PRD-WORLD_SIMULATOR-039` 当前优先剩余项是新主入口 contract 的 QA/release claim 证据，其次继续推进 `PRD-WORLD_SIMULATOR-037/038` 的 experimental follow-up（绝对等待时延、更多 parity 样本、单次可恢复 `provider_unreachable` 观察） / 当前阻断已从“默认入口未实现”收敛为“formal gameplay vs visual QA 证据、README/current-entry 文档口径尚未重跑”；在 `TASK-WORLD_SIMULATOR-304` 完成前，不得把该设计改向表述成已完成 release claim。 / provider-backed 单 NPC 闭环试点与 latency 扩面继续并行；3D 可视化方向保持暂停，并继续仅作为 visual QA/screenshot surface 维护。
