...
- Viewer Web entry compatibility、pixel-world bridge rendering 与 industry-graph semantic zoom 的 completed project pages 已归档为历史 trace；当前专业 authority 分别是 `viewer-web-entry-compatibility.prd.md`、`viewer-pixel-world-bridge-rendering.prd.md` 与 `viewer-industry-graph-semantic-zoom.prd.md`。完成证据继续通过 Git history、GitHub task evidence 与 `.pm/github-project-sync/task-archive.jsonl` 追溯；`legacy_core` facade、tooling gate 与 `main.jsx` 边界的未偿债务只在活跃的 `doc/world-simulator/viewer/viewer-frontend-structure-standard.project.md` 路由，不得因完成态归档而视为已关闭。
- LLM skipped-tick metrics remain an active testing-governance topic until its no-Python aggregation continuity and zero/fallback coverage are repaired; it is not included in this historical compression. Completed multi-scenario evaluation, indirect-control/tick-lifecycle/memory, and industrial-debug triplets have instead been absorbed by `llm-agent-behavior` / `decision-provider-contract`, the gameplay and Viewer control contracts, `doc/world-runtime/wasm/wasm-interface.md`, and the M4 industrial-resource contract. Their historical samples and implementation logs are traceable only through Git history and GitHub task evidence; they do not prove current provider parity, release readiness, player-facing causality, or ordinary industrial progression.
