...
- Acceptance Criteria:
  ...
  - AC-8: GitHub Release 公开下载产物必须直接提供每个平台的主安装/运行包（`oasis7-linux-x86_64.AppImage`、`oasis7-macos-x64.dmg`、`oasis7-windows-x64.exe`）；Linux `oasis7-linux-x64.deb` 仅允许作为次级高级入口存在，公开下载区不得再暴露 `.tar.gz` / `.zip` 包装层作为主入口。
- Non-Goals:
