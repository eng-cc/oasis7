...
- 截图生成链路
  - 采用当前 Viewer/debug Web 闭环（S6）作为默认：repo-owned Viewer regression 脚本或 `oasis7_viewer_live` + `run-viewer-web.sh` + agent-browser；本地真实 provider-backed gameplay 另走 `run-local-letai-game-test.sh`。
