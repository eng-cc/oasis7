...
- 产品体验边界: `doc/product/player-entry-distribution/player-release-distribution-contract-2026-04-14.prd.md`；本文件继续承载发行流水线、资产校验与执行记录。
...
- [x] 任务测试与提交
- 完成内容（2026-05-10 follow-up）：`release-gate-web` 额外纳入 `viewer-primary-web-entry-regression.sh`，在 `software_safe` compat realtime regression 之前先验证公开默认 `/` 与 `render_mode=auto` 发布入口都落到 `viewer` canonical 入口；避免 `retired secondary viewer` 删除后 release gate 只验证强制 compat 路径，却漏掉真正对外发布的默认入口契约。
