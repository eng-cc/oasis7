...
- `doc/site/manual/` 负责静态手册镜像策略与 canonical/manual 映射；先读其 `README.md` 再按主专题或历史增量下钻，不替代公开页面本身。
...
- 默认活跃入口保留在 `doc/site/prd.md`、`doc/site/design.md`、`doc/site/prd.index.md` 与少量仍承担当前公开状态判断职责的正式专题；可变执行状态不写回文档树。
...
## 高密度提示
- 本页不维护容易漂移的文件数量快照；当前模块库存与热点子目录统一以 `./scripts/doc-inventory-report.sh` 为准。默认入口不再尝试把 github-pages 长表直接摊平到模块首页。
...
- 站点信息架构、公开状态口径或镜像入口变化时，优先更新 `doc/site/prd.md` / `doc/site/design.md`；新增默认首读入口或专题后，再同步回写 `doc/site/prd.index.md` 与本页“从这里开始”。
