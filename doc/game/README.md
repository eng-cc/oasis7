...
## 热点子域导航（2026-07-06 快照）
- `gameplay/`（78）：先看 `gameplay/README.md`，再按簇进入玩法骨架、留存修复、preview/beta gate、claim economy、长稳治理、agency 合同、mature-world 小玩家承接与可编程区域设施。
...
## 高密度提示
- `doc/game/` 当前共有 83 份 Markdown，其中 `doc/game/gameplay/` 占 78 份；默认入口只负责把你送到 `gameplay/README.md`，不再尝试把 gameplay 长表直接摊平到模块首页。
