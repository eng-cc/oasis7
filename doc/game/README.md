...
- `gameplay/`（81）：先看 `gameplay/README.md`，再按簇进入玩法骨架、留存修复、preview/beta gate、claim economy、长稳治理、agency 合同与 mature-world 小玩家承接。
...
## 高密度提示
- `doc/game/` 当前共有 86 份 Markdown，其中 `doc/game/gameplay/` 占 81 份；默认入口只负责把你送到 `gameplay/README.md`，不再尝试把 gameplay 长表直接摊平到模块首页。
