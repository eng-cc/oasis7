...
| 精炼恢复提交前判断 | `refine_quote`、`refine_preview`、`compound_mass_g`、`electricity_cost`、`hardware_output`、`electricity_after`、`hardware_shortfall_before`、`hardware_shortfall_after`、`first_goal_relevance`、`recommended_refine_amount`、`refine_value_class` | 当 `refine_compound` 是硬件不足恢复动作时，玩家确认前必须看懂本次精炼是否足够推进首个工厂/制成品目标 | `hidden_cost -> quoted -> confirmed_or_rerouted` | `refine_value_class` 固定为 `enough_to_advance / partial_progress / poor_power_tradeoff`；若缺少提交前 quote，标记 `refine_quote_missing`，不得用执行后 `CompoundRefined` receipt 当作通过；本条不重平衡公式、电力成本或产率 | `gameplay_designer` 冻结口径，`runtime_engineer` / `viewer_engineer` 落地，`qa_engineer` 复核 |
