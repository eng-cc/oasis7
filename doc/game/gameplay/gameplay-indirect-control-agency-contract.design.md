# Gameplay 间接控制 agency 系统设计

- 设计 ID：`DES-GAME-IA`
- 状态：`active`
- Owner role：`producer_system_designer`
- 专业共同审读：`gameplay_designer`、`runtime_engineer`、`agent_engineer`、`viewer_engineer`、`qa_engineer`
- 对应专业需求：[`PRD-GAME-014`](gameplay-indirect-control-agency-contract.prd.md#1-executive-summary)
...
- 当前事实审读基线：`b53f733ff9a733347248c75c541d99d6bc6c1613`；后续实现变化由新 Task evidence 重新绑定，不能由本文自动推断。
...
- 用稳定的 `DES-GAME-IA-*` 条款承接产品 REQ/AC 与 `PRD-GAME-014`；
...
| --- | --- | --- | --- | --- |
| [`REQ-WR-IA-001`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#req-wr-ia-001) / [`AC-WR-IA-001`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#ac-wr-ia-001) | 接受、推进、权威后果与下一步保持可区分 | [DES-GAME-IA-001](#des-game-ia-001)、[DES-GAME-IA-002](#des-game-ia-002)、[DES-GAME-IA-005](#des-game-ia-005) | gameplay / runtime / Agent | 不规定具体 schema 或 UI |
| [`REQ-WR-IA-002`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#req-wr-ia-002) / [`AC-WR-IA-002`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#ac-wr-ia-002) | 阻塞、离开与回流仍保留有效下一决策 | [DES-GAME-IA-003](#des-game-ia-003)、[DES-GAME-IA-004](#des-game-ia-004)、[DES-GAME-IA-006](#des-game-ia-006) | runtime / Agent / Viewer | 不承诺所有意图都可恢复 |
| [`REQ-WR-IA-003`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#req-wr-ia-003) / [`AC-WR-IA-003`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#ac-wr-ia-003) | 关键决策的理由、stakes、替代、纠正和结果处于同一因果链 | [DES-GAME-IA-004](#des-game-ia-004)、[DES-GAME-IA-007](#des-game-ia-007)、[DES-GAME-IA-008](#des-game-ia-008) | Agent / runtime / Viewer / QA | 不暴露私有 prompt 或内部 trace |
| [`PRD-GAME-014 causal-decision receipt`](gameplay-indirect-control-agency-contract.prd.md#21-reusable-causal-decision-receipt) | gameplay 失败签名与 receipt 语义可由系统边界实现和验证 | [DES-GAME-IA-001](#des-game-ia-001) 至 [DES-GAME-IA-009](#des-game-ia-009) | gameplay_designer | 本文不重新拥有字段表和失败裁决 |
...
<a id="des-game-ia-001"></a>
### DES-GAME-IA-001：accepted intent 身份与接受/生效边界
...
<a id="des-game-ia-002"></a>
### DES-GAME-IA-002：execution status、主因果与世界效果
...
<a id="des-game-ia-003"></a>
### DES-GAME-IA-003：bounded response、阻塞与 fallback
...
<a id="des-game-ia-004"></a>
### DES-GAME-IA-004：interrupt、reprioritize 与意图交接
...
<a id="des-game-ia-005"></a>
### DES-GAME-IA-005：有界后果可读性
...
<a id="des-game-ia-006"></a>
### DES-GAME-IA-006：resume anchor 与回流恢复
...
<a id="des-game-ia-007"></a>
### DES-GAME-IA-007：causal-decision receipt 与记忆纠正
...
<a id="des-game-ia-008"></a>
### DES-GAME-IA-008：Viewer / pure API parity、权限与隐私
...
<a id="des-game-ia-009"></a>
### DES-GAME-IA-009：稳定追踪与证据绑定

长期设计只保存 `REQ/AC -> PRD-GAME-014 -> DES-GAME-IA-* -> 验证方法`。每次实现由实际 `Task UID` 在 GitHub issue evidence 中绑定 source/integration/tested tree、PR、CI、评审和产物；任务状态、动态 HEAD 和 verdict 不回写本文。`TASK-GAME-071~075` 是玩法工作包标签，不替代 GitHub Task UID。
...
| --- | --- | --- | --- | --- | --- |
| intent submission / DES-GAME-IA-001 | Viewer/API -> Agent/runtime | caller + request/intent identity；具体 schema 由专业 authority 定义 | 重试规则必须显式 | accepted/rejected/pending 与 applied 分离 | 新字段不得改变旧状态含义 |
| interpretation / DES-GAME-IA-001, DES-GAME-IA-007 | Agent -> runtime | 关联 intent 与 Agent contract version | advisory；不得自行提交世界 | plan/reason 或结构化失败 | 旧 Agent 输出不能绕过 validation |
| authoritative result / DES-GAME-IA-002 | runtime -> journal/receipt | world/candidate + intent/action identity | canonical order；可重放 | canonical `committed/rejected/failed/pending`；blocker 由 reason/disposition 表达 | gameplay 分类保留原 disposition，不新增 runtime enum |
| agency projection / DES-GAME-IA-005, DES-GAME-IA-008 | `WorldSnapshot.player_gameplay` / receipt -> Viewer/API | 同候选、调用者、时间/版本边界 | 只读投影；local feedback 仅 provisional overlay | 四类 invariant 或结构化 unavailable | 两入口语义等价，不要求 payload 相同 |
| resume / DES-GAME-IA-006 | snapshot/journal/receipt -> continuation surface | world/session + continuation identity | 恢复必须幂等或显式冲突 | resumed/stale/replan-required | 旧快照缺字段时不得伪造当前值 |

具体字段集合继续由 [`PRD-GAME-014`](gameplay-indirect-control-agency-contract.prd.md#2-user-experience-functionality) 与各专业 authority 管理；本文只冻结跨组件语义和禁止条件。
...
| --- | --- | --- | --- | --- | --- |
| accepted vs applied | 受支持意图、固定候选 | 任一入口不把 ack 当作 world effect | [`PRD-GAME-014` acceptance](gameplay-indirect-control-agency-contract.prd.md#prd-game-014-acceptance) | contract + integration test | 具体入口 coverage 未验证 |
| bounded response | PRD 定义的观察窗口 | 无推进时升级为 blocker/fallback/replan | [`NFR-CFC-6`](gameplay-indirect-control-agency-contract.prd.md#prd-game-014-nfr) | QA matrix / session evidence | 时间阈值由专业 authority 冻结 |
| cross-entry parity | 同候选 Viewer 与 pure API | 四类 invariant 语义一致 | [`NFR-CFC-2`](gameplay-indirect-control-agency-contract.prd.md#prd-game-014-nfr) | paired API/Web comparison | payload 可不同 |
| resume/replay | 可恢复 snapshot/journal/receipt | 恢复同一意图链或明确 replan-required | AC-WR-IA-002 | replay/reconnect scenario | 跨版本兼容需候选证明 |
| memory correction | 实际使用长期记忆的决策 | 摘要、来源、纠正与结果可关联 | [`NFR-CFC-7`](gameplay-indirect-control-agency-contract.prd.md#prd-game-014-nfr) | Agent + surface scenario | 不要求未使用记忆的流程引入记忆 |
...
| --- | --- | --- | --- | --- | --- |
| [`REQ-WR-IA-001`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#req-wr-ia-001) / [`AC-WR-IA-001`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#ac-wr-ia-001) | [DES-GAME-IA-001](#des-game-ia-001)、[DES-GAME-IA-002](#des-game-ia-002)、[DES-GAME-IA-005](#des-game-ia-005) | ack 不冒充 applied；world change 与 intent 可关联 | `crates/oasis7/src/viewer/runtime_live/tests/snapshot_progress.rs#compat_snapshot_surfaces_control_feeling_contract_fields_from_gameplay_feedback`；`auth_actions_agent_chat.rs#runtime_agent_chat_acceptance_publishes_durable_primary_intent_handoff`；required runtime + same-candidate surface layer | Task issue evidence + CI/artifact | 未纳入的意图/入口 |
| [`REQ-WR-IA-002`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#req-wr-ia-002) / [`AC-WR-IA-002`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#ac-wr-ia-002) | [DES-GAME-IA-003](#des-game-ia-003)、[DES-GAME-IA-004](#des-game-ia-004)、[DES-GAME-IA-006](#des-game-ia-006) | 阻塞有恢复面；交接和回流不丢因果 | `crates/oasis7/src/viewer/runtime_live/tests/wait_resolution_quote.rs#compat_snapshot_quotes_when_a_queued_intent_can_be_safely_waited_on`；`auth_actions_agent_chat.rs#runtime_agent_chat_reprioritizes_the_durable_primary_intent`；`crates/oasis7/src/simulator/tests/agent_cognition_live_harness.rs#target_actor_continuation_is_runtime_owned_and_rejected_continuation_cannot_reenter`；required + full recovery layer | Task issue evidence + QA matrix | 跨版本/跨设备范围 |
| [`REQ-WR-IA-003`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#req-wr-ia-003) / [`AC-WR-IA-003`](../../product/world-rules-core-gameplay/indirect-control-agency-and-continuation.prd.md#ac-wr-ia-003) | [DES-GAME-IA-004](#des-game-ia-004)、[DES-GAME-IA-007](#des-game-ia-007)、[DES-GAME-IA-008](#des-game-ia-008) | reason/stakes/alternative/correction/result 同链；入口 parity | `crates/oasis7/src/simulator/tests/agent_cognition_live_harness.rs#target_actor_memory_intents_require_matching_committed_runtime_receipt_exactly_once`；[`scripts/oasis7-pure-api-parity-smoke.sh`](../../../scripts/oasis7-pure-api-parity-smoke.sh)；[`testing-manual.md` S6](../../../testing-manual.md#s6web-ui-闭环-smoke-套件l4a)；required + full + headed-browser layer | Task issue evidence + review/browser artifact | 完整 stakes/alternative/correction receipt 与真实同候选 parity 当前未证明 |
| [`PRD-GAME-014 AC`](gameplay-indirect-control-agency-contract.prd.md#prd-game-014-acceptance) / [`NFR`](gameplay-indirect-control-agency-contract.prd.md#prd-game-014-nfr) | [DES-GAME-IA-001](#des-game-ia-001) 至 [DES-GAME-IA-009](#des-game-ia-009) | 专业失败签名可定位到责任边界 | 文档互链、contract、integration、Web/API、active-LLM 分层验证 | 固定候选的 GitHub evidence | 文档 green 不证明行为 |
...
| ADR-GAME-IA-002 | 设计条款使用稳定 `DES-GAME-IA-*` | 直接用 Task/TASK-GAME 标签充当设计身份 | 长期设计与一次交付生命周期分离；ID 规范变更时迁移并留映射 |
