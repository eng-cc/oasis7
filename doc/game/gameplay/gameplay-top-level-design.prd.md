...
- **调运 lane：`direct_handoff` 与 `path_bound_transit`。** `TransferMaterial` 的玩家语义必须先区分两条不可互相冒充的 lane：`direct_handoff` 仅在 `from_ledger != to_ledger`、`distance_km = 0`、没有 `route_id`/`route_ids` 或其他显式 path binding 且 `auto_reroute = false` 时成立；它在执行步骤产生一次 source debit 与 destination credit 的 `MaterialTransferred` receipt，不进入在途、路径容量或 path reservation。任一显式 route/path binding 或 `auto_reroute = true`（即使距离为零）在路径校验/选择成功后才进入 `path_bound_transit`，保留 effective path、在途与到达边界；路径无效或不可用时 atomic reject，不进入任一 lane；正距离但未绑定路径的既有距离运输继续服从现有 `TransferMaterial` transit 合同，不得被此补充改名为 direct 或 path-bound。
- `direct_handoff` 的预览/确认必须让玩家看到“立即交接、无运输等待、无路径容量占用、无路径 tariff、无在途损耗”，不得展示会让玩家等待或预留运输的 ETA；`path_bound_transit` 必须让玩家看到 effective path、容量/预留占用、Electricity tariff、预计损耗/实际收到量与到达 tick/`ready_at`。上述是玩家语义标签，不新增 quote 字段、数值公式或 ABI；若既有 preview 同时包含执行时间与运输 ETA，必须明确二者不是同一边界。
...
- 该补充的 deterministic acceptance 至少覆盖：零距离、无绑定样例只产生一次 `MaterialTransferred`，无 pending transit、path reservation、tariff 或 transit loss；零距离显式 path 且校验/选择成功的样例只走 `MaterialTransitStarted`/completion，保留 path identity 与声明的容量/tariff/loss 结果，不能出现 immediate transfer；stale/invalid path 样例 atomic reject 或 requote 且不回退 direct；Viewer 与 pure API 对 lane、成本、时间、容量、损耗、完成边界和下一动作同义；同一 action/request identity 的重复投递、重连或 replay 不复制 transfer/arrival、reservation、production 或 delivery progress，新的 action/request identity 按新的确认独立处理。本文只冻结玩家动作、失败恢复、完成边界与 anti-abuse 验收，不新增 runtime 字段/schema/API、物流公式、UI 布局或当前实现完成声明。
...
- 来源结算或物流抵达只证明原材料批次存在，不等于它仍适用于当前配方。在批次进入 join 或首个不可逆 input sink 前，玩家必须能打开只读 `batch_quality_decision_preview`，读取 batch/profile 身份、当前 `usable / degraded / blocked / expired / quarantine / unknown`、证据与 custody segment、适用的时效/环境或下一复查边界、当前配方适用性、已占用 ledger/buffer/hold 与继续保管的机会成本、潜在损失、primary blocker 和对稳定窗口 `W` 的影响。未有权威证据时显示 `unknown` 并在消费侧保持 `blocked`；只有 profile 明确证明质量下降时才显示 `degraded`，不得以同名材料、到货状态或客户端时钟推断可用。
- 玩家只比较专业 profile 真实支持的 `consume_now`、`hold_for_revalidation`、`rework_or_salvage`、`return_or_replace_source` 与 `defer`；不支持的动作不展示也不自动 fallback。`usable` 仅在 fresh recipe-fit 复验后可消耗；`degraded` 只有在目标配方明示接受该等级时才能进入 join；`blocked / expired / quarantine / unknown` 在首个不可逆 sink 前 fail closed。等待/复验的即时收益是保留可用性或避免误投料，代价是继续占用保管、buffer 与交付时间；返工、salvage、退回或换源保留已发生的运输/保管/损耗与 parent lineage，每项处置只结算一次。不得自动续期、混批、重贴标签、免费替换或把复验本身计为产出、`W` 进度或里程碑。
- 批次质量/保管玩法验收至少使用一组 deterministic fixture 覆盖：稳定批次正常消耗；运输或保管超过声明边界；适用证据缺失；配方接受与拒绝 `degraded` 的成对样例；受支持的复验、返工/salvage 和退回/换源恢复。每个样例均须证明处置与 hold/release 各至多一次、旧 batch/custody/parent receipt 不被改写，且 Viewer、pure API 与 Agent 对状态、根因、已占用/已损失价值、配方适用性、下一动作与复查点给出同义结果。本补充只冻结玩家决策、失败恢复与 anti-abuse 验收；不冻结保质期、污染/温度/损耗/质量公式，不新增 runtime/ABI 字段、自动保管、价格平衡、UI 布局或当前已实现声明。
...
- `ScheduleRecipe` 之前还必须先完成一次配方生命周期决策，而不能只显示一个通用 `recipe-fit` blocker。最小 `recipe_lifecycle_decision_preview` 必须同时说明 `recipe_version/state/scope`、匹配的 factory capability、原材料批次数量与品质/保管适用性、必需 logistics path/capacity、power/terminal 前置、primary blocker、已占用或已消费、机会成本、对稳定窗口 `W` 的影响和下一复查点。未知权威显示 `unknown/degraded` 并禁用会引发不可逆 sink 的候选，不能显示为零成本、可用或安全。

  | recipe lifecycle state | 玩家可比较的真实动作 | 排程、恢复与 progression 边界 |
  | --- | --- | --- |
  | `pending_validation` | 查看缺失证据，选择 `wait_for_validation`、补证/复验或 `defer` | 不可排程；不产生 hold、input sink、`W` 或奖励。 |
  | `validated_pending_admission` | 查看准入范围与复核点，选择等待准入、重新报价、合法替代或延期 | 不可排程；“已验证”不得伪装为已生效。 |
  | `active` | 比较当前 batch/工厂/来源/路径/终端后排程或延期 | 只有提交时 fresh revalidation 仍通过才启动一次。 |
  | `restricted` | 比较作用域内排程、缩小/改道到合法作用域、重新授权或延期 | 只在 owner/区域/用途/工厂/时间范围内可选；越界无 sink 且不继承 `W`。 |
  | `retiring` | 选择 profile 真实支持的 drain/finish、hold、quarantine、return、rework、salvage、terminate，或显式确认 successor | 不得新排程或为新候选累积 `W`/奖励；退役前已接受的存量任务保留 pinned 旧身份，仅在 profile 允许时 drain/finish 并产生其唯一旧 receipt/效果，其他处置同样只生效一次。Successor 必须是 parent-linked 新候选且 `W=0`。 |
  | `retired` | 查看历史/处置 receipt，选择当前有效新版本或其他工业目标 | 旧版本终态且不可排程/复活；重连、重试、回放不生成新效果。 |

- 配方生命周期玩法验收至少覆盖六状态各一例，并在一条两阶段 `factory → raw-material batch → logistics → recipe → terminal` fixture 中分别注入 factory fit、原料品质/保管、path capacity 与 terminal 漂移。`pending_validation` / `validated_pending_admission` 不产生排程 sink、`W` 或奖励；`retiring` / `retired` 不产生新排程或新候选 `W`/奖励，但退役前已接受且 profile 允许 drain/finish 的旧 WIP 保留 pinned 旧身份，且仅可产生其原合同允许的一次旧 receipt/效果；`active` / 作用域内 `restricted` 只在 fresh validation 后启动一次。已接受工作保留原 recipe/factory/input/path/terminal identity，不被静默迁移；退役处置与 successor handoff 各至多产生一次效果，successor 从 `W=0` 开始。到达乱序、重连、重复提交、snapshot restore 和 replay 不改变状态/根 blocker 也不复制 sink、产出、receipt、`W` 或奖励；Viewer 与 pure API 必须对 lifecycle state、blocker、机会成本、真实可用动作和下一复查点给出同义结果。该补充只冻结玩家循环与验收，不新增 runtime 枚举/字段/事件、配方比率/数值、队列/公平算法、物流规划、UI 布局、自动换配方或当前已实现声明。
