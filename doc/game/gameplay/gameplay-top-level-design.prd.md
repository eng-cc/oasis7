...
- 首局与后续补料不能从推荐目标直接跳到采集。进入 extraction 前，玩家必须获得只读 `source_assessment`，把地图/Agent 发现提示与当前权威可采机会分开，并比较当前真实支持的 `extract_now`、`reduce_batch`、`move_to_alternate_source`、`wait_for_replenishment`、`prepare_access_or_route`、`use_another_legal_material_or_recipe` 与 `defer`。每个候选说明 source 身份、预计可满足量及估计/权威边界、材料/质量提示、访问与竞争条件、采集和物流成本、枯竭/补充状态、对当前配方或首个工业目标的影响、主要风险、`next_action` 与 `next_recheck`；没有安全来源时不得无限推荐等待、静默自动换源或把同名材料视为等价输入。
- 来源选择的核心取舍是“更近但拥挤、较远但稳定、减量尽快开工、等待补充、换材料/配方或延期”，不是刷新地图直到库存出现。提交按 fresh authoritative snapshot 重验；多个玩家/Agent 争用有限来源时只能消费专业合同公开、确定的 allocation 结果。成功只对实际获配量生成一次 source receipt 与 material batch；不足/耗尽/权限或路线漂移必须给出真实的 `full/partial/deferred/denied/expired` 结果、重新报价或无 sink 的原子拒绝。预览/扫描不锁资源或顺位，拆批、重连、retry、乱序与 replay 不刷新竞争优先级、不复制采出/补充/奖励。
