...
- [x] `viewer_engineer` / `runtime_engineer`：已收口首次进入与最小控制地板的前台控制门控与 ack 语义，让 headed Web/UI 与 `software_safe` 不再把明确 `blocked` / `no_progress` 压扁成伪 timeout；`TASK-GAME-065` 的 `hold` 只保留为 2026-04-15 历史 baseline，当前 fresh formal truth 已由 `issue-160-first-capability-closeout` 刷新为 `pass`。
