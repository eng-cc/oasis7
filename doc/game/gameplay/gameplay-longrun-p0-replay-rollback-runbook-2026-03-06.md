...
9. 玩家动作处置：对受 C→T/fork 影响的每个动作记录 `preserved_at_target`、`replayed`、`rejected_fork` 或 `compensation_required`。生产路径中，被 fork 拒绝且可归属玩家的动作必须创建 `compensation_required`，关联公开安全的 owner/ticket 别名与状态；状态推进或未知归属修复必须由 active governance rollback authority 对绑定 `issued_at_ms`、`expires_at_ms` 与一次性 nonce 的 canonical 请求签名，并通过 Viewer 与 nonce 消费记录原子持久化。过期、超出 future-skew 或重放请求必须在 mutation 前拒绝。`rejected` 是已裁决终态，不再阻塞 all-clear；缺失玩家归属必须以 batch/event/action 身份进入 census 并留下 `player_attribution_incomplete` blocker，直至授权修复。
