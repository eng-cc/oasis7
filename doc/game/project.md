...
口径更新（2026-05-17）: `PRD-GAME-012` 的正式 verdict 继续保持双层：`10-minute trust gate` 与 `first capability gate` 分开判定。2026-04-15 的 `trust gate = hold / capability gate = not_run` 现在保留为历史 baseline；当前 fresh active-LLM formal truth 已更新为 `trust gate = pass`、`first capability gate = pass`，证据见 `doc/testing/evidence/issue-160-first-capability-closeout-2026-05-17.md` 与 `issue160-trust-refresh-fix11-capability-window` artefact。
...
- 最新完成: `TASK-GAME-065`（`qa_engineer` 已建立 active-LLM formal trust/capability 双层门禁，并保留 2026-04-15 的 `hold/not_run` baseline 作为历史 failure archive；当前最新正式 truth 由后续 `issue-160-first-capability-closeout` 进一步推进并刷新。）
- 最新完成: `issue-160-first-capability-closeout`（`runtime_engineer` 已为 `environment.current_observation` 补齐 build-ready 派生上下文，并通过 `issue160-trust-refresh-fix10-build-context` 与 `issue160-trust-refresh-fix11-capability-window` 两轮 fresh formal sample 证明 active-LLM lane 已从 `20%` capability stall 推进到 `post_onboarding.choose_first_expansion_tradeoff / 92%`，当前 formal truth 为 `trust gate = pass`、`first capability gate = pass`。）
...
- 最新完成: `TASK-GAME-062`（`viewer_engineer` 已收口首次进入与最小控制地板的前台控制门控与 ack 语义，不再把明确 `blocked` / `completed_no_progress` 压扁成伪 timeout；该任务完成不等于 active-LLM provider timeout 已解除。）
